...
bool CSocketManager::Start(WORD port) // OK
{
	...
	LogAdd(LOG_BLUE,"Khởi động Port %d thành công!",this->m_port);
	return 1;
}
...
bool CSocketManager::CreateListenSocket() // OK
{
	...
	{
		LogAdd(LOG_RED,"Lỗi chưa biết: %d",WSAGetLastError());
		return 0;
	}
	...
	{
		LogAdd(LOG_RED,"Port %d đã bị trùng hoặc đang sử dụng.",this->m_port);
		LogAdd(LOG_RED,"Hãy kiểm tra lại DS trước đó còn chạy hay không.",WSAGetLastError());
		return 0;
	}
...
bool CSocketManager::CreateCompletionPort() // OK
{
	...
	{
		LogAdd(LOG_RED,"Lỗi chưa biết: %d",GetLastError());
		closesocket(socket);
		return 0;
	}
...
bool CSocketManager::CreateServerQueue() // OK
{
	...
	{
		LogAdd(LOG_RED,"[ThangCuoi] SetThreadPriority() failed with error: %d",GetLastError());
		return 0;
	}
...
bool CSocketManager::DataRecv(int index,IO_MAIN_BUFFER* lpIoBuffer) // OK
{
	...
	while(true)
	{
		...
		{
			LogAdd(LOG_RED,"[ThangCuoi] Protocol header error (Index: %d, Header: %x)",index,lpMsg[count]);
			return 0;
		}

		if(size < 3 || size > MAX_MAIN_PACKET_SIZE)
		{
			LogAdd(LOG_RED,"[ThangCuoi] Protocol size error (Index: %d, Header: %x, Size: %d, Head: %x)",index,header,size,head);
			return 0;
		}
...
bool CSocketManager::DataSend(int index,BYTE* lpMsg,int size) // OK
{
	...
	{
		LogAdd(LOG_RED,"Lỗi ID: %d, vượt quá Size mặc định %d MB/8GB)",index,size);
		this->m_critical.unlock();
		return 0;
	}
	...
	if(lpIoContext->IoSize > 0)
	{
		...
		{
			LogAdd(LOG_RED,"[ThangCuoi] Max msg size (Type: 2, Index: %d, Size: %d)",index,(lpIoContext->IoSideBuffer.size+size));
	...
	if(WSASend(lpServerManager->m_socket,&lpIoContext->wsabuf,1,&SendSize,Flags,&lpIoContext->overlapped,0) == SOCKET_ERROR)
	{
		...
		{
			LogAdd(LOG_RED,"[ThangCuoi] WSASend() failed with error: %d",WSAGetLastError());
			this->Disconnect(index);
			this->m_critical.unlock();
			return 0;
		}
...
{
	gQueryManager.ExecQuery("EXEC WZ_ResetAllData");
	gQueryManager.Close();
}
