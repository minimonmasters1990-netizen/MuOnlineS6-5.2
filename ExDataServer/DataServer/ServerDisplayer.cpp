...
void CServerDisplayer::Init(HWND hWnd)
{
	...
	HWND hReloadButton = CreateWindowW(L"BUTTON", L"Support Sever", WS_TABSTOP | WS_VISIBLE | WS_CHILD | BS_DEFPUSHBUTTON, 585, 80, 167, 30, this->m_hwnd, (HMENU)IDM_ABOUT, GetModuleHandle(NULL), NULL);
	SendMessageW(hReloadButton, WM_SETFONT, (WPARAM)this->SPK_gFont, TRUE);
	HWND hAboutButton = CreateWindowW(L"BUTTON", L"Exit DataSever", WS_TABSTOP | WS_VISIBLE | WS_CHILD | BS_DEFPUSHBUTTON, 585, 111, 167, 30, this->m_hwnd, (HMENU)IDM_EXIT, GetModuleHandle(NULL), NULL);
...
void CServerDisplayer::LogTextPaint()
{
	...
	for(int n=0;n < MAX_LOG_TEXT_LINE;n++)
	{
		switch(this->m_log[count].color)
		{
			...
			case LOG_ORANGE:	SetTextColor(hdc,Spk_vDarkGreen);	break;
