...
void CCommandManager::GDCommandBlocAccRecv(SDHP_COMMAND_BLOC_RECV* lpMsg,int index) // OK
{
	...
	pMsg.index = lpMsg->index;
	pMsg.result = 0;
	if(gQueryManager.ExecQuery("SELECT * FROM MEMB_INFO WHERE MEMB___ID='%s'",lpMsg->namebloc) == 0 || gQueryManager.Fetch() == SQL_NO_DATA)
	{
		gQueryManager.Close();
	}
	else
	{
		gQueryManager.Close();
		if (gQueryManager.ExecQuery("UPDATE MEMB_INFO SET BLOC_CODE=1,BLOC_EXPIRE=(getdate()+%d) WHERE MEMB___ID='%s'", lpMsg->days, lpMsg->namebloc) == 0)
		{
			gQueryManager.Close();
		}
		else
...
void CCommandManager::GDCommandGiftCodeRecv(SDHP_COMMAND_GIFT_RECV* lpMsg, int index) // OK
{
	...
	if (lpMsg->GioiHan == 1)
	{
		...
		if (gQueryManager.ExecQuery("SELECT * FROM GiftData WHERE GiftCode='%s'", lpMsg->GiftCode) != 0 && gQueryManager.Fetch() != SQL_NO_DATA)
		{
			gQueryManager.Close();
			pMsg.Result = 0;
		}
	...
	else if (lpMsg->GioiHan == 0)
	{

		if (gQueryManager.ExecQuery("SELECT * FROM GiftData WHERE GiftCode='%s' AND Account='%s'", lpMsg->GiftCode, lpMsg->Name) != 0 && gQueryManager.Fetch() != SQL_NO_DATA)
		{
			gQueryManager.Close();
			pMsg.Result = 0;
		}
...
void CCommandManager::GDCommandGiftCodeSave(SDHP_COMMAND_GIFT_SAVE* lpMsg, int index) // OK
{
	gQueryManager.ExecQuery("INSERT INTO GiftData (Account, GiftCode) VALUES ('%s', '%s')", lpMsg->Name, lpMsg->GiftCode);
	gQueryManager.Close();
}
