...
void CServerManager::AddServer(int index,char* ip,SOCKET socket) // OK
{
	...
	this->m_PacketTime = 0;
	LogAdd(LOG_BLACK,"Nhận diện được IP %s nội bộ.",this->m_IpAddr);
}

void CServerManager::DelServer() // OK
{
	LogAdd(LOG_BLACK,"Đã xóa IP %s khỏi danh sách kết nối.",this->m_IpAddr);
...
void CServerManager::SetServerInfo(char* name,WORD port,WORD code) // OK
{
	...
	LogAdd(LOG_ORANGE,"Cụm %s và Port %d của nhóm %d đã liên kết.",this->m_ServerName,this->m_ServerPort,this->m_ServerCode);
}
