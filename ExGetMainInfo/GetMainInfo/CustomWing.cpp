...
void CCustomWing::Load(char* path)
{
	...
	try
	{
		while(true)
		{
			...
			info.IsCape = lpMemScript->GetAsNumber();
...
void CCustomWing::SetInfo(CUSTOM_WING_INFO info)
{
	...
	//printf("%d %d %d %s %d\n", info.Index, info.ItemIndex, info.ModelType, info.ModelName, info.IsCape);
...
void CCustomWing::ConvertOptionIndex(int* OptionIndex)
{
	switch((*OptionIndex))
	{
		case 80:
