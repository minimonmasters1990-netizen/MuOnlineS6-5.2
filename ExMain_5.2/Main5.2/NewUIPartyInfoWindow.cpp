...
void CNewUIPartyInfoWindow::InitButtons()
{
	...
	for (int i = 0; i < MAX_PARTYS; i++)
	{
		...
		m_BtnPartyExit[i].ChangeButtonInfo(m_Pos.x + 159, m_Pos.y + 63 + iVal, 11, 11);
	}
}
...
void CNewUIPartyInfoWindow::RenderMemberStatue(int iIndex, PARTY_t* pMember, bool bExitBtnRender /*= false*/)
{
	...
	if (iIndex == 0)
	{
		...
		RenderImage(IMAGE_PARTY_FLAG, m_Pos.x + 146, iPosY + 23, 9, 10);
	}
	else
	{
		g_pRenderText->SetFont(g_hFont);
	...
	unicode::_sprintf(szText, GlobalText[2374], IsToolKit.QN(pMember->currHP), IsToolKit.QN(pMember->maxHP));
	g_pRenderText->RenderText(iPosX + 10, iPosY + 51, szText, 150, 0, RT3_SORT_RIGHT);
...
void CNewUIPartyInfoWindow::LoadImages()
{
	...
	LoadBitmap("Interface\\BarExpLine.tga", IMAGE_UI_BARHP, GL_LINEAR);

	LoadBitmap("Interface\\ACuoi\\PartyBarNew_1.tga", IMAGE_UI_BARHP_NEW1, GL_LINEAR);
	LoadBitmap("Interface\\ACuoi\\PartyBarNew_2.tga", IMAGE_UI_BARHP_NEW2, GL_LINEAR);
}
...
void CNewUIPartyInfoWindow::UnloadImages()
{
	...
	DeleteBitmap(IMAGE_UI_BARHP);
	DeleteBitmap(IMAGE_UI_BARHP_NEW1);
	DeleteBitmap(IMAGE_UI_BARHP_NEW2);
}
