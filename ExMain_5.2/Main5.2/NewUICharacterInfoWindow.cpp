...
SEASON3B::CNewUICharacterInfoWindow::CNewUICharacterInfoWindow()
{
	m_pNewUIMng = NULL;
	m_Pos.x = m_Pos.y = 0;
	m_StatInputBox = nullptr;
	m_SelectedStatType = 0;
}
...
bool SEASON3B::CNewUICharacterInfoWindow::Create(CNewUIManager* pNewUIMng, int x, int y)
{
	...
	m_StatInputBox = new CUITextInputBox;
	m_StatInputBox->Init(g_hWnd, 50, 14, 3, false);
	m_StatInputBox->SetFont(g_hFont);
	m_StatInputBox->SetTextColor(255, 255, 230, 210);
	m_StatInputBox->SetBackColor(0, 0, 0, 25);
	m_StatInputBox->SetState(UISTATE_HIDE);
...
void SEASON3B::CNewUICharacterInfoWindow::SetButtonInfo()
{
	...
	m_BtnTruStat[STAT_STRENGTH].ChangeButtonImgState(true, IMAGE_CHAINFO_BTN_STAT_TRU, false);
	m_BtnTruStat[STAT_STRENGTH].ChangeButtonInfo(m_Pos.x + 87, m_Pos.y + HEIGHT_STRENGTH + 2, 16, 15);

	m_BtnTruStat[STAT_DEXTERITY].ChangeButtonImgState(true, IMAGE_CHAINFO_BTN_STAT_TRU, false);
	m_BtnTruStat[STAT_DEXTERITY].ChangeButtonInfo(m_Pos.x + 87, m_Pos.y + HEIGHT_DEXTERITY + 2, 16, 15);

	m_BtnTruStat[STAT_VITALITY].ChangeButtonImgState(true, IMAGE_CHAINFO_BTN_STAT_TRU, false);
	m_BtnTruStat[STAT_VITALITY].ChangeButtonInfo(m_Pos.x + 87, m_Pos.y + HEIGHT_VITALITY + 2, 16, 15);

	m_BtnTruStat[STAT_ENERGY].ChangeButtonImgState(true, IMAGE_CHAINFO_BTN_STAT_TRU, false);
	m_BtnTruStat[STAT_ENERGY].ChangeButtonInfo(m_Pos.x + 87, m_Pos.y + HEIGHT_ENERGY + 2, 16, 15);

	m_BtnTruStat[STAT_CHARISMA].ChangeButtonImgState(true, IMAGE_CHAINFO_BTN_STAT_TRU, false);
	m_BtnTruStat[STAT_CHARISMA].ChangeButtonInfo(m_Pos.x + 87, m_Pos.y + HEIGHT_CHARISMA + 2, 16, 15);
	...
	if (gProtect->m_MainInfo.ButtonCharracter[0])
	{
...
bool SEASON3B::CNewUICharacterInfoWindow::BtnProcess()
{
	...
	{
		if (m_BtnTruStat[a].UpdateMouseEvent() == true)
		{
...
bool SEASON3B::CNewUICharacterInfoWindow::UpdateKeyEvent()
{
	...
	if (m_StatInputBox && m_StatInputBox->GetState() == UISTATE_NORMAL)
	{
		if (IsPress(VK_RETURN))
		{
			unicode::t_char szText[10] = { 0 };
			m_StatInputBox->GetText(szText);
			int iValue = atoi(szText);
			if (iValue > 0 && iValue <= 999)
			{
				for (int j = 0; j < iValue; ++j)
					//SendRequestAddPoint(m_SelectedStatType);
					SendRequestAddPoint(g_SelectedStatType);
			}
			m_StatInputBox->SetState(UISTATE_HIDE);
			return true;
		}
...
{
	if (m_StatInputBox && m_StatInputBox->GetState() == UISTATE_NORMAL)
		m_StatInputBox->DoAction();
	return true;
}

void SEASON3B::CNewUICharacterInfoWindow::RenderFrame()
{
	RenderImage(IMAGE_CHAINFO_BACK, m_Pos.x + 2, m_Pos.y + 3, 186.f, 426.f);
...
void SEASON3B::CNewUICharacterInfoWindow::RenderSubjectTexts()
{
	...
	g_pRenderText->RenderText(m_Pos.x, m_Pos.y + 10, strID, 190, 0, RT3_SORT_CENTER);

	unicode::t_char strServerName[MAX_TEXT_LENGTH];

	const char* apszGlobalText[4]
		= { gProtect->m_MainInfo.ServerName[0], gProtect->m_MainInfo.ServerName[1], gProtect->m_MainInfo.ServerName[2], gProtect->m_MainInfo.ServerName[3] };
	...
	g_pRenderText->RenderText(m_Pos.x, m_Pos.y + 32, strClassName, 190, 0, RT3_SORT_CENTER);
	g_pRenderText->SetTextColor(255, 255, 255, 127 * fAlpha);
	g_pRenderText->RenderText(m_Pos.x, m_Pos.y + 32, strServerName, 190, 0, RT3_SORT_CENTER);
}

void SEASON3B::CNewUICharacterInfoWindow::RenderTableTexts()
{
	unicode::t_char strLevel[32];
	unicode::t_char strReset[32];
	unicode::t_char strExp[128];
	unicode::t_char strPoint[128];

	CHARACTER* c = &CharactersClient[1];;
	// ACuoi Character info reset/level
	unicode::_sprintf(strLevel, GlobalText[200], CharacterAttribute->Level);
	unicode::_sprintf(strReset, "RS: %s/%s|[RL: %d]", IsToolKit.QN(CharacterAttribute->ViewReset), IsToolKit.QN(CharacterAttribute->ViewMaxReset), CharacterAttribute->ViewMReset);
	unicode::_sprintf(strExp, GlobalText[3179], IsToolKit.QN(CharacterAttribute->ViewNextExperience - CharacterAttribute->ViewExperience));	// Exp : %u/%u
	...
	if (CharacterAttribute->Level > 9)
	{
		...
		unicode::_sprintf(strPoint, "%s %d/%d | %s %d/%d", GlobalText[1412], CharacterAttribute->AddPoint, CharacterAttribute->MaxAddPoint, GlobalText[1903], iMinus, iMaxMinus);
	}
	else
	{
	...
	g_pRenderText->RenderText(m_Pos.x + 18, m_Pos.y + 88, strReset);

	if (CharacterAttribute->ViewPoint > 0)
	{
		unicode::t_char strLevelUpPoint[128];

		if (gCharacterManager.IsMasterLevel(CharacterAttribute->Class) == false || CharacterAttribute->ViewPoint > 0)
		{
			unicode::_sprintf(strLevelUpPoint, GlobalText[217], IsToolKit.QN(CharacterAttribute->ViewPoint));
		}
		else
		...
		g_pRenderText->SetBgColor(0, 0, 0, 0);
		//ACuoi ViewPoint
		g_pRenderText->RenderText(m_Pos.x + 70, m_Pos.y + 58, strLevelUpPoint, 100, 0, 4);
	}
	...
	if (GuildMark[Hero->GuildMarkIndex].GuildName != NULL && strlen(GuildMark[Hero->GuildMarkIndex].GuildName) > 0)
	{
		::CreateGuildMark(Hero->GuildMarkIndex);
		::RenderBitmap(BITMAP_GUILD, m_Pos.x + 18, m_Pos.y + 101, 8, 8);

		char GetInfoGuil[255];
		if (Hero->GuildStatus == G_PERSON)
			wsprintf(GetInfoGuil, "%s - %s", GuildMark[Hero->GuildMarkIndex].GuildName, GlobalText[1330]);
		else if (Hero->GuildStatus == G_MASTER)
			wsprintf(GetInfoGuil, "%s - %s", GuildMark[Hero->GuildMarkIndex].GuildName, GlobalText[1300]);
		else if (Hero->GuildStatus == G_SUB_MASTER)
			wsprintf(GetInfoGuil, "%s - %s", GuildMark[Hero->GuildMarkIndex].GuildName, GlobalText[1301]);
		else if (Hero->GuildStatus == G_BATTLE_MASTER)
			wsprintf(GetInfoGuil, "%s - %s", GuildMark[Hero->GuildMarkIndex].GuildName, GlobalText[1302]);

		g_pRenderText->RenderText(m_Pos.x + 30, m_Pos.y + 101, GetInfoGuil);
	}
	else
	{
...
void SEASON3B::CNewUICharacterInfoWindow::RenderAttribute()
{
	...
	DWORD wStrength;
	//ACuoi ViewStrength
	wStrength = CharacterAttribute->ViewStrength + CharacterAttribute->AddStrength;
	...
	unicode::_sprintf(strStrength, "%s", IsToolKit.QN(wStrength));
	...
	{
		DWORD wDexterity = CharacterAttribute->ViewDexterity + CharacterAttribute->AddDexterity;
	...
	{
		int nTemp = CharacterAttribute->ViewStrength + CharacterAttribute->AddStrength + CharacterAttribute->ViewDexterity + CharacterAttribute->AddDexterity;
		float fTemp = int(CharacterAttribute->ViewEnergy / 30) / 100.f;
	...
	if (iAttackRating > 0)
	{
		...
		{
			unicode::_sprintf(strAttakMamage, GlobalText[203], IsToolKit.QN(iAttackDamageMax + iMaxIndex), IsToolKit.QN(iAttackDamageMax + iMaxIndex), IsToolKit.QN(iAttackRating));
		}
		else
		{
			unicode::_sprintf(strAttakMamage, GlobalText[203], IsToolKit.QN(iAttackDamageMin + iMinIndex), IsToolKit.QN(iAttackDamageMax + iMaxIndex), IsToolKit.QN(iAttackRating));
		}
	}
	else
	{
	...
	if (iAttackRatingPK > 0)
	{
		...
		{
			unicode::_sprintf(strAttakMamage, GlobalText[2109], IsToolKit.QN(iAttackRatingPK), IsToolKit.QN(itemoption380Attack + iAttackPowerRate));
		}
		else
		{
			unicode::_sprintf(strAttakMamage, GlobalText[2044], IsToolKit.QN(iAttackRatingPK));
		}
	...
	//ACuoi ViewDexterity
	DWORD wDexterity = CharacterAttribute->ViewDexterity + CharacterAttribute->AddDexterity;
	unicode::_sprintf(strDexterity, "%s", IsToolKit.QN(wDexterity));
	...
	{
		if (iBaseClass == CLASS_MAGIC)
		{
			if ((CharacterMachine->Equipment[i].Type == -1 && (i != EQUIPMENT_HELM && iBaseClass == CLASS_MAGIC))
	...
	if (g_isCharacterBuff((&Hero->Object), eBuff_Berserker))
	{
		int nTemp = CharacterAttribute->ViewDexterity + CharacterAttribute->AddDexterity;
		float fTemp = (40 - int(CharacterAttribute->ViewEnergy / 60)) / 100.f;
	...
	{
		int _AddStat = (10 + (float)(CharacterAttribute->ViewEnergy - 80) / 10);
		if (_AddStat > 100)
			_AddStat = 100;
	...
	if (bDexSuccess)
	{
		...
		if (CharacterAttribute->SuccessfulBlocking > 0)
		{
			if (nAdd_FulBlocking)
			{
				unicode::_sprintf(strBlocking, GlobalText[206], IsToolKit.QN(t_adjdef + maxdefense + iChangeRingAddDefense),
					IsToolKit.QN(CharacterAttribute->SuccessfulBlocking), IsToolKit.QN((CharacterAttribute->SuccessfulBlocking) / 10 + nAdd_FulBlocking));
			}
			else
			{
				unicode::_sprintf(strBlocking, GlobalText[206], IsToolKit.QN(t_adjdef + maxdefense + iChangeRingAddDefense),
					IsToolKit.QN(CharacterAttribute->SuccessfulBlocking), IsToolKit.QN((CharacterAttribute->SuccessfulBlocking) / 10));
			}
		}
		else
		{
			unicode::_sprintf(strBlocking, GlobalText[207], IsToolKit.QN(t_adjdef + maxdefense + iChangeRingAddDefense), IsToolKit.QN((t_adjdef + iChangeRingAddDefense) / 10));
		}
	}
	else
	{
		if (CharacterAttribute->SuccessfulBlocking > 0)
		{
			if (nAdd_FulBlocking)
			{
				unicode::_sprintf(strBlocking, GlobalText[206], IsToolKit.QN(t_adjdef + maxdefense + iChangeRingAddDefense), IsToolKit.QN(CharacterAttribute->SuccessfulBlocking), IsToolKit.QN(nAdd_FulBlocking));
			}
			else
			{
				unicode::_sprintf(strBlocking, GlobalText[208], IsToolKit.QN(t_adjdef + maxdefense + iChangeRingAddDefense), IsToolKit.QN(CharacterAttribute->SuccessfulBlocking));
			}
		}
		else
		{
			unicode::_sprintf(strBlocking, GlobalText[209], IsToolKit.QN(t_adjdef + maxdefense + iChangeRingAddDefense));
		}
	}
	...
	unicode::_sprintf(strBlocking, GlobalText[64], IsToolKit.QN(wAttackSpeed));
	iY += 13;
	...
	{
		unicode::_sprintf(strBlocking, GlobalText[2110], IsToolKit.QN(CharacterAttribute->SuccessfulBlockingPK + (Add_Dfe)), IsToolKit.QN(itemoption380Defense + iDefenseRate));
	}
	else
	{
		unicode::_sprintf(strBlocking, GlobalText[2045], IsToolKit.QN(CharacterAttribute->SuccessfulBlockingPK + Add_Dfe));
	}
	...
	g_pRenderText->SetFont(g_hFont);
	//ACuoi ViewVitality
	DWORD wVitality = CharacterAttribute->ViewVitality + CharacterAttribute->AddVitality;
	...
	else if (g_isCharacterBuff((&Hero->Object), eBuff_Hp_up_Ourforces))
	{
		...
		wVitality = CharacterAttribute->ViewVitality + CharacterAttribute->AddVitality;
		g_pRenderText->SetTextColor(100, 150, 255, 255);
	}
	...
	unicode::_sprintf(strVitality, "%s", IsToolKit.QN(wVitality));
	g_pRenderText->SetBgColor(0);
	...
	if (gCharacterManager.IsMasterLevel(Hero->Class) == true)
	{
		unicode::_sprintf(strVitality, GlobalText[211], IsToolKit.QN(CharacterAttribute->ViewCurHP), IsToolKit.QN(Master_Level_Data.ViewMaxHP));
	}
	else
	{
		unicode::_sprintf(strVitality, GlobalText[211], IsToolKit.QN(CharacterAttribute->ViewCurHP), IsToolKit.QN(CharacterAttribute->ViewMaxHP));
	}
#else
	unicode::_sprintf(strVitality, GlobalText[211], IsToolKit.QN(CharacterAttribute->ViewCurHP), IsToolKit.QN(CharacterAttribute->ViewMaxHP));
#endif
	...
	DWORD wEnergy = CharacterAttribute->ViewEnergy + CharacterAttribute->AddEnergy;
	...
	unicode::_sprintf(strEnergy, "%s", IsToolKit.QN(wEnergy));
	...
	if (gCharacterManager.IsMasterLevel(Hero->Class) == true)
	{
		unicode::_sprintf(strEnergy, GlobalText[213], IsToolKit.QN(CharacterAttribute->ViewCurMP), IsToolKit.QN(Master_Level_Data.ViewMaxMP));
	}
	else
		unicode::_sprintf(strEnergy, GlobalText[213], IsToolKit.QN(CharacterAttribute->ViewCurMP), IsToolKit.QN(CharacterAttribute->ViewMaxMP));
#else
	unicode::_sprintf(strEnergy, GlobalText[213], IsToolKit.QN(CharacterAttribute->ViewCurMP), IsToolKit.QN(CharacterAttribute->ViewMaxMP));
#endif
	...
	if (iBaseClass == CLASS_WIZARD || iBaseClass == CLASS_MAGIC || iBaseClass == CLASS_SUMMONER)
	{
		...
		if (g_isCharacterBuff((&Hero->Object), eBuff_Berserker))
		{
			int nTemp = CharacterAttribute->ViewEnergy + CharacterAttribute->AddEnergy;
			float fTemp = int(CharacterAttribute->ViewEnergy / 30) / 100.f;
			...
			)
		{
			...
			int a = iMagicDamageMin + maxMg;
			int b = iMagicDamageMax + maxMg;
			int c = (int)((iMagicDamageMaxInitial + maxMg) * magicPercent);

			unicode::_sprintf(strEnergy, GlobalText[215], IsToolKit.QN(a), IsToolKit.QN(b), IsToolKit.QN(c));
		}
		else
		{
			int a = iMagicDamageMin + maxMg;
			...
			unicode::_sprintf(strEnergy, GlobalText[216], IsToolKit.QN(a), IsToolKit.QN(b));
		}
	...
	if (iBaseClass == CLASS_SUMMONER)
	{
		...
		if (ITEM_STAFF + 21 <= pWeaponLeft->Type && pWeaponLeft->Type <= ITEM_STAFF + 29)
		{
			...
			unicode::_sprintf(strEnergy, GlobalText[1693],
				IsToolKit.QN(iCurseDamageMin), IsToolKit.QN(iCurseDamageMax),
				IsToolKit.QN((int)((iCurseDamageMax)*fCursePercent)));
		}
		else
		{
	...
	if (iBaseClass == CLASS_WIZARD)
	{
		unicode::_sprintf(strEnergy, GlobalText[582], IsToolKit.QN(CharacterAttribute->ManaClassReg[0]));
		g_pRenderText->RenderText(m_Pos.x + 18, m_Pos.y + iY, strEnergy);
	}

	if (iBaseClass == CLASS_KNIGHT)
	{
		unicode::_sprintf(strEnergy, GlobalText[582], IsToolKit.QN(CharacterAttribute->ManaClassReg[1]));
		g_pRenderText->RenderText(m_Pos.x + 18, m_Pos.y + iY, strEnergy);
	}

	if (iBaseClass == CLASS_ELF)
	{
		unicode::_sprintf(strEnergy, GlobalText[582], IsToolKit.QN(CharacterAttribute->ManaClassReg[2]));
		g_pRenderText->RenderText(m_Pos.x + 18, m_Pos.y + iY, strEnergy);
	}

	if (iBaseClass == CLASS_MAGIC)
	{
		unicode::_sprintf(strEnergy, GlobalText[582], IsToolKit.QN(CharacterAttribute->ManaClassReg[3]));
		g_pRenderText->RenderText(m_Pos.x + 18, m_Pos.y + iY, strEnergy);
	}

	if (iBaseClass == CLASS_DARK_LORD)
	{
		unicode::_sprintf(strEnergy, GlobalText[582], IsToolKit.QN(CharacterAttribute->ManaClassReg[4]));
		g_pRenderText->RenderText(m_Pos.x + 18, m_Pos.y + iY, strEnergy);
	}

	if (iBaseClass == CLASS_SUMMONER)
	{
		unicode::_sprintf(strEnergy, GlobalText[582], IsToolKit.QN(CharacterAttribute->ManaClassReg[5]));
		g_pRenderText->RenderText(m_Pos.x + 18, m_Pos.y + iY, strEnergy);
	}

	if (iBaseClass == CLASS_RAGEFIGHTER)
	{
		unicode::_sprintf(strEnergy, GlobalText[3156], IsToolKit.QN(CharacterAttribute->ManaClassReg[6]));
		g_pRenderText->RenderText(m_Pos.x + 18, m_Pos.y + iY, strEnergy);

		iY += 13;

		unicode::_sprintf(strEnergy, GlobalText[3157], IsToolKit.QN(CharacterAttribute->ManaClassReg[7]));
		g_pRenderText->RenderText(m_Pos.x + 18, m_Pos.y + iY, strEnergy);
	}
	...
	if (iBaseClass == CLASS_DARK_LORD)
	{
		...
		wCharisma = CharacterAttribute->ViewLeadership + CharacterAttribute->AddCharisma;
		...
		unicode::_sprintf(strCharisma, "%s", IsToolKit.QN(wCharisma));
...
void SEASON3B::CNewUICharacterInfoWindow::LoadImages()
{
	...
	LoadBitmap("Interface\\MacroUI_RangeMinus.tga", IMAGE_CHAINFO_BTN_STAT_TRU, GL_LINEAR);
