...
int CSocketItemMgr::CalcSocketOptionValue(int iOptionType, float fOptionValue)
{
	switch (iOptionType)
	{
	case 1:		return int(fOptionValue);
	case 2:		return int(fOptionValue);
	case 3:
	{
		WORD wLevel;
...
void CSocketItemMgr::CheckSocketSetOption()
{
	...
	for (int i = 0; i < MAX_EQUIPMENT; ++i)
	{
		...
		for (int j = 0; j < pItem->SocketCount; ++j)
		{
			...
			{
				SOCKET_OPTION_INFO* pInfo = &m_SocketOptionInfo[SOT_SOCKET_ITEM_OPTIONS][pItem->SocketSeedID[j]];
	...
	{
		BYTE* pNeed = m_SocketOptionInfo[SOT_EQUIP_SET_BONUS_OPTIONS][i].m_bySocketCheckInfo;
		bool bMatch = true;

		for (int j = 0; j < 6; ++j)
		{
			if (iSeedSum[j] < pNeed[j]) {
				bMatch = false;
				break;
			}
		}
		...
		if (bMatch)
		{
			int id = m_SocketOptionInfo[SOT_EQUIP_SET_BONUS_OPTIONS][i].m_iOptionID;
			m_EquipSetBonusList.push_back(id);
		}
	}
}
//void CSocketItemMgr::CheckSocketSetOption()
