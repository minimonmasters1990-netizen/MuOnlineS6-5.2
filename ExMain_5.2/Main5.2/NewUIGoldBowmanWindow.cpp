...
bool CNewUIGoldBowmanWindow::UpdateMouseEvent()
{
	...
	if( m_EditBox && m_BtnSerial.UpdateMouseEvent() )
	{
		...
		if ( pNewInventoryCtrl->FindEmptySlot(2, 4, false) == -1 )
		{
...
{
	// frame
	RenderImage(IMAGE_GB_BACK, m_Pos.x + 2, m_Pos.y + 3, 186.f, 426.f);
...
void CNewUIGoldBowmanWindow::RenderTexts()
{
	...
	RenderText( Text, m_Pos.x, m_Pos.y+15, 190, 0, HEX_COLOR_WHITE, 0x00000000, RT3_SORT_CENTER );

	memset(&Text, 0, sizeof(char)*100);
	sprintf( Text, GlobalText[891]); //"100%%
	RenderText( Text, m_Pos.x, m_Pos.y+80, 190, 0, HEX_COLOR_WHITE, 0x00000000, RT3_SORT_CENTER );

	memset(&Text, 0, sizeof(char)*100);
	sprintf( Text, GlobalText[892]);
	RenderText( Text, m_Pos.x, m_Pos.y+95, 190, 0, HEX_COLOR_WHITE, 0x00000000, RT3_SORT_CENTER );

	memset(&Text, 0, sizeof(char)*100);
	sprintf( Text, GlobalText[897]);
	RenderText( Text, m_Pos.x, m_Pos.y+110, 190, 0, HEX_COLOR_WHITE, 0x00000000, RT3_SORT_CENTER );

	memset(&Text, 0, sizeof(char)*100);
	sprintf( Text, GlobalText[898]);
	RenderText( Text, m_Pos.x, m_Pos.y+125, 190, 0, HEX_COLOR_WHITE, 0x00000000, RT3_SORT_CENTER );
	...
	RenderText( Text, m_Pos.x, m_Pos.y+180, 190, 0, HEX_COLOR_WHITE, 0x00000000, RT3_SORT_CENTER );
	...
	RenderText( Text, m_Pos.x, m_Pos.y+210, 190, 0, HEX_COLOR_WHITE, 0x00000000, RT3_SORT_CENTER );
	
	memset(&Text, 0, sizeof(char)*100);
	sprintf( Text, GlobalText[917]);
	RenderText( Text, m_Pos.x, m_Pos.y+225, 190, 0, HEX_COLOR_WHITE, 0x00000000, RT3_SORT_CENTER );
