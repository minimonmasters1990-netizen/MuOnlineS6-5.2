...
void SEASON3B::CNewUIChatInputBox::SetButtonInfo()
{
	...
	m_BtnSize.ChangeButtonInfo(m_WndPos.x + FRAME_RESIZE_START_X - BUTTON_WIDTH, m_WndPos.y, BUTTON_WIDTH, BUTTON_HEIGHT);

	m_BtnTransparency.ChangeButtonImgState(true, IMAGE_INPUTBOX_BTN_TRANSPARENCY, false);

	m_BtnTransparency.ChangeButtonInfo(m_WndPos.x + TRANSPARENCY_START_X - BUTTON_WIDTH, m_WndPos.y, BUTTON_WIDTH, BUTTON_HEIGHT);
}
...
bool SEASON3B::CNewUIChatInputBox::UpdateMouseEvent()
{
	...
	//ACuoi iSelectedInputType
	int iSelectedInputType = -1;
	for (int i = 0; i < INPUT_MESSAGE_TYPE_COUNT - 1; ++i)
	{
	...
	if (CheckMouseIn(m_WndPos.x + BLOCK_WHISPER_START_X - 27, m_WndPos.y, BUTTON_WIDTH, BUTTON_HEIGHT) == true && SEASON3B::IsRelease(VK_LBUTTON))
	{
	...
	//ACuoi m_bShowChatLog
	if (m_bShowChatLog)
	{
		if (CheckMouseIn(m_WndPos.x + SYSTEM_ON_START_X - 27, m_WndPos.y, BUTTON_WIDTH, BUTTON_HEIGHT) == true && SEASON3B::IsRelease(VK_LBUTTON))
		{
			...
			if (releaseMouse)
			{
				...
				{
					m_pNewUIChatLogWnd->ShowSystemChat();
				}
				else
				{
					m_pNewUIChatLogWnd->HideSystemChat();
				}
	...
	//ACuoi m_bShowChatLog
	if (CheckMouseIn(m_WndPos.x + CHATLOG_ON_START_X - 27, m_WndPos.y, BUTTON_WIDTH, BUTTON_HEIGHT) == true && SEASON3B::IsRelease(VK_LBUTTON))
	{
	...
	//ACuoi m_iTooltipType
	if (CheckMouseIn(m_WndPos.x + FRAME_ON_START_X - 27, m_WndPos.y, BUTTON_WIDTH, BUTTON_HEIGHT) == true && SEASON3B::IsRelease(VK_LBUTTON))
	{
...
//ACuoi IMAGE_INPUTBOX_BACK
bool SEASON3B::CNewUIChatInputBox::RenderFrame()
{
	RenderImage(IMAGE_INPUTBOX_BACK, m_WndPos.x, m_WndPos.y, CHATBOX_WIDTH - 26, CHATBOX_HEIGHT);
...
void SEASON3B::CNewUIChatInputBox::RenderButtons()
{
	...
	if (m_bBlockWhisper)
	{
		//ACuoi m_bBlockWhisper
		RenderImage(IMAGE_INPUTBOX_WHISPER_ON, m_WndPos.x + BLOCK_WHISPER_START_X, m_WndPos.y, BUTTON_WIDTH, BUTTON_HEIGHT);
	}

	if (m_bShowChatLog)
	{
		RenderImage(IMAGE_INPUTBOX_CHATLOG_ON, windowX + CHATLOG_ON_START_X - BUTTON_WIDTH, windowY, BUTTON_WIDTH, BUTTON_HEIGHT);
		if (m_bOnlySystemMessage)
		{
			//ACuoi m_bShowChatLog
			RenderImage(IMAGE_INPUTBOX_CHATLOG_ON, m_WndPos.x + SYSTEM_ON_START_X + BUTTON_WIDTH, m_WndPos.y, BUTTON_WIDTH, BUTTON_HEIGHT);
		}
	}
	...
	if (m_pNewUIChatLogWnd->IsShowFrame())
	{
		//ACuoi m_pNewUIChatLogWnd
		RenderImage(IMAGE_INPUTBOX_FRAME_ON, m_WndPos.x + FRAME_ON_START_X - BUTTON_WIDTH, m_WndPos.y, BUTTON_WIDTH, BUTTON_HEIGHT);
