...
{
	int x = (640/2) + gPosWide.x_GetAddPos;
	int yPos = m_PosResult.y + 20;	
    ...
    g_pRenderText->RenderText(x, yPos, GlobalText[1147], 0, 0, RT3_WRITE_CENTER);
    yPos += 16;
    ...
    if ( m_iNumResult )
    {
		...
		yPos += 16;
        unicode::_sprintf(lpszStr, "%s", GlobalText[858]);
		g_pRenderText->RenderText(x, yPos, lpszStr, 0, 0, RT3_WRITE_CENTER);
    }
    else
    {
		unicode::_sprintf(lpszStr, "%s", GlobalText[860]);
		g_pRenderText->RenderText(x, yPos, lpszStr, 0, 0, RT3_WRITE_CENTER);
		yPos += 16;
	...
	unicode::_sprintf( lpszStr, GlobalText[861], IsToolKit.QN(pResult->m_dwExp));
	g_pRenderText->RenderText(x, yPos, lpszStr, 0, 0, RT3_WRITE_CENTER); yPos += 20;
	
    unicode::_sprintf( lpszStr, GlobalText[863], IsToolKit.QN(pResult->m_iScore) );
	g_pRenderText->RenderText(x, yPos, lpszStr, 0, 0, RT3_WRITE_CENTER); yPos += 20;
	
    unicode::_sprintf( lpszStr, GlobalText[862], IsToolKit.QN(pResult->m_iZen) );
