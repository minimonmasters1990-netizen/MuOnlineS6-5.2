...
void SEASON3B::CNewUIChatLogWindow::Init()
{
	...
	m_iCurrentRenderEndLineMsg = -1; //Separate Chat
	m_nShowingLinesMsg = 5; //Separate Chat
	m_fBackAlpha = 0.6f;
	...
	m_bShowChatLog = true;
	m_bShowSystemMesage = true;//Separate chat

	m_postitemdata.clear();
...
bool SEASON3B::CNewUIChatLogWindow::RenderBackground()
{
	if (m_bShowFrame)
	{
		...
		RenderColor(fRenderPosX, fRenderPosY, (float)m_WndSize.cx - 27, (float)m_WndSize.cy);
...
bool SEASON3B::CNewUIChatLogWindow::RenderMessages()
{
	...
	for (int i = iRenderStartLine, s = 0; i <= GetCurrentRenderEndLine(); i++, s++)
	{
		...
		{
			g_pRenderText->SetBgColor(30, 30, 30, 150);
			g_pRenderText->SetTextColor(255, 195, 20, 255);
		...
		{
			g_pRenderText->SetBgColor(30, 30, 30, 150);
			g_pRenderText->SetTextColor(255, 225, 80, 255);
		}
		// Chat Guild
		...
		{
			// Nền vàng cam (RGBA: R=255, G=165, B=0, Alpha=150)
			g_pRenderText->SetBgColor(30, 30, 30, 150);
			g_pRenderText->SetTextColor(255, 195, 20, 255);
		}
		...
		{
			g_pRenderText->SetBgColor(250, 250, 250, 60);
		...
		{
			if (m_bPointedMessage && m_iPointedMessageIndex == i)
			{
				auto it = m_postitemdata.find(m_iPointedMessageIndex);
				if (it != m_postitemdata.end() && it->second)
				{
					ITEM_ATTRIBUTE* pItemAttr = &ItemAttribute[it->second->Type];
					if (pItemAttr)
					{
						ITEM* pNewItem = g_pNewItemMng->CreateItem(it->second);
						if (pNewItem && pNewItem->Type != -1)
						{
							m_Pos.x = gPosWide.x_GetAddPos * 2;
							m_Pos.y = 100;

							RenderItemInfo(m_Pos.x, m_Pos.y, pNewItem, 0, 0, false, true);
							IsToolKit.RenderItemKit(m_Pos.x - 11, m_Pos.y - 20, pNewItem->Type, pNewItem->Level);
						}
					}
				}
			...
			//Mau Chat
			//0xFFFFFFFF La mau trang
			//0xFF9500FF la mau cam
			//0x6496FFFF la mau xanh port
			//0x00FF00FF la mau xanh la
			if (pMsgText->GetType() == TYPE_CHAT_MESSAGE)
			{
				std::string strIDUTF8;
				g_pMultiLanguage->ConvertANSIToUTF8OrViceVersa(strIDUTF8, pMsgText->GetID().c_str());

				const char* pszID = strIDUTF8.c_str();
				const char* pszText = pMsgText->GetText().c_str();

				DWORD tColorO = 0xFFFFFFFF;

				if (strIDUTF8 == HeroID)
				{
					tColorO = 0xFFFFFFFF;
				}
				DWORD tColorW = 0xFFFFFFFF;
				IsToolKit.ThisFont(ptRenderPos.x, ptRenderPos.y, tColorW, 80, 0, 0, 1, "%s : %s", pszID, pszText);
				IsToolKit.ThisFont(ptRenderPos.x, ptRenderPos.y, tColorO, 0, 0, 0, 1, "%s :", pszID);
			}
			else if (pMsgText->GetType() == TYPE_WHISPER_MESSAGE)
			{
				std::string strIDUTF8;
				g_pMultiLanguage->ConvertANSIToUTF8OrViceVersa(strIDUTF8, pMsgText->GetID().c_str());
				...
				if (strIDUTF8 == "[POST]")
				{
					std::string fullText = pszText;
					std::string Username = fullText.substr(0, 10); 
					size_t realLen = Username.find(' ');
					if (realLen != std::string::npos)
						Username = Username.substr(0, realLen); 

					std::string pszContent = fullText.substr(Username.length() + 1); 

					IsToolKit.ThisFont(ptRenderPos.x, ptRenderPos.y, 0x6496FFFF, 100, 0, 0, 1, "%s : [%s] %s", pszID, Username.c_str(), pszContent.c_str());
					IsToolKit.ThisFont(ptRenderPos.x, ptRenderPos.y, 0x6496FFFF, 0, 0, 0, 1,	"%s: %s", pszID, Username.c_str(), pszContent.c_str());
					IsToolKit.ThisFont(ptRenderPos.x, ptRenderPos.y, 0x6496FFFF, 0, 0, 0, 1,	"%s:", pszID, Username.c_str(), pszContent.c_str());
				}
				else
				{
					IsToolKit.ThisFont(ptRenderPos.x, ptRenderPos.y, 0x6496FFFF, 100, 0, 0, 1, "%s : %s", pszID, pszText);  //chat rieng
				}
			}
			else if (pMsgText->GetType() == TYPE_GUILD_MESSAGE)
			{
				...
				if (strIDUTF8 == "[MU]")
				{
					...
					IsToolKit.ThisFont(ptRenderPos.x, ptRenderPos.y, 0x00FF00FF, 100, 0, 0, 1,   "%s: %s %s", pszID, Username.c_str(), pszContent.c_str());
					IsToolKit.ThisFont(ptRenderPos.x, ptRenderPos.y, 0x00FF00FF, 0, 0, 0, 1,     "%s: %s", pszID, Username.c_str(), pszContent.c_str());
					IsToolKit.ThisFont(ptRenderPos.x, ptRenderPos.y, 0x00FF00FF, 0, 0, 0, 1,     "%s:", pszID, Username.c_str(), pszContent.c_str());
				}
				else
				{
					IsToolKit.ThisFont(ptRenderPos.x, ptRenderPos.y, 0x00FF90FF, 200, 0, 0, 1, "%s: %s", pszID, pszText);
				}
			}
			else if (pMsgText->GetType() == TYPE_POST_ITEM)
			{
				...
				DWORD tColorO = 0xFFFFFFFF;  // Trắng cho tên nhân vật
				if (strIDUTF8 == HeroID)
				{
					tColorO = 0xFFFFFFFF;    // vẫn trắng nếu là chính mình
				}
				const char* pszID = strIDUTF8.c_str();
				const char* pszText = pMsgText->GetText().c_str();

				IsToolKit.ThisFont(ptRenderPos.x, ptRenderPos.y, 0xFFDE26FF, 100, 0, 0, 1, "%s : [Sell]  %s", pszID, pszText);
				//IsToolKit.ThisFont(ptRenderPos.x, ptRenderPos.y, 0xFFDE26FF, 100, 0, 0, 1, "%s  [Sell]  ", pszID, pszText);
				IsToolKit.ThisFont(ptRenderPos.x, ptRenderPos.y, tColorO, 100, 0, 0, 1, "%s", pszID, pszText);
			}
			else
			{
				std::string strIDUTF8;
...
bool SEASON3B::CNewUIChatLogWindow::RenderMessagesNew()//Separate Chat Function
{
	if (!m_bShowSystemMesage)
		return true;
	...
	// Y static position for chat
	float fSystemChatY = 70.0f;

	float fRenderPosY = fSystemChatY + SCROLL_TOP_BOTTOM_PART_HEIGHT;
...
bool SEASON3B::CNewUIChatLogWindow::RenderFrame()
{
	if (m_bShowFrame)
	{
		...
		RenderImage(IMAGE_DRAG_BTN, fRenderPosX, fRenderPosY - (float)RESIZING_BTN_HEIGHT, RESIZING_BTN_WIDTH - 27, RESIZING_BTN_HEIGHT);
		DisableAlphaBlend();

		glColor4f(1.0f, 1.0f, 1.0f, 1.0f);

		RenderImage(IMAGE_SCROLL_TOP, fRenderPosX + m_WndSize.cx - SCROLL_BAR_WIDTH - WND_LEFT_RIGHT_EDGE - 27, fRenderPosY + WND_TOP_BOTTOM_EDGE, SCROLL_BAR_WIDTH, WND_TOP_BOTTOM_EDGE + 1);

		for (int i = 0; i < (int)GetNumberOfShowingLines(); i++)
		{
			RenderImage(IMAGE_SCROLL_MIDDLE, fRenderPosX + m_WndSize.cx - SCROLL_BAR_WIDTH - WND_LEFT_RIGHT_EDGE - 27,
		...
		RenderImage(IMAGE_SCROLL_BOTTOM, fRenderPosX + m_WndSize.cx - SCROLL_BAR_WIDTH - WND_LEFT_RIGHT_EDGE - 27,
...
void SEASON3B::CNewUIChatLogWindow::ProcessAddText(const type_string& strID, const type_string& strText, MESSAGE_TYPE MsgType, MESSAGE_TYPE ErrMsgType)
{
	...
	if (strText.size() >= 20)
	{
		...
		if (!strText1.empty())
		{
			...
			if (MsgType != TYPE_SYSTEM_MESSAGE) //Separate Chat
			{
...
int SEASON3B::CNewUIChatLogWindow::GetCurrentRenderEndLineMsg() const//SeparateChat
{
	return m_iCurrentRenderEndLineMsg;
}
...
void SEASON3B::CNewUIChatLogWindow::AddItemPost(ITEM* Data)
{
	if (!Data) { return; }
	m_postitemdata.insert(std::pair<int, ITEM*>(GetCurrentRenderEndLine() + 1, Data));
}
	...
	if (SEASON3B::CheckMouseIn(m_WndPos.x, m_WndPos.y - m_WndSize.cy, m_WndSize.cx, m_WndSize.cy))
	{
		...
		for (int i = iRenderStartLine, s = 0; i <= GetCurrentRenderEndLine(); i++, s++)
		{
			...
			if (pMsgText->GetType() == TYPE_WHISPER_MESSAGE
				...
				|| pMsgText->GetType() == TYPE_POST_ITEM
				)
			{
				float fRenderPosX = m_WndPos.x;
				float fRenderPosY = m_WndPos.y - m_WndSize.cy + SCROLL_TOP_BOTTOM_PART_HEIGHT;
...
bool SEASON3B::CNewUIChatLogWindow::Render()
{
	...
	{
		if (this->RenderMessagesNew() == false)
		{
			return false;
		}
...
void SEASON3B::CNewUIChatLogWindow::SeparateText(IN const type_string& strID, IN const type_string& strText, OUT type_string& strText1, OUT type_string& strText2)
{
	...
	size_t MaxFirstLineWidth = CLIENT_WIDTH - (size_t)(TextSize.cx / gPosWide.x_fScreenRate_x);
	...
	while (((size_t)(TextSize.cx / gPosWide.x_fScreenRate_x) > MaxFirstLineWidth) && (iLocToken > -1))
	{
...
void SEASON3B::CNewUIChatLogWindow::UpdateScrollPos()
{
	...
	if (m_EventState != EVENT_SCROLL_BTN_DOWN)
	{
		m_ScrollBtnPos.x = m_WndPos.x + m_WndSize.cx - SCROLL_BAR_WIDTH - WND_LEFT_RIGHT_EDGE - 4 - 27;
...
void SEASON3B::CNewUIChatLogWindow::ShowSystemChat()//Separate Chat
{
	m_bShowSystemMesage = true;

	type_vector_msgs* pSystemMsgs = GetMsgs(TYPE_SYSTEM_MESSAGE);
	if (pSystemMsgs == NULL)
	{
		return;
	}

	// Update render of text line 
	m_iCurrentRenderEndLineMsg = pSystemMsgs->size() - 1;
}

void SEASON3B::CNewUIChatLogWindow::HideSystemChat()//Separate Chat
{
	m_bShowSystemMesage = false;
}