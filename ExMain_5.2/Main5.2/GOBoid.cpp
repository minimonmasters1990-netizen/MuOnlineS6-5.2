...
bool IsMount(ITEM* pItem)
{
    ...
    if (pItem->Type == ITEM_HELPER + 0
        ...
        || gCustomPet.CheckCustomPetByItem(pItem->Type)
        )
    {
        return true;
    }
...
bool CreateMountSub(int Type, vec3_t Position, OBJECT* Owner, OBJECT* o, int SubType, int LinkBone)
{
    ...
    if (!o->Live)
    {
        ...
        if (gCustomPet.GetInfoPetType(o->Type - MODEL_ITEM) == 5)
        {
            o->Scale = 0.9f;
        }
        if (gCustomPet.GetInfoPetType(o->Type - MODEL_ITEM) == 9 || gCustomPet.GetInfoPetType(o->Type - MODEL_ITEM) == 10)
        {
            o->BlendMesh = 1;
            Vector(Owner->Position[0] + (float)(rand() % 512 - 256),
                Owner->Position[1] + (float)(rand() % 512 - 256),
...
bool MoveMount(OBJECT* o, bool bForceRender)
{
    if (o->Live)
    {
        ...
        if (gCustomPet.GetInfoPetType(o->Type - MODEL_ITEM) == 6)
        {
            goto GetDarHore;
        }
      ...
      GetFerrir:
        ...
        //==//Custom Pet Move
        if ((gCustomPet.GetInfoPetType(o->Type - MODEL_ITEM) >= 0 && gCustomPet.GetInfoPetType(o->Type - MODEL_ITEM) <= 4) || (gCustomPet.GetInfoPetType(o->Type - MODEL_ITEM) == 8))
        {
            float FlyRange = 185.f;
            vec3_t Range, Direction;

            if ((gCustomPet.GetInfoPetType(o->Type - MODEL_ITEM) == 8))
            {
                o->Position[2] = o->Owner->Position[2];
            }
            else
            {
                o->Position[2] = o->Owner->Position[2] + (230.0f * o->Owner->Scale);
            }

            gCustomPet.PetFollowCharacter(o, 180);

            VectorSubtract(o->Owner->Position, o->Position, Range);
            ...
            {
                float Angle = CreateAngle2D(o->Position, o->Owner->Position);
                o->Angle[2] = TurnAngle2(o->Angle[2], Angle, 10.0f * FPS_ANIMATION_FACTOR);
            }
            ...
            float Speed = (FlyRange * FlyRange >= Distance) ? 0 : (float)log(Distance) + 1.1f;
        ...
        if (o->Type == MODEL_HELPER || o->Type == MODEL_HELPER + 1 || (gCustomPet.GetInfoPetType(o->Type - MODEL_ITEM) == 9) || (gCustomPet.GetInfoPetType(o->Type - MODEL_ITEM) == 10))
        {
            vec3_t Range;
    ...
    if (o->Live)
    {
        ...
        if (o->Visible)
        {
            ...
            if (gCustomPet.CheckCustomPetByItem(o->Type - MODEL_ITEM))
            {
                ...
                {
                    o->Scale = gCustomPet.GetScaleView(o->Type - MODEL_ITEM);
                }
                else
                {

                    o->Scale = gCustomPet.GetScaleView(o->Type - MODEL_ITEM) + 0.2;
                }
            }
...
void RenderDarkHorseSkill(OBJECT* o, BMD* b)
{
    ...
    {
        int numRings = 3;
        int stonesPerRing = 3 + rand() % 3;
        float ringSpacing = 150.f + rand() % 30;

        for (int r = 0; r < numRings; ++r)
        {
            float radius = (r + 1) * ringSpacing;

            for (int i = 0; i < stonesPerRing; ++i)
            {
                float angleDeg = (360.0f / stonesPerRing) * i + (rand() % 60 - 30);
                float angleRad = angleDeg * 3.14159265f / 180.0f;

                vec3_t Position;
                Position[0] = o->Position[0] + radius * cosf(angleRad);
                Position[1] = o->Position[1] + radius * sinf(angleRad);
                Position[2] = o->Position[2];

                CreateEffectFpsChecked(MODEL_GROUND_STONE + rand() % 2, Position, o->Angle, o->Light);
            }
        }
