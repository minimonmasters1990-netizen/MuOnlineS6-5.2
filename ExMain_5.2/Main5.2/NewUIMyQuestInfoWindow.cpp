...
{
	RenderImage(IMAGE_MYQUEST_BACK, m_Pos.x + 2, m_Pos.y + 3, 186.f, 426.f);
...
void CNewUIMyQuestInfoWindow::SetSelQuestSummary()
{
	...
	m_QuestContentsListBox.AddText(
		g_hFont, 0xff0ab9ff, RT3_SORT_CENTER, g_QuestMng.GetSubject(dwSelQuestIndex));
...
void CNewUIMyQuestInfoWindow::SetSelQuestRequestReward()
{
	...
	for (j = 0; j < 3; ++j)
	{
		if (0 == j)
		{
			m_QuestContentsListBox.AddText(g_hFont, HEX_COLOR_WHITE, RT3_SORT_LEFT, " ");
