...
void CMonkSystem::RegistItem()
{
	...
	for (auto it = gCustomBattleGloves.m_CustomGlovesInfo.begin(); it != gCustomBattleGloves.m_CustomGlovesInfo.end(); ++it)
	{
		if (it->second.ItemIndex != -1)
		{
			m_cItemEqualType.SetModelType(it->second.ItemIndex + MODEL_ITEM, it->second.TextureLeftIndex + MODEL_ITEM, it->second.TextureRightIndex + MODEL_ITEM);
			m_mapItemEqualType.insert(tm_ItemEqualType::value_type(m_cItemEqualType.GetModelType(), m_cItemEqualType));
		}
	}
}
...
bool CMonkSystem::IsSwordformGlovesUseSkill(int _nSkill)
{
	...
	case AT_SKILL_DRAGON_KICK:
	{
		ITEM* pLeft = &CharacterMachine->Equipment[EQUIPMENT_WEAPON_LEFT];
		ITEM* pRight = &CharacterMachine->Equipment[EQUIPMENT_WEAPON_RIGHT];

		if (pLeft->Type == -1 && pRight->Type == -1)
		{
			return true;
		}

		if (g_CMonkSystem.IsSwordformGlovesItemType(pLeft->Type) || g_CMonkSystem.IsSwordformGlovesItemType(pRight->Type))
		{
			return true;
		}
...
bool CMonkSystem::RageCreateEffect(OBJECT* _pObj, int _nSkill)
{
	...
	{
	case AT_SKILL_THRUST:
	case MASTER_SKILL_ADD_LARGE_RING_BLOWER_IMPROVED: // SkillFix
	case MASTER_SKILL_ADD_LARGE_RING_BLOWER_ENHANCED: // SkillFix
	{
		if (m_bUseEffectOnce)
			return false;
	...
	return true;
	case AT_SKILL_STAMP:
	case MASTER_SKILL_ADD_UPPER_BEAST_IMPROVED: // SkillFix
	{
		if (m_bUseEffectOnce)
			return false;
	...
	return true;
	case AT_SKILL_GIANTSWING:
	case MASTER_SKILL_ADD_CHAIN_DRIVER_IMPROVED: // SkillFix
	case MASTER_SKILL_ADD_CHAIN_DRIVER_ENHANCED: // SkillFix
	{
	...
	case MASTER_SKILL_ADD_DRAGON_LORE_ENHANCED:
	{
		...
		vec3_t vLight;
		Vector(1.0f, 1.0f, 1.0f, vLight);
		CreateEffectFpsChecked(BITMAP_LAVA, _pObj->Position, _pObj->Angle, vLight, 0, _pObj, -1, 0, 0, 0, 1.0f);	//fpschecked fix gam thet RF aLongfix5)
	...
	break;
	case AT_SKILL_DARKSIDE:
	case MASTER_SKILL_ADD_DARK_SIDE_IMPROVED:	//them case fix hien thi hieu ung AALH master aLongfix15)
	case MASTER_SKILL_ADD_DARK_SIDE_ENHANCED:	//them case fix hien thi hieu ung AALH master aLongfix15)
	{
		if (m_bUseEffectOnce)
			return false;
