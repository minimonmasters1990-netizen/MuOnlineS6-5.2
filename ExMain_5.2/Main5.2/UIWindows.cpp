	...
	case UI_MESSAGE_BTNLCLICK:
		{
			...
			case 1:	
				if (m_iShowType == 1)
				{
					...
					SetLimitSize(250 + 80, 150, 540 / gPosWide.x_fScreenRate_x + 80);
					m_ChatListBox.SetResizeType(3, -80 - 80, -16);
				...
				else if (m_iShowType >= 2)
				{
					m_iShowType = 1;
					SetSize(GetWidth() - 80, GetHeight());
					SetLimitSize(250, 150, 540 / gPosWide.x_fScreenRate_x);
...
void CUIPhotoViewer::RenderPhotoCharacter()
{
	...
	glPushMatrix();
	glLoadIdentity();
	glViewport2(m_iPos_x * gPosWide.x_fScreenRate_x, m_iPos_y * gPosWide.x_fScreenRate_y, m_iWidth * gPosWide.x_fScreenRate_x, 141 * gPosWide.x_fScreenRate_y);
	gluPerspective2(1.f,(float)(m_iWidth * gPosWide.x_fScreenRate_x)/(float)(141 * gPosWide.x_fScreenRate_y), 2000, 20000);//CameraViewNear,CameraViewFar);
...
void CUIPhotoViewer::Render()
{
	...
	//ACuoi Clear Icon Mouse 3DModel E
	/*
	if (CheckOption(UIPHOTOVIEWER_CANCONTROL))
	{
		glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
		...
		else
		{
			...
			TextSize.cy /= gPosWide.x_fScreenRate_y;
...
void CUILetterWriteWindow::Init(const char * pszTitle, DWORD dwParentID)
{
	...
	size.cx = (size.cx / gPosWide.x_fScreenRate_x) + 0.5f;
...
//		}
	...
	g_pRenderText->SetTextColor(230, 220, 200, 255);
	g_pMultiLanguage->_GetTextExtentPoint32(g_pRenderText->GetFontDC(), GlobalText[1000], lstrlen(GlobalText[1000]), &size);
	g_pRenderText->RenderText(RPos_x(3), RPos_y(3), GlobalText[1000], size.cx/gPosWide.x_fScreenRate_x, 0, RT3_SORT_RIGHT);
	g_pRenderText->RenderText(RPos_x(3), RPos_y(18), GlobalText[1005], size.cx/gPosWide.x_fScreenRate_x, 0, RT3_SORT_RIGHT);
...
void CUIFriendWindow::RenderSub()
{
	...
	if (m_FriendListWnd.GetTitle() != NULL)
	{
		...
		g_pRenderText->RenderText(RPos_x(0) + (52 - (float)TextSize.cx / gPosWide.x_fScreenRate_x + 0.5f) / 2,
			RPos_y(0) + (24 - (float)TextSize.cy / gPosWide.x_fScreenRate_y + 0.5f) / 2, m_FriendListWnd.GetTitle());
	}
	if (m_LetterBoxWnd.GetTitle() != NULL)
	{
		...
		g_pRenderText->RenderText(RPos_x(54) + (52 - (float)TextSize.cx / gPosWide.x_fScreenRate_x + 0.5f) / 2,RPos_y(0) + (24 - (float)TextSize.cy / gPosWide.x_fScreenRate_y + 0.5f) / 2, m_LetterBoxWnd.GetTitle());
	}
	if (m_ChatRoomListWnd.GetTitle() != NULL)
	{
		...
		g_pRenderText->RenderText(RPos_x(107) + (52 - (float)TextSize.cx / gPosWide.x_fScreenRate_x + 0.5f) / 2,	RPos_y(0) + (24 - (float)TextSize.cy / gPosWide.x_fScreenRate_y + 0.5f) / 2, m_ChatRoomListWnd.GetTitle());
	}

	g_pRenderText->SetTextColor(230, 220, 200, 255);
	g_pMultiLanguage->_GetTextExtentPoint32(g_pRenderText->GetFontDC(), GlobalText[1035], strlen(GlobalText[1035]), &TextSize );
	g_pRenderText->RenderText(RPos_x(0) + RWidth() - (float)TextSize.cx / gPosWide.x_fScreenRate_x - 2, RPos_y(0) + (24 - (float)TextSize.cy / gPosWide.x_fScreenRate_y + 0.5f) / 2,	GlobalText[1035]);
	
	float fCheckBoxPos_x = RPos_x(0) + RWidth() - (float)TextSize.cx / gPosWide.x_fScreenRate_x - 2 - 14;
	float fCheckBoxPos_y = RPos_y(0) + (24 - (float)TextSize.cy / gPosWide.x_fScreenRate_y + 0.5f) / 2;
...
void CUIFriendMenu::RenderSub()
{
	if (m_fLineHeight == 0)
	{
		...
		m_fLineHeight = TextSize.cy / gPosWide.x_fScreenRate_y;
	}
