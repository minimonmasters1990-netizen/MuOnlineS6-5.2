...
void CreateFrustrum2D(vec3_t Position)
{
	...
	else
	{
		...
		{
			Width = (float)GetScreenWidth() / GetWindowsX();// * 0.1f;
		...
		{
			Width = (float)GetScreenWidth() / GetWindowsX() * 1.0f;
		...
		else
		{
			...
			case 0:
				...
				{
					//Width = (float)GetScreenWidth() / GetWindowsX() * 9.1f * 0.404998f;
					Width = (float)GetScreenWidth() / 640.f * 9.1f * 0.404998f;
				}
				else if (g_Direction.m_CKanturu.IsMayaScene())
				{
					Width = (float)GetScreenWidth() / GetWindowsX() * 10.0f * 0.115f;
				}
				else
				{
					Width = (float)GetScreenWidth() / 300.f;
				}
				if (SceneFlag == LOG_IN_SCENE)
				{
				...
				{
					CameraViewFar = 5400.f * 9.1f * 0.404998f;
				}
				else if (gMapManager.WorldActive == WD_39KANTURU_3RD)
				{
					CameraViewFar = 5400.f * 10.0f * 0.115f;
				}
				else
				{
					CameraViewFar = 5400.f;
				}
				...
				{
					//ACuoi Fix Full Map Camera
					CameraViewNear = (CameraViewFar * 0.19f);//0.22
					CameraViewTarget = (CameraViewFar * 0.47f);//0.47
					WidthFar = 1400.f * Width; // 1140.f
					WidthNear = 1400.f * Width; // 540.f
				}
				break;
			case 1:
				Width = (float)GetScreenWidth() / GetWindowsX() + 0.1f;// * 0.1f;
...
bool TestFrustrum2D(float x, float y, float Range)
{
	...
	for (int i = 0; i < 4; j = i, i++)
	{
		...
		if (gCamera.CameraZoom > 0)
			Range -= (15.f * gCamera.CameraZoom);

		if (gCamera.AngleY3D < 0)
			Range += 15.f * gCamera.AngleY3D;
