...
void RenderTipText(int sx, int sy, const char* Text)
{
	...
	RenderColor((float)sx - 2, (float)sy - 3, (float)TextSize.cx / gPosWide.x_fScreenRate_x + 4, (float)1);	// ��
	RenderColor((float)sx - 2, (float)sy - 3, (float)1, (float)TextSize.cy / gPosWide.x_fScreenRate_y + 4);	// ��
...
void SetBooleanPosition(CHAT* c)
{
	...
	if (g_isCharacterBuff((&c->Owner->Object), eBuff_GMEffect) || // GM effect
