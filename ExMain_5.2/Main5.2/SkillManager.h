...
// 主天赋/大师技能编号枚举（Master Skill IDs）
// 每一项对应游戏内的一个大师技能常量，用于标识和映射大师技能功能。
...
// ACuoi 技能类型枚举（ActionSkillType）
// 列举了大量的动作/技能类型常量，用于区分技能逻辑和表现。
...
// 技能需求/属性条件结构体（DemendConditionInfo）
// 保存一个技能或人物对属性的需求；构造函数会初始化为零。
// operator<= 被重载用于检查右侧属性是否满足或超过左侧需求（逐属性比较）。
typedef struct DemendConditionInfo
{
	WORD SkillType;                // 技能类型（ActionSkillType）
	char SkillName[100];           // 技能名（C 风格字符串）
	WORD SkillLevel;               // 技能等级
	DWORD SkillStrength;           // 所需力量
	DWORD SkillDexterity;          // 所需敏捷
	DWORD SkillVitality;           // 所需体力
	DWORD SkillEnergy;             // 所需能量
	DWORD SkillCharisma;           // 所需魅力
	...
	// 比较运算符：如果当前对象的所有属性需求都小于等于 rhs 的属性值，则返回 true
