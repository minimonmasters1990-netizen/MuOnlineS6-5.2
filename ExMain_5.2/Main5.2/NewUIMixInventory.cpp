...
void SEASON3B::CNewUIMixInventory::RenderButtonInit()
{
...
bool CNewUIMixInventory::UpdateMouseEvent()
{
	...
	if (CheckMouseIn(m_pNewInventoryCtrl->GetPos().x, m_pNewInventoryCtrl->GetPos().y,
		m_pNewInventoryCtrl->GetNumberOfColumn() * INVENTORY_SQUARE_WIDTH,
		m_pNewInventoryCtrl->GetNumberOfRow() * INVENTORY_SQUARE_HEIGHT))
	{
		if (SEASON3B::IsPress(VK_RBUTTON))
		{
			...
			if (pItem)
			{
				...
				if (dstIndex != -1)
				{
					...
					STORAGE_TYPE mixType = g_MixRecipeMgr.GetMixInventoryEquipmentIndex();

					SendRequestEquipmentItem(mixType, srcIndex, pItem, STORAGE_TYPE::INVENTORY, dstIndex);
					m_pNewInventoryCtrl->RemoveItem(pItem);
...
{
	RenderImage(IMAGE_MIXINVENTORY_BACK, m_Pos.x + 2, m_Pos.y + 3, 186.f, 426.f);
	...
	g_pRenderText->RenderText(m_Pos.x, m_Pos.y + 10, szText, 160.0f, 0, RT3_SORT_CENTER);
	...
	case SEASON3A::MIXTYPE_TRAINER:
		...
		g_pRenderText->RenderText(fPos_x, fPos_y + fLine_y + 10, szText, 160.0f, 0, RT3_SORT_CENTER);
		break;
	default:
		fLine_y -= 5;
		break;
...
bool CNewUIMixInventory::ProcessMyInvenItemAutoMove()
{
	...
	if (!g_MixRecipeMgr.IsMixSource(pItemObj))
	{
	...
	if (!m_pNewInventoryCtrl->FindEmptySlot(pAttr->Width, pAttr->Height, col, row))
	{
		g_pChatListBox->AddText("", GlobalText[375], SEASON3B::TYPE_ERROR_MESSAGE);
		return false;
	}

	int srcIndex = pMyInvenCtrl->GetIndexByItem(pItemObj);
	int dstIndex = row * m_pNewInventoryCtrl->GetNumberOfColumn() + col;

	STORAGE_TYPE mixType = g_MixRecipeMgr.GetMixInventoryEquipmentIndex();
	SendRequestEquipmentItem(STORAGE_TYPE::INVENTORY, srcIndex, pItemObj, mixType, dstIndex);
