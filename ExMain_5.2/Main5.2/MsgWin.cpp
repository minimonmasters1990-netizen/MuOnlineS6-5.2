		...
		if (m_nMsgCode == MESSAGE_DELETE_CHARACTER_RESIDENT)
			if (g_iChatInputType == 1)
				g_pSinglePasswdInputBox->SetPosition(
					int((m_sprInput.GetXPos() + 10) / gPosWide.x_fScreenRate_x),
					int((m_sprInput.GetYPos() + 8) / gPosWide.x_fScreenRate_y));
		break;
	}
}
...
void CMsgWin::RenderControls()
{
	...
	if (1 == m_nMsgLine)
	{
		nTextPosX = int(m_sprBack.GetXPos() / gPosWide.x_fScreenRate_x);
		if (MWT_NON != m_eType)
			nTextPosY = int((m_sprBack.GetYPos() + 38) / gPosWide.x_fScreenRate_y);
		else
			nTextPosY = int((m_sprBack.GetYPos() + 54) / gPosWide.x_fScreenRate_y);
		g_pRenderText->RenderText(nTextPosX, nTextPosY, m_aszMsg[0],
			m_sprBack.GetWidth() / gPosWide.x_fScreenRate_x, 0, RT3_SORT_CENTER);
	}
	else if (2 == m_nMsgLine)
	{
		nTextPosX = int((m_sprBack.GetXPos() + 25) / gPosWide.x_fScreenRate_x);
		if (MWT_NON != m_eType)
			nTextPosY = int((m_sprBack.GetYPos() + 32) / gPosWide.x_fScreenRate_y);
		else
			nTextPosY = int((m_sprBack.GetYPos() + 44) / gPosWide.x_fScreenRate_y);
		g_pRenderText->RenderText(nTextPosX, nTextPosY, m_aszMsg[0]);

		if (MWT_NON != m_eType)
			nTextPosY = int((m_sprBack.GetYPos() + 51) / gPosWide.x_fScreenRate_y);
		else
			nTextPosY = int((m_sprBack.GetYPos() + 66) / gPosWide.x_fScreenRate_y);
	...
	if (m_nMsgCode == MESSAGE_DELETE_CHARACTER_RESIDENT)
	{
		...
		else if (g_iChatInputType == 0)
		{
			...
			::RenderInputText(
				int((m_sprInput.GetXPos() + 10) / gPosWide.x_fScreenRate_x),
				int((m_sprInput.GetYPos() + 8) / gPosWide.x_fScreenRate_y), 0, 0);
			InputTextWidth = 256;
		}
	}
...
void CMsgWin::ManageOKClick()
{
	...
	case MESSAGE_VERSION:
	case RECEIVE_LOG_IN_FAIL_ERROR:
		::PostMessage(g_hWnd, WM_CLOSE, 0, 0); break;
	case MESSAGE_INPUT_ID:
	case RECEIVE_LOG_IN_FAIL_ID:
	case RECEIVE_LOG_IN_FAIL_ID_CONNECTED:
	case RECEIVE_LOG_IN_FAIL_SERVER_BUSY:
