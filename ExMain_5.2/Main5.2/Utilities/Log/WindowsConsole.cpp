...
void CConsoleWindow::Show(bool bShow)
{
	if(m_hWnd)
	{
		...
		{
			::SetLastError(0);
			//::SetWindowPos(m_hWnd, nullptr, 1913, 0, 100, 0, SWP_NOSIZE | SWP_NOZORDER);
			::SetWindowPos(m_hWnd, nullptr, 0, 0, 100, 0, SWP_NOSIZE | SWP_NOZORDER);
			::ShowWindow(m_hWnd, bShow ? SW_MAXIMIZE : SW_HIDE);
			::UpdateWindow(m_hWnd);
