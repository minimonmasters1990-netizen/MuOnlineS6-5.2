...
{
	m_BtnExit.ChangeButtonInfo((IsToolKit.GetPositionScreen() - 27), 3, 30, 25);
}
...
bool SEASON3B::CNewUIMiniMap::Render()
{
	...
	EnableAlphaTest();
	RenderColor(0, 0, IsToolKit.GetPositionScreen(), 430, 0.85f, 1);
	DisableAlphaBlend();
	EnableAlphaTest();
	...
	float Ch_wid = 12;
	RenderImage(IMAGE_MINIMAP_INTERFACE + 3, (IsToolKit.GetPositionScreen() + 32) / 2, 230, Ch_wid, Ch_wid, 0.f, 0.f, 17.5f / 32.f, 17.5f / 32.f);
	...
	for (i = 0; i < 13; i++)
	{
		...
		RenderBitmapRotate(IMAGE_MINIMAP_INTERFACE + 2, IsToolKit.GetPositionScreen() - (Ui_Hig / 2.f), i * (Ui_wid - 3.f), Ui_wid, Ui_Hig, 90.f, 0.f, 0.f, uvxy, uvxy_Line);
	}

	RenderImage(IMAGE_MINIMAP_INTERFACE + 1, 0, 0, Ui_wid, Ui_wid, 0.f, 0.f, uvxy, uvxy);
	RenderImage(IMAGE_MINIMAP_INTERFACE + 1, (IsToolKit.GetPositionScreen() - Ui_wid), 0, Ui_wid, Ui_wid, uvxy, 0.f, -uvxy, uvxy);
	RenderImage(IMAGE_MINIMAP_INTERFACE + 1, 0, 430 - Ui_wid, Ui_wid, Ui_wid, 0.f, uvxy, uvxy, -uvxy);
	RenderImage(IMAGE_MINIMAP_INTERFACE + 1, (IsToolKit.GetPositionScreen() - Ui_wid), 430 - Ui_wid, Ui_wid, Ui_wid, uvxy, uvxy, -uvxy, -uvxy);
...
bool SEASON3B::CNewUIMiniMap::UpdateMouseEvent()
{
	...
	/*if (CheckMouseIn(0, 0, IsToolKit.GetPositionScreen(), 430))
	{
		return false;
	}*/
