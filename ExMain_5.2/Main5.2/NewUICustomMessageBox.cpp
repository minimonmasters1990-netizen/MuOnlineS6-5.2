...
void SEASON3B::CGemIntegrationMsgBox::RenderTextsButton()
{
	float x, y, width, height;
	...
	IsToolKit.ThisFont(x - 1, y + 10, HEX_COLOR_WHITE, 0x00000000, width, 0, 3, GlobalText[1801]);
	y += BTN_GAP;
	IsToolKit.ThisFont(x - 1, y + 10, HEX_COLOR_WHITE, 0x00000000, width, 0, 3, GlobalText[1800]);
	y += BTN_GAP;
	IsToolKit.ThisFont(x - 1, y + 10, HEX_COLOR_WHITE, 0x00000000, width, 0, 3, GlobalText[1002]);
}
...
void SEASON3B::CGemIntegrationUnityMsgBox::SetButtonInfo()
{
	...
	int	  nBtnIndexMove[COMGEM::eGEMTYPE_END] = { -10, -13, -13, -15, -18, -10, -19, -10, -4, -4 };
	...
	for (int i = 0; i < (int)COMGEM::eGEMTYPE_END; i++)
	{
		...
		cButton.MoveTextPos(nBtnIndexMove[i], 0);
		m_cJewelButton.push_back(cButton);
	}

	int	  nBtnIndex2Move[COMGEM::eCOMTYPE_END] = { 13, 15, 15};
	...
	for (int k = 0; k < (int)COMGEM::eCOMTYPE_END; k++)
	{
		...
		cButton.MoveTextPos(nBtnIndex2Move[k], 0);
...
void CSystemMenuMsgBox::RenderTextOption()
{
	float x, y, width, height;
	float msgboxhalfwidth = (GetSize().cx / 2.f);
	...
	IsToolKit.ThisFont(x - 1, y + 10, HEX_COLOR_WHITE, 0x00000000, width, 0, 3, GlobalText[381]);
	y += 30.f;
	IsToolKit.ThisFont(x - 1, y + 10, HEX_COLOR_WHITE, 0x00000000, width, 0, 3, GlobalText[382]);
	y += 30.f;
	IsToolKit.ThisFont(x - 1, y + 10, HEX_COLOR_WHITE, 0x00000000, width, 0, 3, GlobalText[383]);
	y += 30.f;
	IsToolKit.ThisFont(x - 1, y + 10, HEX_COLOR_WHITE, 0x00000000, width, 0, 3, GlobalText[385]);
	y += 30.f;
	IsToolKit.ThisFont(x - 1, y + 10, HEX_COLOR_WHITE, 0x00000000, width, 0, 3, GlobalText[384]);
}
...
CALLBACK_RESULT SEASON3B::CSystemMenuMsgBox::GameOverBtnDown(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	...
	else
	{
		...
		LogOut = true;
		SendRequestLogOut(0);
		g_ConsoleDebug->Write(MCD_SEND, "0xF1 [SendRequestLogOut] 0");
	}
...
CALLBACK_RESULT SEASON3B::CSystemMenuMsgBox::ChooseServerBtnDown(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	...
	else
	{
		...
		LogOut = true;
		SendRequestLogOut(2);
		g_ConsoleDebug->Write(MCD_SEND, "0xF1 [SendRequestLogOut] 2");
	}
...
CALLBACK_RESULT SEASON3B::CSystemMenuMsgBox::ChooseCharacterBtnDown(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	...
	else
	{
		...
		LogOut = true;
	    SendRequestLogOut(1);
		g_ConsoleDebug->Write(MCD_SEND, "0xF1 [SendRequestLogOut] 1");
	}
...
void SEASON3B::CChaosMixMenuMsgBox::RenderTextsButton()
{
	float x, y, width, height;
	...
	IsToolKit.ThisFont(x - 1, y + 10, HEX_COLOR_WHITE, 0x00000000, width, 0, 3, GlobalText[735]);
	y = GetPos().y + 155;
	IsToolKit.ThisFont(x - 1, y + 10, HEX_COLOR_WHITE, 0x00000000, width, 0, 3, GlobalText[736]);
	y = GetPos().y + 225;
	IsToolKit.ThisFont(x - 1, y + 10, HEX_COLOR_WHITE, 0x00000000, width, 0, 3, GlobalText[2193]);

	y = GetPos().y + GetSize().cy - (MSGBOX_BTN_EMPTY_HEIGHT + MSGBOX_BTN_BOTTOM_BLANK);
	IsToolKit.ThisFont(x + 32, y + 10, HEX_COLOR_WHITE, 0x00000000, 62, 0, 3, GlobalText[1002]);
}
	...
	else
	{
		...
		if(pPickedItem)
		{
			...
			if(pPickedItem->GetOwnerInventory() == g_pMyInventory->GetInventoryCtrl())
			{
				...
				SendRequestEquipmentItem(STORAGE_TYPE::INVENTORY, MAX_EQUIPMENT + iSourceIndex, pItemObj, STORAGE_TYPE::MYSHOP, MAX_MY_INVENTORY_EX_INDEX+iTargetIndex);
			}
			else if(pPickedItem->GetOwnerInventory() == NULL)
			{
				...
				SendRequestEquipmentItem(STORAGE_TYPE::INVENTORY, iSourceIndex, pItemObj, STORAGE_TYPE::MYSHOP, MAX_MY_INVENTORY_EX_INDEX + iTargetIndex);
			}
			else if(pPickedItem->GetOwnerInventory() == g_pMyShopInventory->GetInventoryCtrl())
			{
				...
				SendRequestEquipmentItem(STORAGE_TYPE::MYSHOP, MAX_EQUIPMENT_INDEX + iSourceIndex, pItemObj, STORAGE_TYPE::MYSHOP, MAX_MY_INVENTORY_EX_INDEX + iTargetIndex);
			}
...
void SEASON3B::CTrainerRecoverMsgBox::SetButtonInfo()
{
	...
	m_BtnRecoverDarkHorse.MoveTextPos(-20, -2);
	...
	m_BtnRecoverDarkSpirit.MoveTextPos(-20, -2);
...
void SEASON3B::CSeedMasterMenuMsgBox::RenderTextsButton()
{
	float x, y, width, height;
	...
	IsToolKit.ThisFont(x - 1, y + 10, HEX_COLOR_WHITE, 0x00000000, width, 0, 3, GlobalText[2664]);
	y = GetPos().y + 120;
	IsToolKit.ThisFont(x - 1, y + 10, HEX_COLOR_WHITE, 0x00000000, width, 0, 3, GlobalText[2665]);
	y = GetPos().y + GetSize().cy - (MSGBOX_BTN_EMPTY_HEIGHT + MSGBOX_BTN_BOTTOM_BLANK);
	IsToolKit.ThisFont(x - 1, y + 10, HEX_COLOR_WHITE, 0x00000000, width, 0, 3, GlobalText[1002]);
}
...
void SEASON3B::CSeedInvestigatorMenuMsgBox::RenderTextsButton()
{
	float x, y, width, height;
	...
	IsToolKit.ThisFont(x - 1, y + 10, HEX_COLOR_WHITE, 0x00000000, width, 0, 3, GlobalText[2669]);
	y = GetPos().y + 120;
	IsToolKit.ThisFont(x - 1, y + 10, HEX_COLOR_WHITE, 0x00000000, width, 0, 3, GlobalText[2670]);
	y = GetPos().y + GetSize().cy - (MSGBOX_BTN_EMPTY_HEIGHT + MSGBOX_BTN_BOTTOM_BLANK);
	IsToolKit.ThisFont(x - 1, y + 10, HEX_COLOR_WHITE, 0x00000000, width, 0, 3, GlobalText[1002]);
}
...
void SEASON3B::CGuild_ToPerson_Position::RenderTextButtons()
{
	float x, y, width, height;
	...
	IsToolKit.ThisFont(x, y + 10, HEX_COLOR_WHITE, 0x00000000, width, 0, 3, GlobalText[1311]);
	y += 27;
	IsToolKit.ThisFont(x, y + 10, HEX_COLOR_WHITE, 0x00000000, width, 0, 3, GlobalText[1312]);

	width = MSGBOX_BTN_EMPTY_SMALL_WIDTH;
	y += 70;
	IsToolKit.ThisFont(x - 10, y + 10, HEX_COLOR_WHITE, 0x00000000, width, 0, 3, GlobalText[228]);
	x += 64;
	IsToolKit.ThisFont(x - 10, y + 10, HEX_COLOR_WHITE, 0x00000000, width, 0, 3, GlobalText[1002]);
}
...
bool SEASON3B::CMasterSkillPointMsgBoxLayout::SetLayout()
{
	...
	pMsgBox->SetInputBoxOption(UIOPTION_NUMBERONLY | UIOPTION_PAINTBACK);
	pMsgBox->AddMsg(GlobalText[3198]);
...
int g_nSelectedStsButton = 0;

bool CStsConfirmOkCancelLayout::SetLayout()
{
	auto* pMsgBox = GetMsgBox();
	if (!pMsgBox)
		return false;
	...
	char szText[64];
	sprintf(szText, GlobalText[3199], g_nSelectedStsButton + 1);
	pMsgBox->AddMsg(szText);

	pMsgBox->AddCallbackFunc(CStsConfirmOkCancelLayout::OkBtnDown, MSGBOX_EVENT_USER_COMMON_OK);
	pMsgBox->AddCallbackFunc(CStsConfirmOkCancelLayout::CancelBtnDown, MSGBOX_EVENT_USER_COMMON_CANCEL);

	return true;
}

CALLBACK_RESULT CStsConfirmOkCancelLayout::OkBtnDown(CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	char szCmd[32];
	sprintf(szCmd, "/sts %d", g_nSelectedStsButton + 1);
	SendChat(szCmd);
	PlayBuffer(SOUND_CLICK01);
	g_MessageBox->SendEvent(pOwner, MSGBOX_EVENT_DESTROY);
	return CALLBACK_BREAK;
...
CALLBACK_RESULT CStsConfirmOkCancelLayout::CancelBtnDown(CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	PlayBuffer(SOUND_CLICK01);
	g_MessageBox->SendEvent(pOwner, MSGBOX_EVENT_DESTROY);
	return CALLBACK_BREAK;
