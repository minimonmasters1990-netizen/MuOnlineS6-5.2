...
void RenderCharacter(CHARACTER* c, OBJECT* o, int Select)
{
    ...
    int MonsterGolden = gCustomMonsterGlow.IsGolden(c->MonsterIndex);
    ...
    case MODEL_PLAYER:
        ...
        if (gMapManager.InChaosCastle() == false)
        {
            ...
            {
                #if M_CUSTOM_ITEM
                gCustomItem.GetItemColor(c->BodyPart[5].Type, o->Alpha, 0.5f, Light);
                #else
...
void RenderCharactersClient()
{
    for (int i = 0; i < MAX_CHARACTERS_CLIENT; ++i)
    {
        ...
        //ACuoi Run Effect -> For Hon Hoan
        g_pNewUIHonHoan->RenderHonHoanEffect(c, o);
...
CHARACTER* CreateMonster(int Type, int PositionX, int PositionY, int Key)
{
    ...
    if (BossClass)
    {
        ...
        CreateMonsterCustomSkin(c, BODYPART_HELM, MODEL_HELM + BossClass->SlotElement[0], 7);
        CreateMonsterCustomSkin(c, BODYPART_ARMOR, MODEL_ARMOR + BossClass->SlotElement[0], 7);
        CreateMonsterCustomSkin(c, BODYPART_PANTS, MODEL_PANTS + BossClass->SlotElement[0], 7);
        CreateMonsterCustomSkin(c, BODYPART_GLOVES, MODEL_GLOVES + BossClass->SlotElement[0], 7);
        CreateMonsterCustomSkin(c, BODYPART_BOOTS, MODEL_BOOTS + BossClass->SlotElement[0], 7);

        c->Wing.Type        = MODEL_WING + BossClass->SlotElement[3];
        c->Wing.Level       = 7;
        c->Wing.Option1     = 1;
        c->Wing.ExtOption   = 1;

        c->Weapon[EQUIPMENT_WEAPON_RIGHT].Type      = MODEL_ITEM + BossClass->SlotElement[1];
        ...
        c->Weapon[EQUIPMENT_WEAPON_LEFT].Type       = MODEL_ITEM + BossClass->SlotElement[2];
        c->Weapon[EQUIPMENT_WEAPON_LEFT].Level      = 9;
        c->Weapon[EQUIPMENT_WEAPON_LEFT].Option1    = 1;
        c->Weapon[EQUIPMENT_WEAPON_LEFT].ExtOption  = 1;

        if (BossClass->SlotElement[4] != -1)
        {
            c->Skill = BossClass->SlotElement[4];
        }
    ...
    case 257:
        c = CreateCharacter(Key, MODEL_PLAYER, PositionX, PositionY);
        gCustomModelNPC.MakeNpcModel(c, 257);
        strcpy(c->ID, "페이아");
        o = &c->Object;
    ...
    case 249:
        c = CreateCharacter(Key, MODEL_PLAYER, PositionX, PositionY);
        gCustomModelNPC.MakeNpcModel(c, 249);
        SetCharacterScale(c);
        break;
    case 250:
        OpenNpc(MODEL_MERCHANT_MAN);
        ...
        if (gMapManager.InChaosCastle() == false)
        {
            ...
            if (w->Type != -1)
            {
                ...
                if (g_pOption->IsFpsWingMotion())
                {
                    ...
                    {
                        w->PlaySpeed = 0.25f * 0.33f;// FPS_ANIMATION_FACTOR; //ACuoi Animation Wing
                    }
                }
                else
                {
