...
void CSPetDarkSpirit::MovePet(void)
{
    ...
    else if (o->AI == PET_ATTACK || o->AI == PET_ESCAPE)
    {
        ...
        if (o->AI == PET_ATTACK)
        {
            ...
            TargetPosition[2] += 200.f; // SkillFix
...
void    CSPetDarkSpirit::RenderCmdType(void)
{
    ...
    if ((Hero->Helper.Type >= MODEL_HELPER + 0 && Hero->Helper.Type <= MODEL_HELPER + 4) || Hero->Helper.Type == MODEL_HELPER + 37 || gCustomPet.GetInfoPetType(Hero->Helper.Type - MODEL_ITEM) == 5 || gCustomPet.GetInfoPetType(Hero->Helper.Type - MODEL_ITEM) == 6)
    {
        PartyWidth += 60.f;
    }
