...
void CUIGuildInfo::RenderGuildInfoTab()
{
	...
	g_pRenderText->RenderText(ptOrigin.x, ptOrigin.y, szTemp, 140 * gPosWide.x_fScreenRate_x, 0, RT3_SORT_LEFT);
	ptOrigin.y += 13;
	...
	ptOrigin.x -= 5;
	ptOrigin.y += 33;
	g_pRenderText->RenderText(ptOrigin.x + 5, ptOrigin.y, GlobalText[1323], 136 * gPosWide.x_fScreenRate_x, 0, RT3_SORT_LEFT);
...
void CUIGuildInfo::Render()
{
	...
	if (Hero->GuildStatus != G_NONE)
	{
		...
		sprintf(szTemp, GlobalText[3176], GuildMark[Hero->GuildMarkIndex].GuildName, IsToolKit.QN(GuildTotalScore));
		g_pRenderText->RenderText(ptOrigin.x, ptOrigin.y, szTemp, 120, 0, RT3_SORT_CENTER);
	}
