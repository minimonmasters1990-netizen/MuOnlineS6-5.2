...
void RenderTipTextList(const int sx, const int sy, int TextNum, int Tab, int iSort, int iRenderPoint, BOOL bUseBG, BOOL PostItem)
{
	...
	float fScreenHeight = (float)WindowHeight / gPosWide.x_fScreenRate_y;
	float fMinBottom = 40.f;
	if (fsy + fHeight > fScreenHeight - fMinBottom)
	{
		fsy = sy - fHeight - 20.f;
		if (fsy < 0) fsy = 5.f;
	}
...
std::string GetItemDisplayName(ITEM* pItem)
{
	...
	std::string strDisplayName(pAttr->Name);
