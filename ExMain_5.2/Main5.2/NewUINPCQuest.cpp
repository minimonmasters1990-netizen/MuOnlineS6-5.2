...
void CNewUINPCQuest::RenderBackImage()
{
	RenderImage(IMAGE_NPCQUEST_BACK,
		m_Pos.x + 2, m_Pos.y + 3, 186.f, 426.f);
...
void CNewUINPCQuest::RenderText()
{
	...
	if ((Hero->Class == CLASS_DARK_LORD || Hero->Class == CLASS_MAGIC
		|| Hero->Class == CLASS_RAGEFIGHTER)
		&& bCheckNPC)
		g_pRenderText->RenderText(m_Pos.x, m_Pos.y + 10,
	...
	if ((Hero->Class != CLASS_DARK_LORD && Hero->Class != CLASS_MAGIC
		&& Hero->Class != CLASS_RAGEFIGHTER)
		|| !bCheckNPC)
		g_pRenderText->RenderText(m_Pos.x, m_Pos.y + 33,
