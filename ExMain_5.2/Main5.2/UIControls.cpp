...
int CutStr(const char* pszSrcText, char * pTextOut, const int iTargetPixelWidth, const int iMaxOutLine, const int iOutStrLength, const int iFirstLineTab /* = 0 */)
{
	...
	int iScreenRatePixelWidth = iTargetPixelWidth*gPosWide.x_fScreenRate_x-5;
	SIZE iSize;
...
void CUIButton::Render()
{
	...
	if (GetState() == UISTATE_DISABLE)
	{
		...
		if (m_pszCaption != NULL)
		{
			...
			g_pRenderText->RenderText(m_iPos_x + (m_iWidth - (float)TextSize.cx / gPosWide.x_fScreenRate_x + 0.5f) / 2, m_iPos_y + 1 + (m_iHeight - (float)TextSize.cy / gPosWide.x_fScreenRate_y + 0.5f) / 2, m_pszCaption);
		}
	...
	if (m_pszCaption != NULL)
	{
		...
		if (m_bMouseState == TRUE)
		{
			g_pRenderText->RenderText(m_iPos_x + 1 + (m_iWidth - (float)TextSize.cx / gPosWide.x_fScreenRate_x + 0.5f) / 2, m_iPos_y + 2 + (m_iHeight - (float)TextSize.cy / gPosWide.x_fScreenRate_y + 0.5f) / 2, m_pszCaption);
		}
		else
		{
			g_pRenderText->RenderText(m_iPos_x + (m_iWidth - (float)TextSize.cx / gPosWide.x_fScreenRate_x + 0.5f) / 2, m_iPos_y + 1 + (m_iHeight - (float)TextSize.cy / gPosWide.x_fScreenRate_y + 0.5f) / 2, m_pszCaption);
...
void CUISimpleChatListBox::AddTextToRenderList(const char * pszID, const char * pszText, int iType, int iColor)
{
	...
	if (strlen(pszID) + strlen(pszText) >= 20)
	{
		...
		int iLine = CutText3(pszText, Text1[0], m_iWidth - 30, 10, MAX_TEXT_LENGTH + 1, (nameSize.cx + 5) / gPosWide.x_fScreenRate_x);
...
{
	POINT SPK;
	SPK.x = m_iPos_x + gPosWide.x_GetAddPos + 105;
	EnableAlphaTest();
	SetLineColor(7, 0.4f);
	RenderColor(SPK.x - 1, m_iPos_y - m_iHeight - 1, m_iWidth + 1, m_iHeight + 2);
	EndRenderColor();

	if (GetState() != UISTATE_SCROLL)
		ComputeScrollBar();

	g_pGuardWindow->RenderScrollBarFrame(SPK.x + m_iWidth - 8, m_fScrollBarRange_top, m_fScrollBarRange_bottom - m_fScrollBarRange_top);
	g_pGuardWindow->RenderScrollBar(SPK.x + m_iWidth - 12, m_fScrollBarPos_y, (GetState() == UISTATE_SCROLL && MouseLButtonPush));
}
...
{
	EnableAlphaTest();
	int iPos_S = m_iPos_x + gPosWide.x_GetAddPos + 105;
	if (SLGetSelectLineNum() == m_iCurrentRenderEndLine + iLineNumber + 1)
	{
		if (g_dwKeyFocusUIID == GetUIID()) glColor4f(1.0f, 1.0f, 1.0f, 0.7f);
		else glColor4f(1.0f, 1.0f, 1.0f, 0.3f);
		RenderColor(iPos_S, GetRenderLinePos_y(iLineNumber) - 3, m_iWidth - m_fScrollBarWidth + 1, 13);
		glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
		glEnable(GL_TEXTURE_2D);
	...
	g_pRenderText->SetBgColor(0);
	int iPos_x = m_iPos_x + gPosWide.x_GetAddPos + 110;
	int iPos_y = GetRenderLinePos_y(iLineNumber);
...
{
	int iPos_x = m_iPos_x + gPosWide.x_GetAddPos + 113;
	if (::CheckMouseIn(iPos_x, GetRenderLinePos_y(iLineNumber) - 3, m_iWidth - m_fScrollBarWidth + 1, 13))
	{
		if (MouseLButtonPush)
		{
			SLSetSelectLine(m_iCurrentRenderEndLine + iLineNumber + 1);
...
bool CUIRenderTextOriginal::Create(HDC hDC)
{
	...
	DIB_INFO->bmiHeader.biWidth = WindowWidth;		//. 640
	DIB_INFO->bmiHeader.biHeight = -(WindowHeight);		//. 480
	...
	m_dwTextColor = HEX_COLOR_WHITE;		//. Default Text Color
...
void CUIRenderTextOriginal::WriteText(int iOffset, int iWidth, int iHeight)
{
	...
	SIZE FontDCSize = { WindowWidth, WindowHeight };
...
void CUIRenderTextOriginal::UploadText(int sx,int sy,int Width,int Height)
{
	...
	if(Width > 0 && Height > 0 && sx+Width > 0 && sy+Height > 0)
	{
		...
		RenderBitmap(BITMAP_FONT, (float)sx - Blend, (float)sy - Blend, Width, Height, TextureU, TextureV, TextureUWidth, TextureVHeight, false, false);
		RenderBitmap(BITMAP_FONT, (float)sx - Blend, (float)sy + Blend, Width, Height, TextureU, TextureV, TextureUWidth, TextureVHeight, false, false);
		RenderBitmap(BITMAP_FONT, (float)sx + Blend, (float)sy - Blend, Width, Height, TextureU, TextureV, TextureUWidth, TextureVHeight, false, false);
		RenderBitmap(BITMAP_FONT, (float)sx + Blend, (float)sy + Blend, Width, Height, TextureU, TextureV, TextureUWidth, TextureVHeight, false, false);
										...
										int iSort /* = RT3_SORT_LEFT */, OUT SIZE* lpTextSize /* = NULL */)
{
	...
	MU_POINTF RealBoxPos = { (float)iPos_x*gPosWide.x_fScreenRate_x, (float)iPos_y*gPosWide.x_fScreenRate_y };
	SIZEF RealBoxSize = { (float)iBoxWidth*gPosWide.x_fScreenRate_x, (float)iBoxHeight*gPosWide.x_fScreenRate_y };
	...
	if(m_dwBackColor != 0)
	{
		...
		RenderColor(RealBoxPos.x/gPosWide.x_fScreenRate_x, RealBoxPos.y/gPosWide.x_fScreenRate_y, 
			RealBoxSize.cx/gPosWide.x_fScreenRate_x, RealBoxSize.cy/gPosWide.x_fScreenRate_y);
		EndRenderColor();
	}
	...
	if(lpTextSize)
	{
		lpTextSize->cx = RealRenderingSize.cx/gPosWide.x_fScreenRate_x;
		lpTextSize->cy = RealRenderingSize.cy/gPosWide.x_fScreenRate_y;
	}
}
...
void CUITextInputBox::SetIMEPosition()
{
	...
	int iSetPos_x = m_iPos_x * gPosWide.x_fScreenRate_x + WindowWidth;
	int iSetPos_y = m_iPos_y * gPosWide.x_fScreenRate_y + WindowHeight;
...
void CUITextInputBox::SetSize(int iWidth, int iHeight)
{
	...
	memset( DIB_INFO, 0x00, sizeof(BITMAPINFOHEADER) );
	DIB_INFO->bmiHeader.biSize        = sizeof(BITMAPINFOHEADER);
	DIB_INFO->bmiHeader.biWidth       = iWidth*gPosWide.x_fScreenRate_x;
	DIB_INFO->bmiHeader.biHeight      = -(iHeight*gPosWide.x_fScreenRate_y);
	DIB_INFO->bmiHeader.biPlanes      = 1;
	DIB_INFO->bmiHeader.biBitCount    = 24;
	DIB_INFO->bmiHeader.biCompression = BI_RGB;
	...
	if (m_hEditWnd != NULL)
	{
		SetWindowPos(m_hEditWnd, 0, 0, 0, iWidth * gPosWide.x_fScreenRate_x, iHeight * gPosWide.x_fScreenRate_y, SWP_NOMOVE | SWP_NOZORDER);
...
void CUITextInputBox::Init(HWND hWnd, int iWidth, int iHeight, int iMaxLength, BOOL bIsPassword)
{
	...
	m_iRealWindowPos_x = m_iPos_x * gPosWide.x_fScreenRate_x + WindowWidth;
	m_iRealWindowPos_y = m_iPos_y * gPosWide.x_fScreenRate_y + WindowHeight;

	m_hEditWnd = CreateWindowW(L"edit", NULL,WS_CHILD | WS_VISIBLE | dwOptionFlag,m_iRealWindowPos_x, m_iRealWindowPos_y, iWidth*gPosWide.x_fScreenRate_x, iHeight*gPosWide.x_fScreenRate_y,m_hParentWnd, (HMENU)ID_UICEDIT,g_hInst, NULL);
...
void CUITextInputBox::WriteText(int iOffset, int iWidth, int iHeight)
{
	...
	SIZE RealBoxSize = { (long)m_iWidth*gPosWide.x_fScreenRate_x, (long)m_iHeight*gPosWide.x_fScreenRate_y };
	const int LIMIT_WIDTH = 256, LIMIT_HEIGHT = 32;
...
void CUITextInputBox::Render()
{
	...
	POINT RealWndPos = { m_iPos_x*gPosWide.x_fScreenRate_x, m_iPos_y*gPosWide.x_fScreenRate_y };
	SIZE RealWndSize = { m_iWidth*gPosWide.x_fScreenRate_x, m_iHeight*gPosWide.x_fScreenRate_y };
...
void CUISlideHelp::Render(BOOL bForceFadeOut)
{
	...
	if (m_iFontHeight == 0)
	{
		...
		m_iFontHeight = TextSize.cy /= gPosWide.x_fScreenRate_y;
...
void CUISlideHelp::SlideMove()
{
	if (m_iCutSize == 0)
	{
		...
		m_iCutSize /= gPosWide.x_fScreenRate_x;
	}
...
void CUISlideHelp::SetScrollSpeed(float fSpeed)
{
	...
	m_fMaxMoveSpeed = fSpeed * gPosWide.x_fScreenRate_x;
}
...
BOOL CUINewGuildMemberListBox::RenderDataLine(int iLineNumber)
{
	...
	if (iCharacterLevel == 0)
	{
		...
		RenderColor(m_iPos_x, GetRenderLinePos_y(iLineNumber), m_iWidth - m_fScrollBarWidth + 1, 13);
	...
	char Text[MAX_TEXT_LENGTH + 1] = {0};
	sprintf(Text,"%s",m_TextListIter->m_szID);
	g_pRenderText->RenderText(iPos_x, iPos_y + 3, Text);

	if (iCharacterLevel == 0) g_pRenderText->RenderText(iPos_x + 45, iPos_y + 3, GlobalText[1300], 70, 0, RT3_SORT_CENTER);
	else if (iCharacterLevel == 1) g_pRenderText->RenderText(iPos_x + 45, iPos_y + 3, GlobalText[1301], 70, 0, RT3_SORT_CENTER);
	else if (iCharacterLevel == 2) g_pRenderText->RenderText(iPos_x + 45, iPos_y + 3, GlobalText[1302], 70, 0, RT3_SORT_CENTER);
	...
	{	
		...
		sprintf(Text,"%d", m_TextListIter->m_Server + 1);
		g_pRenderText->RenderText(m_iPos_x + m_iWidth - 30, iPos_y + 3, Text);
	}
...
BOOL CUIUnionGuildListBox::RenderDataLine(int iLineNumber)
{
	...
	if (SLGetSelectLineNum() == m_iCurrentRenderEndLine + iLineNumber + 1)
	{
		...
		RenderColor(m_iPos_x, GetRenderLinePos_y(iLineNumber) - 3, m_iWidth - m_fScrollBarWidth + 1, 13);
		glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
		//ACuoi Color NameGuild Lien Minh
		//g_pRenderText->SetTextColor(0, 0, 0, 255);
	}
	else
	{
...
{
	EnableAlphaTest();
	int HoverPos_x = m_iPos_x + gPosWide.x_GetAddPos;
	if(SLGetSelectLineNum() == m_iCurrentRenderEndLine + iLineNumber + 1)
	{
		::glColor4f(0.15f, 0.3f, 0.4f, 0.5f);
		RenderColor(HoverPos_x, GetRenderLinePos_y(iLineNumber) - 3, m_iWidth - m_fScrollBarWidth + 4, 13);
		glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
		glEnable(GL_TEXTURE_2D);
	}
	...
	int iPos_x = m_iPos_x + gPosWide.x_GetAddPos;
	int iPos_y = GetRenderLinePos_y(iLineNumber);
...
{
	if (::CheckMouseIn(m_iPos_x + gPosWide.x_GetAddPos, GetRenderLinePos_y(iLineNumber) - 3, m_iWidth - m_fScrollBarWidth + 1, 13))
	{
		if (MouseLButtonPush)	// MouseLButtonDBClick
		{
			MouseLButtonPush = false;
...
void CUIExtraItemListBox::AddText(const char* pszPattern)
{
	...
	strncpy(text.m_szPattern, pszPattern, MAX_ITEM_NAME + 1);
...
BOOL CUIExtraItemListBox::RenderDataLine(int iLineNumber)
{
	...
	if (SLGetSelectLineNum() == m_iCurrentRenderEndLine + iLineNumber + 1)
	{
		...
		RenderColor(m_iPos_x, GetRenderLinePos_y(iLineNumber) - 3, m_iWidth - m_fScrollBarWidth + 1, 13);
		glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
		g_pRenderText->SetTextColor(255, 127, 39, 255);
	}
	else
	{
		g_pRenderText->SetTextColor(230, 220, 200, 255);
	...
	char Text[MAX_TEXT_LENGTH + 1] = { 0 };
	snprintf(Text, MAX_TEXT_LENGTH + 1, "%s", m_TextListIter->m_szPattern);
	g_pRenderText->RenderText(iPos_x, iPos_y, Text);
...
{
	if (pszPattern == nullptr || strlen(pszPattern) == 0) return;
	for (m_TextListIter = m_TextList.begin(); m_TextListIter != m_TextList.end(); ++m_TextListIter)
	{
		if (strncmp(m_TextListIter->m_szPattern, pszPattern, MAX_ITEM_NAME) == 0)
			break;
	}
	if (m_TextListIter == m_TextList.end()) return;
