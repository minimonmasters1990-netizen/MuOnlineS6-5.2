...
bool SEASON3B::CNewUIQuickCommandWindow::UpdateMouseEvent()
{
	...
	for (int i = 0; i < 5 + MAX_OPTION_CMD; ++i)
	{
	...
	if (m_iSelectedIndex > -1 && SEASON3B::IsRelease(VK_LBUTTON))
	{
		switch (m_iSelectedIndex)
		{
			case 0:
			{
				...
				strncpy(gViewCharItem.CharViewName, m_strID, sizeof(gViewCharItem.CharViewName) - 1);
				gViewCharItem.CharViewName[sizeof(gViewCharItem.CharViewName) - 1] = '\0';
				gViewCharItem.SendRequestViewItem(pCha->Key);
				CloseQuickCommand();
				return false;
			}
			break;
			...
			{
				CHARACTER* pCha = &CharactersClient[m_iSelectedCharacterIndex];
				g_pCommandWindow->CommandGuild(pCha);
				CloseQuickCommand();
	...
	if (CheckMouseIn(m_Pos.x, m_Pos.y + 30, 112, 140) == false)
	{
		m_iSelectedIndex = -1;
	...
	if (CheckMouseIn(m_Pos.x, m_Pos.y, 112, 180))
	{
		return false;
	}
...
void SEASON3B::CNewUIQuickCommandWindow::RenderFrame()
{
	...
	RenderImage(IMAGE_QUICKCOMMAND_BACK, x, y, width, height + (MAX_OPTION_CMD * 17));

	y = m_Pos.y;
	RenderImage(IMAGE_QUICKCOMMAND_FRAME_UP, m_Pos.x, y, 112.f, 45.f);
	y += 45.f;

	for (int i = 0; i < 4 + MAX_OPTION_CMD; ++i)
	{
...
void SEASON3B::CNewUIQuickCommandWindow::RenderContents()
{
	...
	int iGlobalText[] = { 3660, 943, 1124, 944, 948, 3659, 949 };
	for (int i = 0; i < 5 + MAX_OPTION_CMD; ++i)
	{
...
void SEASON3B::CNewUIQuickCommandWindow::RenderArrow()
{
	...
	float x, y;
	x = m_Pos.x + 16.f;
	y = m_Pos.y + 43.f + (m_iSelectedIndex * 20);
