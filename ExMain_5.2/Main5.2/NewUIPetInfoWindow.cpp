...
bool CNewUIPetInfoWindow::Render()
{
	...
	g_pRenderText->RenderText(m_Pos.x, m_Pos.y + 10, GlobalText[1217], 190, 0, RT3_SORT_CENTER);
	m_BtnTab.Render();
...
bool CNewUIPetInfoWindow::RenderDarkHorseInfo(PET_INFO* pPetInfo)
{
	...
	unicode::_sprintf(szText, GlobalText[358], IsToolKit.QN(pPetInfo->m_wLife), IsToolKit.QN(255));
	...
	unicode::_sprintf(szText, GlobalText[203], IsToolKit.QN(m_aiDamage[0]), IsToolKit.QN(m_aiDamage[1]), IsToolKit.QN(pPetInfo->m_wAttackSuccess));
	g_pRenderText->RenderText(iPosX + 10, iPosY + 72, szText, 0, 0, RT3_SORT_CENTER);
	unicode::_sprintf(szText, GlobalText[64], IsToolKit.QN(pPetInfo->m_wAttackSpeed));
