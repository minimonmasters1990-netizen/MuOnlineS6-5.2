...
void SEASON3B::CNewUIItemEnduranceInfo::SetPos(int x, int y)
{
	...
	m_ItemDurUIStartPos.x = gPosWide.x_GetAddPos * 2 + GetScreenWidth() - ITEM_DUR_WIDTH - 2;
...
{
	m_ItemDurUIStartPos.x = gPosWide.x_GetAddPos * 2 + x - ITEM_DUR_WIDTH - 2;
	m_ItemDurUIStartPos.y = 140;
}
...
void SEASON3B::CNewUIItemEnduranceInfo::RenderTooltip(int iX, int iY, const ITEM* pItem, const DWORD& dwTextColor)
{
	...
	int iTooltipWidth = (/*TextSize.cx*/7 * iTextlen) / gPosWide.x_fScreenRate_x;
	...
	g_pRenderText->RenderText(iX + gPosWide.x_GetAddPos * 2, iY, szText, 0, 0, RT3_WRITE_CENTER);

}

bool SEASON3B::CNewUIItemEnduranceInfo::RenderEquipedHelperLife(int iX, int iY)
{
	int type = gCustomPet.GetInfoPetType(Hero->Helper.Type - MODEL_ITEM);
	if (Hero->Helper.Type >= MODEL_HELPER && Hero->Helper.Type <= MODEL_HELPER + 4
		|| Hero->Helper.Type == MODEL_HELPER + 64
		|| Hero->Helper.Type == MODEL_HELPER + 65
		|| Hero->Helper.Type == MODEL_HELPER + 67
		...
		|| type != -1)
	{
		...
		switch (Hero->Helper.Type)
		{
			...
			default:
			{
				int IndexCustompet = Hero->Helper.Type - MODEL_ITEM;

				if (gCustomPet.CheckCustomPetByItem(IndexCustompet)) 
				{
					if (IndexCustompet >= 0 && IndexCustompet < MAX_ITEM)
					{
						unicode::_sprintf(szText, ItemAttribute[IndexCustompet].Name);
					}
					else
					{
						unicode::_sprintf(szText, "Unknown Custom Pet");
					}
				}
				else
				{
					unicode::_sprintf(szText, "Unknown Pet");
				}
			}
			break;
		}
