...
{
	m_Pos.x = x + gPosWide.x_GetAddPos;// = STANDARD_POS_X;
	m_Pos.y = STANDARD_POS_Y - (20 * (MENU_MAX_INDEX_MENU - 4));
}
...
bool SEASON3B::CNewUIWindowMenu::UpdateMouseEvent()
{
	...
	for (int i = 0; i < MENU_MAX_INDEX_MENU; ++i)
	{
	...
	if (m_iSelectedIndex > -1 && SEASON3B::IsRelease(VK_LBUTTON))
	{
		switch (m_iSelectedIndex)
		{
			...
			case 6:
			{
				if (g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_EVENCOIN))
				{
					g_pNewUISystem->Hide(SEASON3B::INTERFACE_EVENCOIN);
				}
				else
				{
					g_pNewUISystem->Show(SEASON3B::INTERFACE_EVENCOIN);
				}
				g_pNewUISystem->Hide(SEASON3B::INTERFACE_WINDOW_MENU);
				return false;
			}
	...
	if (CheckMouseIn(m_Pos.x, m_Pos.y + 20, 112, 95 + 20 * (MENU_MAX_INDEX_MENU - 5)) == false)
	{
		m_iSelectedIndex = -1;
	}
	else
...
void SEASON3B::CNewUIWindowMenu::RenderFrame()
{
	...
	y = m_Pos.y; 
	width = 112.f;
	height = 105 + (MENU_MAX_INDEX_MENU - 4) * 20;
	...
	for (int i = 0; i < MENU_MAX_INDEX_MENU - 1; ++i)
	{
...
void SEASON3B::CNewUIWindowMenu::RenderTexts()
{
	...
	int iTextNumber[] = { 1741, 1742, 364, 1743, 3055, 3103, 3807, 1097, 1987 };
