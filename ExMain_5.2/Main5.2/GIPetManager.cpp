...
namespace giPetManager
{
	...
	static  PET_INFO        g_PetInfo[PET_TYPE_END];
	...
	bool RenderPetCmdInfo(int sx, int sy, int Type)
	{
		...
		if (gCharacterManager.GetBaseClass(Hero->Class) == CLASS_DARK_LORD)
		{
			...
			int Height = (int)(((TextNum - SkipNum) * TextSize.cy + SkipNum * TextSize.cy / 2) / gPosWide.x_fScreenRate_y);
			sy -= Height;
	...
	bool RenderPetItemInfo(int sx, int sy, ITEM* pItem, int iInvenType)
	{
		...
		sprintf(TextList[TextNum], "\n"); TextNum++; SkipNum++;
		sprintf(TextList[TextNum], "\n"); TextNum++; SkipNum++;
		sprintf(TextList[TextNum], GlobalText[201], IsToolKit.QN(pPetInfo->m_dwExp1), IsToolKit.QN(pPetInfo->m_dwExp2)); TextNum++; SkipNum++;
		sprintf(TextList[TextNum], "%s : %d", GlobalText[368], pPetInfo->m_wLevel); TextNum++; SkipNum++;

		if (pItem->Type == ITEM_HELPER + 5)
		{
			sprintf(TextList[TextNum], GlobalText[203], IsToolKit.QN(pPetInfo->m_wDamageMin), IsToolKit.QN(pPetInfo->m_wDamageMax), IsToolKit.QN(pPetInfo->m_wAttackSuccess)); TextNum++; SkipNum++;
			sprintf(TextList[TextNum], GlobalText[64], IsToolKit.QN(pPetInfo->m_wAttackSpeed)); TextNum++; SkipNum++;
		}
