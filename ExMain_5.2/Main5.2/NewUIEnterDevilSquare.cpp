...
void CNewUIEnterDevilSquare::EnterInsideEventMap()
{
	...
	RenderImage(SEASON3B::CNewUIBloodCastle::IMAGE_BLOODCASTLE_TIME_WINDOW, m_Pos.x + 60, m_Pos.y + 345, 124.f, 81.f);

	if (gIsTimerDs.TimeRemain > 0)
	{
		int hour = gIsTimerDs.TimeRemain / 3600;
		int minute = (gIsTimerDs.TimeRemain % 3600) / 60;
		int second = gIsTimerDs.TimeRemain % 60;
		int milli = (int)(1000.0f * (1.0f - (WorldTime - (int)WorldTime)));
		if (milli >= 1000) milli = 999;

		IsToolKit.ThisFontBig(m_Pos.x + 72, m_Pos.y + 356, 0xE66400FF, 0, 100, 0, 3, "%02d:%02d:%02d.%03d", hour, minute, second, milli);
	}
	else
	{
		IsToolKit.ThisFontBig(m_Pos.x + 72, m_Pos.y + 356,0xE66400FF, 0, 100, 0, 3, "00:00:00.000");
	}

	IsToolKit.ThisFont(m_Pos.x + 72, m_Pos.y + 383, 0xE66400FF, 0, 100, 0, 3, GlobalText[1842]);
	IsToolKit.ThisFont(m_Pos.x + 72, m_Pos.y + 395, 0xE66400FF, 0, 100, 0, 3, GlobalText[1843], gIsTimerDs.KillCur, gIsTimerDs.KillMax);
}
...
void CNewUIEnterDevilSquare::OpenningProcess()
{
	...
	for (int i = 0; i < MAX_ENTER_GRADE - 1; i++)
	{
		...
		m_BtnEnter[i].SetFont(g_hFont);
		m_BtnEnter[i].ChangeText(sztext);
	}

	unicode::_sprintf(sztext, GlobalText[1778], 7);
	m_BtnEnter[MAX_ENTER_GRADE - 1].SetFont(g_hFont);
