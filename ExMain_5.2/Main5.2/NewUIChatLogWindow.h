...
namespace SEASON3B 
{
	...
	private:
		...
		typedef std::map<int, ITEM*> type_postItem;
		...
		SIZE	m_WndSize;
		int		m_nShowingLines;
		int		m_nShowingLinesMsg; //Separate Chat
			
		MESSAGE_TYPE		m_CurrentRenderMsgType;
		bool	m_bShowChatLog;
		bool	m_bShowSystemMesage;	//Separate chat
		int		m_iCurrentRenderEndLine;
		int		m_iCurrentRenderEndLineMsg;//Separate chat
		...
		bool RenderMessages();
		bool RenderMessagesNew();//Separate Chat
		bool RenderFrame();
	...
	public:
		...
		void HideChatLog();
		//
		void ShowSystemChat();//Separate Chat
		void HideSystemChat();//Separate Chat

		int GetCurrentRenderEndLine() const;
		int GetCurrentRenderEndLineMsg() const;//Separate Chat
