...
int SEASON3B::CNewUIInventoryCtrl::FindEmptySlot(IN int cx, IN int cy, bool MyInventoryExt)
{
...
//ACuoi Fix Bung do EXC NPC Thu Mua
...
void SEASON3B::CNewUIInventoryCtrl::RenderNumberOfItem()
{
	...
	for (; li != m_vecItem.end(); ++li)
	{
		...
		if (pItem->Type >= ITEM_POTION && pItem->Type <= ITEM_POTION + 8 && pItem->Durability > 1)
		{
			...
			SEASON3B::RenderNumber(x + width - 8, y + 12, pItem->Durability, 0.77f);
		}
		...
		else if (COMGEM::isCompiledGem(pItem))
		{
			...
			glColor3f(1.f, 0.9f, 0.7f);
			SEASON3B::RenderNumber(x + width - 8, y + 12, (Level + 1) * COMGEM::FIRST, 0.77f);
		}
	}
	glColor3f(1.f, 1.f, 1.f);
	DisableAlphaBlend();
