...
void SEASON3B::CNewUICursedTempleEnter::RenderText()
{
	...
	if( CheckEnterLevel(enterlevel) )
	{
		...
		DrawText(Text, m_Pos.x+3, m_Pos.y+42, HEX_COLOR_WHITE, 0x00000000, RT3_SORT_CENTER, CURSEDTEMPLE_ENTER_WINDOW_WIDTH-10, false );
		...
		for( int i = 0; i < EnterLevelCount+1; ++i )
		{
			...
			if( enterlevel == i+1 )
			{
				...
				DrawText( Text, m_Pos.x+3, m_Pos.y+67+(i*15), HEX_COLOR_WHITE, 0xff0000ff, RT3_SORT_CENTER, CURSEDTEMPLE_ENTER_WINDOW_WIDTH-10, false );
				EnableAlphaTest();
			}
			else
			{
				sprintf( Text, "%s %s", Text, GlobalText[2413] );
				DrawText( Text, m_Pos.x+3, m_Pos.y+67+(i*15), HEX_COLOR_WHITE, 0x00000000, RT3_SORT_CENTER, CURSEDTEMPLE_ENTER_WINDOW_WIDTH-10, false );
			}
		}
