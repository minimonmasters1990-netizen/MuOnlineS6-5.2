...
void CNewUITrade::SetPos(int x, int y)
{
	...
	if (m_pYourInvenCtrl)
	{
		m_pYourInvenCtrl->SetPos(x + 16, y + 68);
	}

	if (m_pMyInvenCtrl)
	{
		m_pMyInvenCtrl->SetPos(x + 16, y + 274);
	}
...
bool CNewUITrade::UpdateMouseEvent()
{
	...
	int invenWidth = m_pMyInvenCtrl->GetNumberOfColumn() * INVENTORY_SQUARE_WIDTH;
	int invenHeight = m_pMyInvenCtrl->GetNumberOfRow() * INVENTORY_SQUARE_HEIGHT;

	if (CheckMouseIn(m_pMyInvenCtrl->GetPos().x, m_pMyInvenCtrl->GetPos().y,
		m_pMyInvenCtrl->GetNumberOfColumn() * INVENTORY_SQUARE_WIDTH,
		m_pMyInvenCtrl->GetNumberOfRow() * INVENTORY_SQUARE_HEIGHT))
	{
		if (SEASON3B::IsPress(VK_RBUTTON))
		{
			g_pMyInventory->ResetMouseRButton();

			ITEM* pItem = m_pMyInvenCtrl->FindItemAtPt(MouseX, MouseY);
			if (pItem)
			{
				int emptySlot = g_pMyInventory->FindEmptySlot(pItem);
				if (emptySlot != -1)
				{
					int srcIndex = m_pMyInvenCtrl->GetIndexByItem(pItem);
					SendRequestItemToMyInven(pItem, srcIndex, emptySlot);

					m_pMyInvenCtrl->RemoveItem(pItem);
...
void CNewUITrade::RenderText()
{
	...
	g_pRenderText->RenderText(m_Pos.x + 32, m_Pos.y + 46, m_szYourID);
	...
	g_pRenderText->RenderText(m_Pos.x + 120, m_Pos.y + 46, szTemp, 50, 0, 4);
...
bool CNewUITrade::ProcessMyInvenItemAutoMove()
{
	if (g_pPickedItem && g_pPickedItem->GetItem())
		return false;

	const auto pMyInvenCtrl = g_pMyInventory->GetInventoryCtrl();
	if (ITEM* pItemObj = pMyInvenCtrl->FindItemAtPt(MouseX, MouseY))
	{
		...
		int col = 0, row = 0;
		if (!m_pMyInvenCtrl->FindEmptySlot(pAttr->Width, pAttr->Height, col, row))
		{
		...
		const int dstIndex = row * m_pMyInvenCtrl->GetNumberOfColumn() + col;

		SendRequestItemToTrade(pItemObj, srcIndex, dstIndex);
