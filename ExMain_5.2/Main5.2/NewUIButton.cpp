...
m_NameColor(HEX_COLOR_WHITE), m_NameBackColor(0x00000000),
m_CurImgColor(HEX_COLOR_WHITE), m_TooltipTextColor(HEX_COLOR_WHITE), m_IsTopPos(false),
...
m_CurImgIndex(0), m_CurImgState(0), m_ImgWidth(0), m_ImgHeight(0), m_CurImgColor(HEX_COLOR_WHITE)
{
	Initialize();
}
...
void CNewUIRadioButton::ChangeFrame(BUTTON_STATE eventstate)
{
	...
	if (m_Name.size() != 0)
	{
		if (GetBTState() == BUTTON_STATE_UP)
			...
			ChangeTextColor(HEX_COLOR_WHITE);
	}
}
...
bool CNewUIRadioButton::Render()
{
	...
	if (m_Name.size() != 0)
	{
		...
		int x = m_Pos.x + ((m_Size.x / 2) - (Fontsize.cx / 2)) - 5;
		int y = m_Pos.y + ((m_Size.y / 2) - (Fontsize.cy / 2)) - 1;
