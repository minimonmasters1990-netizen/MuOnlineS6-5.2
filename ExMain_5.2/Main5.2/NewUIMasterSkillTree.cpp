...
void SEASON3B::CNewUIMasterSkillTree::LoadImages()
{
	...
	LoadBitmap("ACuoi\\MasterBoxSS5.tga", IMAGE_MASTER_INTERFACE + 4, GL_LINEAR);
	...
	LoadBitmap("Interface\\ACuoi\\RenderClassName.tga", IMAGE_MASTER_INTERFACE + 14, GL_LINEAR);
}
...
void SEASON3B::CNewUIMasterSkillTree::RenderText()
{
	...
	EnableAlphaTest();
	IsToolKit.ThisFont(GetPos.x + 502, 22, 0xFFC800FF, 0, 50, 0, 3, "%d / %d", Master_Level_Data.nMLevel, Master_Level_Data.nMLevelUpMPoint);
	
	//IsToolKit.ThisFontBig(GetPos.x + 120, 19, 0xFFC800FF, 0, 150, 0, 1, GlobalText[this->ClassNameTextIndex]);
	
	sprintf(Buffer, GlobalText[this->CategoryTextIndex], this->CategoryPoint[0]);
	IsToolKit.ThisFont(GetPos.x + 282, 22, 0x32C832FF, 0, 50, 0, 3, Buffer);
	
	sprintf(Buffer, GlobalText[this->CategoryTextIndex + 1], this->CategoryPoint[1]);
	IsToolKit.ThisFont(GetPos.x + 348, 22, 0x3C3CFFFF, 0, 50, 0, 3, Buffer);
	
	sprintf(Buffer, GlobalText[this->CategoryTextIndex + 2], this->CategoryPoint[2]);
	IsToolKit.ThisFont(GetPos.x + 416.0, 22, 0xFF0000FF, 0, 50, 0, 3, Buffer);

	//IsToolKit.ThisFont(100, 100, 0xFF0000FF, 0, 50, 0, 3, "%d", Hero->Class);
