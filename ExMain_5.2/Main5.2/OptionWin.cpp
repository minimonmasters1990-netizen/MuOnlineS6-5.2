...
void COptionWin::RenderControls()
{
	...
	g_pRenderText->RenderText(int(m_winBack.GetXPos() / gPosWide.x_fScreenRate_x),
		int((m_winBack.GetYPos() + 10) / gPosWide.x_fScreenRate_y),
		GlobalText[385], m_winBack.GetWidth() / gPosWide.x_fScreenRate_x, 0, RT3_SORT_CENTER);	

	const char* apszBtnText[3] =
	{ GlobalText[386], GlobalText[387], GlobalText[919] };
	for (int i = 0; i <= OW_BTN_SLIDE_HELP; ++i)
	{
		g_pRenderText->RenderText(int((m_aBtn[i].GetXPos() + 24) / gPosWide.x_fScreenRate_x),
			int((m_aBtn[i].GetYPos() + 4) / gPosWide.x_fScreenRate_y), apszBtnText[i]);
	}
	...
	for (int i = 0; i < OW_SLD_MAX; ++i)	
	{
		nTextPosY = int((m_aSlider[i].GetYPos() - 18) / gPosWide.x_fScreenRate_y);
		g_pRenderText->RenderText(int(m_aSlider[i].GetXPos() / gPosWide.x_fScreenRate_x),nTextPosY, apszSldText[i]);

		::_itoa(anVal[i], szVal, 10);
		g_pRenderText->RenderText(int((m_aSlider[i].GetXPos() + 85)/ gPosWide.x_fScreenRate_x),nTextPosY, szVal);
