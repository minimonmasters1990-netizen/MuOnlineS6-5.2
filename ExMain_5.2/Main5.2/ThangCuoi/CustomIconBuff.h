...
class BuffIcon
{
	...
	void RenderBasicTooltip(float x, float y, const std::vector<std::string>& lines);
	void LoadEng(RenderTooltipBuff* info);
	void Recv(PMSG_SEND_BUFF_ICON* DataRecv);
	void DeleteAllBuff();
	void GC_BuffInfo(PMSG_SEND_BUFF_ICON_EX* aRecv);
	void RenderTipBuff(int BuffIndex, int IndexTime, float RenderX, float RenderY);
	void FormatBuffTooltip(int BuffIndex, const char* fmt, ...);
