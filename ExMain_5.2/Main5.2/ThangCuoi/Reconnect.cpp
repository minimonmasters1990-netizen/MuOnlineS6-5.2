...
void CReconnect::ReconnecGameServerLoad()
{
	if (CreateSocket(s_Data.GameServerAddress, s_Data.GameServerPort) != 0)
	{
		...
		ReconnectSetInfo(RECONNECT_STATUS_RECONNECT, RECONNECT_PROGRESS_CONNECTED, 10000, gProtect->m_MainInfo.ReconnectTime * 1000);
	}
}
...
void CReconnect::ReconnectOnCloseSocket()
{
	...
	{
		ReconnectSetInfo(RECONNECT_STATUS_RECONNECT, RECONNECT_PROGRESS_NONE, 30000, gProtect->m_MainInfo.ReconnectTime * 1000);
	...
	if (s_Data.ReconnectProgress == RECONNECT_PROGRESS_CONNECTED)
	{
		if (s_Data.ReconnectAuthSend != 0)
		{
			if (result == 1)
			{
				...
				ReconnectSetInfo(RECONNECT_STATUS_RECONNECT, RECONNECT_PROGRESS_JOINED, 30000, gProtect->m_MainInfo.ReconnectTime * 1000);
			}
			else
			{
				if (result == 3)
...
void CReconnect::ReconnectOnCharacterList()
{
	if (s_Data.ReconnectProgress == RECONNECT_PROGRESS_JOINED)
	{
		...
		ReconnectSetInfo(RECONNECT_STATUS_RECONNECT, RECONNECT_PROGRESS_CHAR_LIST, 30000, gProtect->m_MainInfo.ReconnectTime * 1000);
	}
}
