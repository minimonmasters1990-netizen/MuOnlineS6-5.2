...
void CNewUIStorageInventoryExt::RenderBackImage() const
{
	...
	RenderImage(IMAGE_STORAGE_BACK, x + 2, y + 3, STORAGE_WIDTH - 4, STORAGE_HEIGHT - 3);
...
void CNewUIStorageInventoryExt::RenderText() const
{
	...
	g_pRenderText->RenderText(m_Pos.x, m_Pos.y+10, GlobalText[3339], STORAGE_WIDTH, 0, RT3_SORT_CENTER);
}
...
bool CNewUIStorageInventoryExt::ProcessMyInvenItemAutoMove()
{
	...
	if (const auto pItemObj = pMyInvenCtrl->FindItemAtPt(MouseX, MouseY))
	{
		...
		if (emptySlotIndex != -1)
		{
			...
			SendRequestEquipmentItem(STORAGE_TYPE::INVENTORY, nSrcIndex, pItemObj, STORAGE_TYPE::VAULT, emptySlotIndex);
