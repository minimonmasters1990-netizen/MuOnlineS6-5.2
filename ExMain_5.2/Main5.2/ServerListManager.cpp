...
{
	FILE* fp = ::fopen("Data\\ACuoi\\Config\\ServerList.bmd", "rb");

	if (fp == NULL)
	{
		char szMessage[256];
		::sprintf(szMessage, "Data\\ACuoi\\Config\\ServerList.bmd file not found.\r\n");
...
void CServerListManager::InsertServer(CServerGroup* pServerGroup, int iConnectIndex, int iServerPercent)
{
	...
	{
		case 0: sprintf(pServerInfo->m_bName, gProtect->m_MainInfo.ServerName[0], pServerGroup->m_szName, pServerInfo->m_iIndex, GlobalText[iTextIndex]);		break;
		case 1: sprintf(pServerInfo->m_bName, gProtect->m_MainInfo.ServerName[1], pServerGroup->m_szName, pServerInfo->m_iIndex, GlobalText[iTextIndex]);		break;
		case 2: sprintf(pServerInfo->m_bName, gProtect->m_MainInfo.ServerName[2], pServerGroup->m_szName, pServerInfo->m_iIndex, GlobalText[iTextIndex]);		break;
		case 3: sprintf(pServerInfo->m_bName, gProtect->m_MainInfo.ServerName[3], pServerGroup->m_szName, pServerInfo->m_iIndex, GlobalText[iTextIndex]);		break;
	}
