...
bool CNewUIExchangeLuckyCoin::Create(CNewUIManager* pNewUIMng, int x, int y)
{
	...
	m_BtnExchange[0].SetFont(g_hFont);
	...
	m_BtnExchange[1].SetFont(g_hFont);
	...
	m_BtnExchange[2].SetFont(g_hFont);
...
{
	RenderImage(IMAGE_EXCHANGE_LUCKYCOIN_WINDOW_BACK, m_Pos.x + 2, m_Pos.y + 3, 186.f, 426.f);
...
void CNewUIExchangeLuckyCoin::UnLockExchangeBtn()
{
	for (int i = 0; i < 3; i++)
	{
		...
		m_BtnExchange[i].ChangeTextColor(HEX_COLOR_WHITE);
	}
}
