...
// cái trên t nhặt về dùng thử.. như cặt mà quên tháo ra toafn queen vl 
// con trỏ gốc
#define SendRequestMagic( p_Type, p_Key)\
{\
...
__forceinline bool SendRequestEquipmentItem(STORAGE_TYPE iSrcType, int iSrcIndex, ITEM* pItem, STORAGE_TYPE iDstType, int iDstIndex)
{
	...
	if (pItem->option_380)
	{
		//splitType = ((BYTE)(pItem->Type>>5)&240) | 0x08; //ACuoi Fix Crash use item lien tuc
		splitType = ((BYTE)((pItem->Type >> 5) & 0xF0)) | 0x08;
	}
	else
...
//ACuoi DataSend Packet
__forceinline void DataSend(BYTE* lpMsg, DWORD size) // OK
{
...
//ACuoi PacketSender - Code by Me
#define SendRequestDataSend(Type, Data) \
{										\
    CStreamPacketEngine spe;			\
    spe.Init(0xC1, Type);				\
    spe << (BYTE)Data;					\
...
#define SendRequestDataSendType(Type, Data, Access) \
{													\
    CStreamPacketEngine spe;						\
    spe.Init(0xC1, Type);							\
    spe << (BYTE)Data;								\
...
#define SendRequestDataSendTypeMax(Type, Data, Access, Count)	\
{																\
    CStreamPacketEngine spe;									\
    spe.Init(0xC1, Type);										\
    spe << (BYTE)Data;											\
...
#define SendPetCommandRequest(petType, commandType, targetIndex) \
{ \
    CStreamPacketEngine spe; \
    spe.Init(0xC1, 0xA1); \
    spe << (BYTE)(petType); \
    spe << (BYTE)(commandType); \
    spe << (WORD)(targetIndex); \
    spe.Send(); \
}

#define SendRepairItemRequest(pos, mode) \
{ \
    CStreamPacketEngine spe; \
    spe.Init(0xC1, 0xC3); \
    spe << (BYTE)0x34; \
...
#define SendPickupItemRequest(itemIndex) \
{ \
    CStreamPacketEngine spe; \
    spe.Init(0xC1, 0x22); \
    spe << (BYTE)((itemIndex) >> 8); \
    spe << (BYTE)((itemIndex) & 0xFF); \
    spe.Send(); \
}

#define SendRequestResetInfo() \
{ \
	CStreamPacketEngine spe; \
	spe.Init(0xC1, 0xF3);\
	spe << (BYTE)(0xE9);\
...
#define SendRequestResetInfoAuto(type1, type2) \
{ \
	CStreamPacketEngine spe; \
	spe.Init(0xC1, 0xF3);\
	spe << (BYTE)(0xE7);\
