...
void Projection(vec3_t Position, int* sx, int* sy)
{
	...
	*sx = *sx * GetWindowsX() / (int)WindowWidth;
...
{
	return x * gPosWide.x_fScreenRate_x;
}

float ConvertY(float y)
{
	return y * gPosWide.x_fScreenRate_y;
}
...
void RenderColorTron(float x, float y, float Radius, float Alpha, int Flag)
{
	DisableTexture();
	...
	for (int i = 0; i <= NumSegments; ++i)
	{
		...
		float px = x + cosf(angle) * ConvertX(Radius);
		float py = y + sinf(angle) * ConvertY(Radius);
		glVertex2f(px, py);
	}
...
void RenderBitmapRotated2D(int Texture, float x, float y, float Width, float Height,
	...
	float Alpha, float Rotation)
{
	...
	for (int i = 0; i < 4; ++i)
	{
		float rx = p[i][0] * cosA - p[i][1] * sinA + cx;
		float ry = p[i][0] * sinA + p[i][1] * cosA + cy;
...
void RenderPointRotate(int Texture, float ix, float iy, float iWidth, float iHeight, float x, float y, float Width, float Height, float Rotate, float Rotate_Loc, float uWidth, float vHeight, int Num)
{
	...
	if (Num > -1)
	{
		...
		dx = dx * (float)(640.f / WindowWidth) + gPosWide.x_GetAddPos;
