...
void CMsgBoxIGSCommon::RenderFrame()
{
	...
	float Px = GetPos().x + 1;
	float Py = GetPos().y + 5;
	float Pw = m_iMsgBoxWidth - 2;
	float Ph = m_iMsgBoxHeight - 5;

	RenderImage(IMAGE_IGS_BACK, Px, Py, Pw, Ph);
...
void CMsgBoxIGSCommon::RenderTexts()
{
	...
	g_pRenderText->RenderText(GetPos().x + (IMAGE_IGS_FRAME_WIDTH / 2) - (IMAGE_IGS_BTN_WIDTH / 2), (GetPos().y + m_iMsgBoxHeight) - (IMAGE_IGS_BTN_HEIGHT + IGS_BTN_POS_Y) + 8, GlobalText[228], IMAGE_IGS_BTN_WIDTH, 0, RT3_SORT_CENTER);
}
