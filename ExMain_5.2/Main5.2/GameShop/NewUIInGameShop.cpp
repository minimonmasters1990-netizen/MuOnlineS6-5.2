...
void CNewUIInGameShop::RenderTexts()//
{
	...
	g_pRenderText->RenderText(m_Pos.x + TEXT_IGS_CASH_POS_X, m_Pos.y + TEXT_IGS_CASH_POS_Y - 1, szText, TEXT_IGS_CASH_WIDTH, 0, RT3_SORT_LEFT);
	g_pRenderText->RenderText(m_Pos.x + TEXT_IGS_CASH_POS_X + 50, m_Pos.y + TEXT_IGS_CASH_POS_Y - 1, szValue, TEXT_IGS_CASH_WIDTH - 56, 0, RT3_SORT_RIGHT);

	//Prepaid 
	ConvertGold(g_InGameShopSystem->GetCashPrepaid(), szValue);
	sprintf(szText, GlobalText[3145], "");
	g_pRenderText->RenderText(m_Pos.x + TEXT_IGS_CASH_POS_X, m_Pos.y + TEXT_IGS_MILEAGE_POS_Y - 1, szText, TEXT_IGS_CASH_WIDTH, 0, RT3_SORT_LEFT);
	g_pRenderText->RenderText(m_Pos.x + TEXT_IGS_CASH_POS_X + 50, m_Pos.y + TEXT_IGS_MILEAGE_POS_Y - 1, szValue, TEXT_IGS_CASH_WIDTH - 56, 0, RT3_SORT_RIGHT);

	ConvertGold(g_InGameShopSystem->GetTotalMileage(), szValue);
	sprintf(szText, GlobalText[2884], "");
	g_pRenderText->RenderText(m_Pos.x + TEXT_IGS_CASH_POS_X, m_Pos.y + TEXT_IGS_POINT_POS_Y - 1, szText, TEXT_IGS_CASH_WIDTH, 0, RT3_SORT_LEFT);
	g_pRenderText->RenderText(m_Pos.x + TEXT_IGS_CASH_POS_X + 50, m_Pos.y + TEXT_IGS_POINT_POS_Y - 1, szValue, TEXT_IGS_CASH_WIDTH - 56, 0, RT3_SORT_RIGHT);

	ConvertGold(pGetCoin.ThisCoin[3], szValue);
	sprintf(szText, GlobalText[3813], "");
	g_pRenderText->RenderText(m_Pos.x + TEXT_IGS_CASH_POS_X, m_Pos.y + TEXT_IGS_POINT_POS_Y + 14, szText, TEXT_IGS_CASH_WIDTH, 0, RT3_SORT_LEFT);
	g_pRenderText->RenderText(m_Pos.x + TEXT_IGS_CASH_POS_X + 50, m_Pos.y + TEXT_IGS_POINT_POS_Y + 14, szValue, TEXT_IGS_CASH_WIDTH - 56, 0, RT3_SORT_RIGHT);

	ConvertGold(pGetCoin.ThisCoin[4], szValue);
	sprintf(szText, GlobalText[3814], "");
	g_pRenderText->RenderText(m_Pos.x + TEXT_IGS_CASH_POS_X, m_Pos.y + TEXT_IGS_POINT_POS_Y + 28, szText, TEXT_IGS_CASH_WIDTH, 0, RT3_SORT_LEFT);
	g_pRenderText->RenderText(m_Pos.x + TEXT_IGS_CASH_POS_X + 50, m_Pos.y + TEXT_IGS_POINT_POS_Y + 28, szValue, TEXT_IGS_CASH_WIDTH - 56, 0, RT3_SORT_RIGHT);
...
#ifdef ENABLE_EDIT
	if (m_UseButtonSave.UpdateMouseEvent())
	{
		std::string folder = "Data/ScriptCase";
		std::string path = folder + "/ItemPosition.txt";
		...
		int xxx = IsToolKit.VisiblePos(2).x;
		int yyy = IsToolKit.VisiblePos(2).y;
		int a = IsToolKit.VisiblePos(1).x;
		int b = IsToolKit.VisiblePos(1).y;
		float c = IsToolKit.VisiblePos(1).h;
		int posType = IsToolKit.VisiblePos(5).x;

		std::ostringstream oss;
		oss << std::fixed << std::setprecision(1) << c;
		std::string c_str = oss.str();
		...
		std::string caseStr = "case GET_ITEM(" + std::to_string(xxx) + ", " + std::to_string(yyy) + "):";
		std::string newLine = caseStr + " x = sx " + signX + " " + std::to_string(a) +
			"; y = sy " + signY + " " + std::to_string(b) +
			"; s = Scale - " + c_str + "; break;";
		...
		{
			if (line.find(caseStr) != std::string::npos)
			{
				buffer << newLine << "\n";
				found = true;
			}
...
void CNewUIInGameShop::SetBtnInfo()
{
	...
	for (int i = 0; i < INGAMESHOP_DISPLAY_ITEMLIST_SIZE; i++)
	{
		...
		m_ViewDetailButton[i].MoveTextPos(-1, -2);
...
#ifdef ENABLE_EDIT
	m_UseButtonSave.ChangeButtonImgState(true, IMAGE_IGS_VIEWDETAIL_BTN, true, false, true);
	m_UseButtonSave.ChangeButtonInfo(m_Pos.x + 50, m_Pos.y + IMAGE_IGS_USE_BTN_POS_Y, IMAGE_IGS_VIEWDETAIL_BTN_WIDTH, IMAGE_IGS_VIEWDETAIL_BTN_HEIGHT);
	m_UseButtonSave.MoveTextPos(0, -1);
	m_UseButtonSave.ChangeText("SAVE");
#endif
...
bool CNewUIInGameShop::IsInGameShopOpen()
{
	...
	if (!Hero->SafeZone && gMapManager.WorldActive != WD_0LORENCIA && gMapManager.WorldActive != WD_3NORIA && gMapManager.WorldActive != WD_2DEVIAS && gMapManager.WorldActive != WD_51HOME_6TH_CHAR)
	{
		CMsgBoxIGSCommon* pMsgBox = NULL;
		CreateMessageBox(MSGBOX_LAYOUT_CLASS(CMsgBoxIGSCommonLayout), &pMsgBox);
...
void CNewUIInGameShop::InitCategoryBtn()
{
	...
	m_CategoryButton.SetFont(g_hFont);
...
void CNewUIInGameShop::LoadImages()
{
	...
	LoadBitmap("Interface\\InGameShop\\Ingame_ShopbackCt.jpg", IMAGE_IGS_BACK_CT, GL_LINEAR);
	LoadBitmap("Interface\\InGameShop\\Ingame_Bt01.tga", IMAGE_IGS_CATEGORY_BTN, GL_LINEAR);
	LoadBitmap("Interface\\InGameShop\\Ingame_Bt02.tga", IMAGE_IGS_CATEGORY_BTN2, GL_LINEAR);
...
void CNewUIInGameShop::UnloadImages()
{
	...
	DeleteBitmap(IMAGE_IGS_BACK_CT);
	DeleteBitmap(IMAGE_IGS_CATEGORY_BTN);
	DeleteBitmap(IMAGE_IGS_CATEGORY_BTN2);
