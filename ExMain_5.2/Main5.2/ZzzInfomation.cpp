...
bool IsRequireEquipItem(ITEM* pItem)
{
	...
	DWORD wStrength = CharacterAttribute->ViewStrength + CharacterAttribute->AddStrength;
	...
	DWORD wCharisma = CharacterAttribute->ViewLeadership + CharacterAttribute->AddCharisma;
...
char* getMonsterName(int type)
{
	...
	return R"("")";
	//return gCustomMonster.GetNameMonterCustom(type);
}
...
void CHARACTER_MACHINE::CalculateMagicDamage()
{
	...
	{
		int iAdd_MP = (CharacterAttribute->ViewEnergy / 9) * 0.2f;
...
void CHARACTER_MACHINE::CalculateAttackSpeed()
{
	...
	BYTE* speed = pGetSPeedClass.ThisValue;
...
{
	InitAddValue(); //fix test ngựa lỗi cột thể lực
	CalculateBasicState();
