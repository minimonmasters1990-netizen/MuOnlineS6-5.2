...
const char* CMapManager::GetMapName(int iMap)
{
	...
	if (iMap == WD_41CHANGEUP3RD_1ST)
	{
		return (GlobalText[1678]);
	}
	if (iMap == WD_42CHANGEUP3RD_2ND)
	{
	...
	if (iMap == WD_82NEW_CTC)
	{
		return (GlobalText[1450]);
	}
	if (iMap == WD_83NEW_NARS)
	{
		return (GlobalText[1451]);
	}
	if (iMap == WD_84NEW_ACHERON)
	{
		return (GlobalText[1452]);
	}
	if (iMap == WD_85NEW_DEBENTER)
	{
		return (GlobalText[1453]);
	}
