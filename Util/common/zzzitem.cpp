...
#include "..\Source\ItemPrice.h"
#include "..\Source\ItemRank.h"
#include "..\Source\ElfCheat.h"
...
void CItem::Value()
{
	...
	if ( p->BuyMoney != 0 && !g_ItemPrice.CheckItem(this->m_Type, this->m_Level))
	{
		this->m_SellMoney = p->BuyMoney;
		this->m_BuyMoney = p->BuyMoney;
		this->m_SellMoney /= 3;
		this->m_CurrencyBuy = ZEN;
		this->m_CurrencySell = ZEN;		
	...
	__int64 sellTemp, buyTemp;
	CURR_NAME currBuy, currSell;
	bool isCustomPrice = g_ItemPrice.CheckItem(this->m_Type, this->m_Level, &buyTemp, &sellTemp, &currBuy, &currSell);

	if (isCustomPrice)
	{
		this->m_BuyMoney		= buyTemp;
		this->m_CurrencyBuy		= currBuy;
		this->m_SellMoney		= sellTemp;
		this->m_CurrencySell	= currSell;
	}
	else
	{
	this->m_CurrencyBuy		= ZEN;		
...
BOOL CItem::IsExtLifeAdd()
{
	...
	{
		if (g_ItemRank.IsRankItem(this->m_Type))
		{
			Value = g_ItemRank.GetValue(this->m_Type, 0);
			return Value;
		}
		return Value;
	}
...
BOOL CItem::IsExtManaAdd()
{
	...
	if ( (this->m_NewOption & 0x10) == 16 )
	{
		if (g_ItemRank.IsRankItem(this->m_Type))
		{
			Value = g_ItemRank.GetValue(this->m_Type, 1);
			return Value;
		}
		return Value;
	}
...
BOOL CItem::IsExtDamageMinus()
{
	...
	if ( (this->m_NewOption & 8) == 8 )
	{
		if (g_ItemRank.IsRankItem(this->m_Type))
		{
			Value = g_ItemRank.GetValue(this->m_Type, 2);
			return Value;
		}
		return Value;
	}
...
BOOL CItem::IsExtDamageReflect()
{
	...
	if ( (this->m_NewOption & 4) == 4 )
	{
		if (g_ItemRank.IsRankItem(this->m_Type))
		{
			Value = g_ItemRank.GetValue(this->m_Type, 3);
			return Value;
		}
		return Value;
	}
...
BOOL CItem::IsExtDefenseSuccessfull()
{
	...
	if ( (this->m_NewOption & 2) == 2 )
	{
		if (g_ItemRank.IsRankItem(this->m_Type))
		{
			Value = g_ItemRank.GetValue(this->m_Type, 4);
			return Value;
		}
		return Value;
	}
...
BOOL CItem::IsExtMonsterMoney()
{
	...
	if ( (this->m_NewOption & 1) == 1 )
	{
		if (g_ItemRank.IsRankItem(this->m_Type))
		{
			Value = g_ItemRank.GetValue(this->m_Type, 5);
			return Value;
		}
		return Value;
	}
...
int CItem::IsFenrirIncLastAttackDamage() {
    ...
    if ( ( this->m_NewOption & 1 ) == 1 ) {
        return ReadConfig.Fenrir_IncDamagePercent;
    }
    else if ( ( this->m_NewOption & 4 ) == 4 ) {
        if ( ReadConfig.Fenrir_FixGold != 0 ) {
            return ReadConfig.Fenrir_IncDamagePercent;
        }
    }
...
int CItem::IsWhiteTiger() //New pet
{
	...
	if ((this->m_NewOption & 8) == 8) {
		return ElfCheat.Fenrir_DecDamagePercent;
	}
	else if ((this->m_NewOption & 4) == 4) {
		if (ElfCheat.Fenrir_FixGold != 0) {
			return ElfCheat.Fenrir_DecDamagePercent;
		}
	}
