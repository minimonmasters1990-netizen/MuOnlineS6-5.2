...
void CServerManager::AddServer(int index, const char* ip, SOCKET socket) // OK
{
	...
	LogAdd(LOG_BLUE,"Nhận diện IP %s cục bộ đang chạy.",this->m_IpAddr);
}

void CServerManager::DelServer() // OK
{
	LogAdd(LOG_RED,"Đã xóa IP %s cục bộ này.",this->m_IpAddr);
...
void CServerManager::SetServerInfo(const char* name, WORD port, WORD code) // OK
{
	...
	LogAdd(LOG_BLACK,"%s Port %d đã được sát nhập thành công!",this->m_ServerName,this->m_ServerPort);
}
