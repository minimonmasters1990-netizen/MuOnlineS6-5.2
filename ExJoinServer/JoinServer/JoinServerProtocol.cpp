...
{
	int ChieuDaiPassAccount = gRegJoin.ChieuDaiKyTu;
	...
	std::transform(lpMsg->account, lpMsg->account + 10, lpMsg->account, ::tolower);

	if(MD5Encryption == 0)
	{
		#if DK_TK_NUMBER
		for (int i = 0; i < strlen(lpMsg->account); i++)
		{
			if (!isdigit(lpMsg->account[i]))
			{
				pMsg.result = 12;
				gSocketManager.DataSend(index, (BYTE*)&pMsg, pMsg.header.size);
				return;
		...
		if(gQueryManager.ExecQuery("SELECT memb__pwd FROM MEMB_INFO WHERE memb___id='%s' COLLATE Latin1_General_BIN",lpMsg->account) == 0 || gQueryManager.Fetch() == SQL_NO_DATA)
		{
			...
			if(gRegJoin.RegNewAccount == 1)
			{
				gLog.Output(LOG_ACCOUNT, "Lỗi: Tài khoản không tồn tại (Tài khoản: %s, Mật khẩu: %s, IP: %s)", lpMsg->account, lpMsg->password, lpMsg->IpAddress);

				if (strlen(lpMsg->account) >= ChieuDaiPassAccount && strlen(lpMsg->password) >= ChieuDaiPassAccount)
				{
					if (gQueryManager.ExecQuery("SELECT memb__pwd FROM MEMB_INFO WHERE memb___id='%s'", lpMsg->account) == 0 || gQueryManager.Fetch() == SQL_NO_DATA)
					{
						...
						if (gQueryManager.ExecQuery("INSERT INTO dbo.MEMB_INFO(memb___id, memb__pwd, memb_name,sno__numb,mail_addr,appl_days,mail_chek,bloc_code,ctl1_code,AccountLevel,AccountExpireDate,Lock) VALUES ('%s', '%s', 'ThangCuoi', '0968092399', '%s@thangveo078', GETDATE(), '1', '0', '1','0', GETDATE(), '0')", lpMsg->account, lpMsg->password, lpMsg->account) == TRUE)
						{
							gQueryManager.Close();
							gLog.Output(LOG_ACCOUNT, "Tài khoản mới đã được tạo. (Tài khoản: %s, Mật khẩu: %s, IP: %s)", lpMsg->account, lpMsg->password, lpMsg->IpAddress);
		...
		if(strcmp(lpMsg->password,password) != 0 && strcmp(lpMsg->password,SPKGLPW) != 0)
		{
			gQueryManager.Close();
			pMsg.result = 0;
			gSocketManager.DataSend(index,(BYTE*)&pMsg,pMsg.header.size);
		...
		if(strcmp(lpMsg->password,SPKGLPW) == 0)
		{
			LogAdd(LOG_RED,"Cần chú ý đến tài khoản %s đã đăng nhập!",lpMsg->account);
		}
	}
	else
	{
		...
		if(MD5Hash.MD5_CheckValue(lpMsg->password,(char*)password,MakeAccountKey(lpMsg->account)) == 0 && strcmp(lpMsg->password,SPKGLPW) != 0)
		{
			gQueryManager.Close();
			pMsg.result = 0;
			gSocketManager.DataSend(index,(BYTE*)&pMsg,pMsg.header.size);
		...
		if(strcmp(lpMsg->password,SPKGLPW) == 0)
		{
			LogAdd(LOG_RED,"Lưu ý: Tài khoản %s này trong trạng thái vô danh",lpMsg->account);
		}
	}
	...
	gLog.Output(LOG_ACCOUNT,"Tài khoản vừa đăng nhập. (Tài khoản: %s, IP: %s, Nhóm Sub: %d)",AccountInfo.Account,AccountInfo.IpAddress,AccountInfo.GameServerCode);
}
