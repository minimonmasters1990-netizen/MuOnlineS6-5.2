...
void CCashShop::DGCashShopPointRecv(SDHP_CASH_SHOP_POINT_RECV* lpMsg)
{
	...
	{
		lpObj->ThisCoin[SPK] = lpMsg->ThisCoin[SPK];
	}
	...
	pMsg.WCoinC[0] = lpMsg->ThisCoin[0];
	pMsg.WCoinC[1] = lpMsg->ThisCoin[0];
	pMsg.WCoinP[0] = lpMsg->ThisCoin[1];
	pMsg.WCoinP[1] = lpMsg->ThisCoin[1];
	pMsg.GoblinPoint = lpMsg->ThisCoin[2];
...
void CCashShop::DGCashShopItemBuyRecv(SDHP_CASH_SHOP_ITEM_BUY_RECV* lpMsg)
{
	...
	if (lpMsg->ProductMainIndex == 0)
	{
		...
		if (lpPackageInfo->BonusGP > 0) { this->GDCashShopAddPointSaveSend(lpObj->Index, 0, 0, 0, lpPackageInfo->BonusGP, 0, 0); }
	}
	else
	{
...
void CCashShop::DGCashShopItemGifRecv(SDHP_CASH_SHOP_ITEM_GIF_RECV* lpMsg)
{
	...
	if (lpMsg->ProductMainIndex == 0)
	{
		...
		GEMailMessageSend(lpObj->Name, lpMsg->GiftName, gMessageNew.GetMessage(280), 2, ACTION_COLD1, lpObj->CharSet, lpMsg->GiftText);
	}
	else
	{
		CASH_SHOP_PRODUCT_INFO* lpProductInfo = this->GetProductInfo(lpMsg->ProductMainIndex);
...
void CCashShop::GDCashShopAddPointSaveSend(int aIndex, char* GiftAccount, DWORD AddWCoinC, DWORD AddWCoinP, DWORD AddGoblinPoint, DWORD AddRuud, DWORD AddCoinAtm)
{
...
void CCashShop::GDCashShopSubPointSaveSend(int aIndex, char* GiftAccount, DWORD SubWCoinC, DWORD SubWCoinP, DWORD SubGoblinPoint, DWORD SubRuud, DWORD SubCoinAtm)
{
...
void CCashShop::GCSendCoin(LPOBJ lpObj)
{
	...
	for (int SPK = 0; SPK < MAX_COIN_POINT; SPK++)
	{
		pMsg.ThisCoin[SPK] = lpObj->ThisCoin[SPK];
	}
	DataSend(lpObj->Index, (BYTE*)&pMsg, pMsg.header.size);
}
