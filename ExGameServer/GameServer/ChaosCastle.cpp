...
void CChaosCastle::ProcState_EMPTY(CHAOS_CASTLE_LEVEL* lpLevel)
{
	if (lpLevel->RemainTime > 0 && lpLevel->RemainTime <= (this->m_WarningTime * 60))
	{
		...
		if (lpLevel->MinutesLeft != minutes)
		{
			...
			if (lpLevel->Level == 0)
			{
				gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessageNew.GetMessage(60), (lpLevel->MinutesLeft + 1));
			}
		}
	...
	if (lpLevel->RemainTime <= 0)
	{
		if (lpLevel->Level == 0) 
		{ 
			gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessageNew.GetMessage(61)); 
		}

		this->NoticeSendToAll(lpLevel, 1, gMessageNew.GetMessage(62), (lpLevel->Level + 1), this->m_NotifyTime);
...
void CChaosCastle::ProcState_STAND(CHAOS_CASTLE_LEVEL* lpLevel)
{
	...
	if (lpLevel->RemainTime <= 0)
	{
		this->NoticeSendToAll(lpLevel, 0, gMessageNew.GetMessage(63));
...
void CChaosCastle::ProcState_START(CHAOS_CASTLE_LEVEL* lpLevel)
{
	...
	if (lpLevel->RemainTime <= 0)
	{
		this->NoticeSendToAll(lpLevel, 0, gMessageNew.GetMessage(64));
...
void CChaosCastle::MonsterDieProc(LPOBJ lpObj, LPOBJ lpTarget)
{
	...
	if (gServerInfo.ChaosCSReward[0] == 1)
	{
		if (lpObj->Map == MAP_CHAOS_CASTLE7)
		{
			lpTarget->ScoreKillCS++;
			gNotice.GCNoticeSend(lpTarget->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(171), lpTarget->ScoreKillCS, gServerInfo.ChaosCSReward[1]);
			if (lpTarget->ScoreKillCS == gServerInfo.ChaosCSReward[1])
			{
				gItemBagManager.DropItemBySpecialValue(ITEM_BAG_CHAOS_VIP, &gObj[lpUser->Index], gObj[lpUser->Index].Map, gObj[lpUser->Index].X, gObj[lpUser->Index].Y);
				this->NoticeSendToAll(&this->m_ChaosCastleLevel[level], 0, gMessageNew.GetMessage(172), lpTarget->Name, lpTarget->ScoreKillCS);
				return;
			}
			if (lpTarget->ScoreKillCS > gServerInfo.ChaosCSReward[1])
			{
				lpTarget->ScoreKillCS = 0;
				Sleep(500);
			}
		}
	}
...
void CChaosCastle::CGChaosCastleEnterRecv(PMSG_CHAOS_CASTLE_ENTER_RECV* lpMsg, int aIndex)
{
	...
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 2, 0, gMessageNew.GetMessage(266));
		return;
	}
