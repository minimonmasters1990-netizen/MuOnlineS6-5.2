...
public:
	...
	void CalcUserRank(); 
	void CheckThrone();
	void StartThroneCountdown(int guildIndex, int charIndex);
	void StopThroneCountdown();
	void GiveWinningRewards();
	int m_Active;
...
private:
	...
	// Throne related variables
	int m_ThroneMap;
	int m_ThroneX;
	int m_ThroneY;
	int m_ThroneCountdownDuration;
	int m_ThroneCountdown;
	int m_CurrentThroneOwnerIndex;
	int m_CurrentThroneGuildIndex;
	bool m_IsThroneCountingDown;
	
	// Buff reward variables
	int m_BuffStrength;
	int m_BuffAgility;
	int m_BuffVitality;
	int m_BuffEnergy;
	
	// Item drop reward variables
	struct ITEM_DROP_REWARD
	{
		int ItemID;
		int DropRate;
		int ItemLevel;
		int ExcellentOptions;
		int LuckOption;
		int SkillOption;
		int SocketCount;
	};
	std::vector<ITEM_DROP_REWARD> m_ItemDropRewards;
};
