...
void cTuLuyen::LoadConfig(char* FilePath)
{
	this->m_LoadData.clear();
	this->Enable = false;
	this->ThongBao = false;
	...
	this->ThongBao = oTuLuyen.attribute("ThongBao").as_int();
	...
	{
		GetMessageTuLuyen info;
		info.Index = msg.attribute("Index").as_int();
		strcpy_s(info.Message, msg.attribute("Text").as_string());
		this->m_ReadMessage.insert(std::pair<int, GetMessageTuLuyen>(info.Index, info));
	}
	...
	{
		GetDataTuLuyen SaveData;
		SaveData.AllData[0] = CapDo.attribute("LvTuLuyen").as_int();
		...
		this->m_LoadData.insert(std::pair<int, GetDataTuLuyen>(SaveData.AllData[0], SaveData));
	}
}

char* cTuLuyen::GetMessage(int index)
{
	std::map<int, GetMessageTuLuyen>::iterator it = this->m_ReadMessage.find(index);
...
GetDataTuLuyen* cTuLuyen::ReadConfig(int LvTuLuyen)
{
	std::map<int, GetDataTuLuyen>::iterator it = this->m_LoadData.find(LvTuLuyen);
...
void cTuLuyen::SendInfoClient(int aIndex)
{
	if (!this->Enable)
	{
	...
	BYTE send[4096];
	CountListTuLuyen pMsg{};
	pMsg.header.set(0xF3, 0x73, 0);
...
void cTuLuyen::AddOption(LPOBJ lpObj, bool flag)
{
	if (flag != 0 || !this->Enable || OBJECT_RANGE(lpObj->Index) == 0 || gObjIsConnected(lpObj->Index) == false)
	{
		return;
	}
	GetDataTuLuyen* AddValue = this->ReadConfig(lpObj->rTuLuyen);
	if (AddValue <= 0)
	{
		return;
	}
...
bool cTuLuyen::RequestAccess(int aIndex)
{
	if (!this->Enable)
	{
		gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, this->GetMessage(0));
	...
	LPOBJ lpObj = &gObj[aIndex];
	GetDataTuLuyen* pGetDataTuLuyen = this->ReadConfig(lpObj->rTuLuyen + 1);
	...
	int CheckWC = pGetDataTuLuyen->AllData[3];
	//int CheckWP = pGetDataTuLuyen->AllData[4];
	//int CheckGP = pGetDataTuLuyen->AllData[5];
	...
	//if (lpObj->ItemBank[pGetDataTuLuyen->AllData[2]] <= 0 || lpObj->ItemBank[pGetDataTuLuyen->AllData[2]] < pGetDataTuLuyen->AllData[1])
	//{
	//	gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, this->GetMessage(7));
	//	return false;
	//}
	// 
	//gCustomJewelBank.GDCustomJewelBankDelJewel(lpObj, pGetDataTuLuyen->AllData[2], pGetDataTuLuyen->AllData[1]);

	//if (CheckWC > 0 || CheckWP > 0 || CheckGP > 0)
	//{
	//	GDSetCoinSend(lpObj->Index, (CheckWC > 0 ? -CheckWC : 0), (CheckWP > 0 ? -CheckWP : 0), (CheckGP > 0 ? -CheckGP : 0), 0, 0, "rTuLuyen");
	//}

	if (CheckWC > 0)
	{
		GDSetCoinSend(lpObj->Index, -CheckWC, 0, 0, 0, 0, "rTuLuyen");
	}
