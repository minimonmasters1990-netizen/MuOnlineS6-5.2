...
void CCustomNpcCollector::NpcTalk(LPOBJ lpNpc,LPOBJ lpObj)
{
	...
	{
		GCChatTargetSend(lpObj,lpNpc->Index,gMessageNew.GetMessage(540));
		return;
	}
	...
	{
		GCChatTargetSend(lpObj,lpNpc->Index, gMessageNew.GetMessage(541));
		return;
	}
	...
	if (Count == 64)
	{
		for(int i = 0; i < this->m_count; i++)
		{
			...
			{
				//-- Reward
				GCChatTargetSend(lpObj,lpNpc->Index, gMessageNew.GetMessage(542));

				GCFireworksSend(lpNpc,lpNpc->X,lpNpc->Y);

			    gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0, gMessageNew.GetMessage(544));
	...
	GCChatTargetSend(lpObj,lpNpc->Index, gMessageNew.GetMessage(543));
	return;
