...
CDevilSquare::CDevilSquare()
{
	...
	KillCur = 0;
	KillMax = gServerInfo.DevilCSReward[1];
...
void CDevilSquare::ProcState_EMPTY(DEVIL_SQUARE_LEVEL* lpLevel)
{
	if (lpLevel->RemainTime > 0 && lpLevel->RemainTime <= (this->m_WarningTime * 60))
	{
		...
		if (lpLevel->MinutesLeft != minutes)
		{
			...
			if (lpLevel->Level == 0)
			{
				gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessageNew.GetMessage(215), (lpLevel->MinutesLeft + 1));
			}
		}
	...
	this->GCDevilCastleSimpleStateSend(lpLevel, -1);
	...
	{
		if (lpLevel->Level == 0) { gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessageNew.GetMessage(216)); }
		this->NoticeSendToAll(lpLevel, 1, gMessageNew.GetMessage(217), (lpLevel->Level + 1), this->m_NotifyTime);
...
void CDevilSquare::ProcState_STAND(DEVIL_SQUARE_LEVEL* lpLevel)
{
	...
	if (lpLevel->RemainTime <= 0)
	{
		this->NoticeSendToAll(lpLevel, 0, gMessageNew.GetMessage(218));
...
void CDevilSquare::ProcState_START(DEVIL_SQUARE_LEVEL* lpLevel)
{
	...
	this->GCDevilCastleStateSend(lpLevel);
	...
	if (lpLevel->RemainTime <= 0)
	{
		this->NoticeSendToAll(lpLevel, 0, gMessageNew.GetMessage(219));
...
bool CDevilSquare::DelUser(DEVIL_SQUARE_LEVEL* lpLevel, int aIndex)
{
	...
	if (lpLevel->State == DS_STATE_START)
	{
		this->GCDevilCastleSimpleStateSend(lpLevel, aIndex);
	}
...
void CDevilSquare::NpcCharon(LPOBJ lpNpc, LPOBJ lpObj)
{
	...
	if (DS_LEVEL_RANGE(level) == 0)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(220));
		return;
	}
...
void CDevilSquare::MonsterDieProc(LPOBJ lpObj, LPOBJ lpTarget)
{
	...
	if (gServerInfo.DevilCSReward[0] == 1)
	{
		if (lpObj->Map == MAP_DEVIL_SQUARE2)
		{
			lpTarget->ScoreKillDS++;
			KillCur = lpTarget->ScoreKillDS;
			gNotice.GCNoticeSend(lpTarget->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(171), lpTarget->ScoreKillDS, gServerInfo.DevilCSReward[1]);
			if (lpTarget->ScoreKillDS == gServerInfo.DevilCSReward[1])
			{
				...
				this->NoticeSendToAll(&this->m_DevilSquareLevel[level], 0, gMessageNew.GetMessage(172), lpTarget->Name, lpTarget->ScoreKillDS);
				return;
			}
			if (lpTarget->ScoreKillDS > gServerInfo.DevilCSReward[1])
			{
				lpTarget->ScoreKillDS = 0;
				Sleep(500);
			}
		}
	}
...
void CDevilSquare::GCDevilCastleStateSend(DEVIL_SQUARE_LEVEL* lpLevel)
{
	PMSG_DEVIL_CASTLE_STATE_SEND pMsg{};

	pMsg.header.set(0xCD, sizeof(pMsg));

	pMsg.RemainTime = lpLevel->RemainTime;
	pMsg.KillCur = this->KillCur;
	pMsg.KillMax = gServerInfo.DevilCSReward[1];

	this->DataSendToAll(lpLevel, (BYTE*)&pMsg, pMsg.header.size);
}

void CDevilSquare::GCDevilCastleSimpleStateSend(DEVIL_SQUARE_LEVEL* lpLevel, int aIndex)
{
	PMSG_DEVIL_CASTLE_STATE_SEND pMsg{};
