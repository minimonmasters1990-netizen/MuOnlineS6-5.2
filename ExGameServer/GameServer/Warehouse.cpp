...
void CWarehouse::GDWarehouseGuildItemSend(int aIndex,char* account)
{
	...
	for ( int n=0;n<MAX_GUILD_USER;n++)
	{
		...
		if ( lpGuild->Use[n] > 0 && iGuildMemberIndex != -1 )
		{
			...
			{
				gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,gMessageNew.GetMessage(639));
				return;
			}
...
void CWarehouse::GDWarehouseGuildClose(SDHP_WAREHOUSEGUILD_CLOSE_RECV* lpMsg)
{
	...
	gNotice.GCNoticeSend(lpMsg->index,1,0,0,0,0,0, gMessageNew.GetMessage(639));
