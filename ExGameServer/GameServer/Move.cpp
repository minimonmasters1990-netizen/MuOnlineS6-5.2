...
void CMove::Move(LPOBJ lpObj,int index)
{
	...
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessageNew.GetMessage(252),gGate.GetMoveLevel(lpObj,gGate.GetGateMap(MoveInfo.Gate),MoveInfo.MinLevel));
		return;
	}

	if(MoveInfo.MaxLevel != -1 && lpObj->Level > MoveInfo.MaxLevel)
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessageNew.GetMessage(256),MoveInfo.MaxLevel);
		return;
	}

	if(MoveInfo.MinReset != -1 && lpObj->Reset < MoveInfo.MinReset)
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessageNew.GetMessage(254),MoveInfo.MinReset);
		return;
	}

	if(MoveInfo.MaxReset != -1 && lpObj->Reset > MoveInfo.MaxReset)
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessageNew.GetMessage(255),MoveInfo.MaxReset);
		return;
	}

	if(lpObj->Money < ((DWORD)MoveInfo.Money))
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessageNew.GetMessage(257),MoveInfo.Money);
		return;
	}
