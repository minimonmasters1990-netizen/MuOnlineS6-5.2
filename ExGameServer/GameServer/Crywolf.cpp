...
void CCrywolf::ProcState_NOTIFY1()
{
	...
	{
		gCrywolfUtil.SendMapServerGroupMsg(gMessageNew.GetMessage(368));
...
void CCrywolf::ProcState_NOTIFY2()
{
	if ((GetTickCount() - this->m_CrywolfNotifyMsgStartTick) > 70000)
	{
		...
		{
			gCrywolfUtil.SendMapServerGroupMsg(gMessageNew.GetMessage(376), (this->m_StateTimeInfo[this->GetCrywolfState()].GetLeftTime() / 1000));
		}
		else
		{
			gCrywolfUtil.SendMapServerGroupMsg(gMessageNew.GetMessage(377), ((this->m_StateTimeInfo[this->GetCrywolfState()].GetLeftTime() / 1000) / 60));
		}
...
void CCrywolf::ProcState_READY()
{
	...
	if ((GetTickCount() - this->m_CrywolfNotifyMsgStartTick) > 70000)
	{
		if (((this->m_StateTimeInfo[this->GetCrywolfState()].GetLeftTime() / 1000) / 60) == 0)
		{
			gCrywolfUtil.SendMapServerGroupMsg(gMessageNew.GetMessage(378), (this->m_StateTimeInfo[this->GetCrywolfState()].GetLeftTime() / 1000));
		}
		else
		{
			gCrywolfUtil.SendMapServerGroupMsg(gMessageNew.GetMessage(379), ((this->m_StateTimeInfo[this->GetCrywolfState()].GetLeftTime() / 1000) / 60));
		}
...
void CCrywolf::ProcState_START()
{
	...
	{
		gCrywolfUtil.SendCrywolfUserAnyMsg(0, gMessageNew.GetMessage(380));
	...
	{
		this->m_TurnUpBoss = 1;
		gCrywolfUtil.SendCrywolfUserAnyMsg(0, gMessageNew.GetMessage(381));
		this->TurnUpBoss();
	...
	{
		this->m_ChangeAI = 1;
		gCrywolfUtil.SendCrywolfUserAnyMsg(0, gMessageNew.GetMessage(382));
		this->ChangeAI(2);
	...
	{
		gCrywolfUtil.SendCrywolfUserAnyMsg(0, gMessageNew.GetMessage(383));
...
{
	gCrywolfUtil.SendMapServerGroupMsg(gMessageNew.GetMessage(369));
...
{
	gCrywolfUtil.SendMapServerGroupMsg(gMessageNew.GetMessage(370));
...
{
	gCrywolfUtil.SendMapServerGroupMsg(gMessageNew.GetMessage(371));
	...
	if (gCrywolfAltar.GetContractedAltarCount() == 0)
	{
		this->SetOccupationState(1);
		this->SetState(CRYWOLF_STATE_END);
		gCrywolfUtil.SendCrywolfUserAnyMsg(0, gMessageNew.GetMessage(372));
		return;
	}
...
void CCrywolf::SetState_END()
{
	...
	if (this->m_TurnUpBoss != 0 && OBJECT_RANGE(this->m_BossIndex) != 0 && gObj[this->m_BossIndex].Live != 0)
	{
		gCrywolfUtil.SendCrywolfUserAnyMsg(0, gMessageNew.GetMessage(373));
		this->SetOccupationState(1);
	}
	...
	{
		gCrywolfUtil.SendMapServerGroupMsg(gMessageNew.GetMessage(374));
		this->SetOccupationState(1);
		LogAdd(LOG_BLACK, "[ Crywolf ][MVP] FAIL.......!!");
	}
	else
	{
		gCrywolfUtil.SendMapServerGroupMsg(gMessageNew.GetMessage(375));
		this->SetOccupationState(0);
...
void CCrywolf::NotifyCrywolfPersonalRank()
{
	...
	for (int n = OBJECT_START_USER; n < MAX_OBJECT; n++)
	{
		if (gObjIsConnected(n) != 0 && gObj[n].Type == OBJECT_USER && gObj[n].Map == MAP_CRYWOLF)
		{
			...
			gNotice.GCNoticeSend(n, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(384), pMsg.rank);

			gNotice.GCNoticeSend(n, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(385), pMsg.experience);
...
void CCrywolf::CGCrywolfAltarContractRecv(PMSG_CRYWOLF_ALTAR_CONTRACT_RECV* lpMsg, int aIndex)
{
	...
	{
		gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(403));
	}
...
void CCrywolf::CrywolfMonsterDieProc(LPOBJ lpObj, LPOBJ lpTarget)
{
	...
	if (this->GetCrywolfState() == CRYWOLF_STATE_START && lpObj->Map == MAP_CRYWOLF && lpObj->CurrentAI != 0)
	{
		...
		{
			gCrywolfUtil.SendCrywolfUserAnyMsg(0, gMessageNew.GetMessage(388), lpObj->GroupNumber, lpTarget->Name);
		...
		{
			gCrywolfUtil.SendCrywolfUserAnyMsg(0, gMessageNew.GetMessage(387));
			LogAdd(LOG_BLACK, "[ Crywolf ][MVP] [Balgars Dead] by Unknown User");
		}

		if (lpObj->Class == 349 && gObjIsConnected(lpTarget->Index) != 0)
		{
			gCrywolfUtil.SendCrywolfUserAnyMsg(0, gMessageNew.GetMessage(386), lpTarget->Name);
			LogAdd(LOG_BLACK, "[ Crywolf ][MVP] [Balgars Dead] by [%s][%s]", lpTarget->Account, lpTarget->Name);
		}

		if (gObjIsConnected(lpTarget->Index) != 0)
		{
			gNotice.GCNoticeSend(lpTarget->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(389), this->GetUserScore(lpTarget->Index, lpObj->Index, 0));
		}
	}
}
