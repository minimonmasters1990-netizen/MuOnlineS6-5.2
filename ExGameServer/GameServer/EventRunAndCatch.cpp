...
void CEventRunAndCatch::MainProc()
{
	...
	{ 
		...
		{
			gNotice.GCNoticeSendToAll(0,0,0,0,0,0, gMessageNew.GetMessage(583));
			gNotice.GCNoticeSendToAll(0,0,0,0,0,0, gMessageNew.GetMessage(584));
		...
		{
			    this->m_EventRunAndCatchActive = 0;
				gNotice.GCNoticeSendToAll(0,0,0,0,0,0, gMessageNew.GetMessage(585));
...
void CEventRunAndCatch::CommandEventRunAndCatch(LPOBJ lpObj,char* arg) 
{
	...
	if (this->m_RemainTime > 0) 
	{
		...
		gNotice.GCNoticeSendToAll(0,0,0,0,0,0, gMessageNew.GetMessage(586));
	...
	gNotice.GCNoticeSendToAll(0,0,0,0,0,0, gMessageNew.GetMessage(587));
	gNotice.GCNoticeSendToAll(0,0,0,0,0,0, gMessageNew.GetMessage(588));
...
void CEventRunAndCatch::CommandEventRunAndCatchJoin(LPOBJ lpObj,char* arg) 
{
	...
	gNotice.GCNoticeSendToAll(0,0,0,0,0,0, gMessageNew.GetMessage(589),lpObj->Name);
...
void CEventRunAndCatch::EventRunAndCatchHit(int aIndex,int bIndex) 
{
	...
	if (this->m_Members == 1)
	{
		...
		gNotice.GCNoticeSendToAll(0,0,0,0,0,0, gMessageNew.GetMessage(590),lpObj->Name);
	...
	{
		gNotice.GCNoticeSendToAll(0,0,0,0,0,0, gMessageNew.GetMessage(591),lpObj->Name);
