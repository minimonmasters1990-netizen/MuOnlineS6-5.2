...
void CCustomMonster::MonsterDieProc(LPOBJ lpObj,LPOBJ lpTarget)
{
	...
	{
		gNotice.GCNoticeSendToAll(0,0,0,0,0,0, gMessageNew.GetMessage(CustomMonsterInfo.KillMessage),lpTarget->Name);
	}

	if (CustomMonsterInfo.RewardValue1 > 0)
	{
		gNotice.GCNoticeSend(lpTarget->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(51), CustomMonsterInfo.RewardValue1);
	}
	if (CustomMonsterInfo.RewardValue2 > 0)
	{
		gNotice.GCNoticeSend(lpTarget->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(52), CustomMonsterInfo.RewardValue2);
	}
	if (CustomMonsterInfo.RewardValue3 > 0)
	{
		gNotice.GCNoticeSend(lpTarget->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(53), CustomMonsterInfo.RewardValue3);
	}
	if (CustomMonsterInfo.RewardValue4 > 0)
	{
		gNotice.GCNoticeSend(lpTarget->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(54), CustomMonsterInfo.RewardValue4);
	}

	if(CustomMonsterInfo.RewardValue1 > 0
		|| CustomMonsterInfo.RewardValue2 > 0 
		|| CustomMonsterInfo.RewardValue3 > 0 
		|| CustomMonsterInfo.RewardValue4 > 0)
	{
		GDCustomMonsterRewardSaveSend(lpTarget->Index, lpObj->Class, lpObj->Map
			, CustomMonsterInfo.RewardValue1
			, CustomMonsterInfo.RewardValue2
			, CustomMonsterInfo.RewardValue3
			, CustomMonsterInfo.RewardValue4
		);
