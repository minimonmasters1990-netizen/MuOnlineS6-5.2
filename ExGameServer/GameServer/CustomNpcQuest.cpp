...
bool CCustomNpcQuest::CheckCharacter(LPOBJ lpObj,int quest,int Npc)
{
	for(std::vector<CUSTOM_QUEST_CHARACTER>::iterator it=this->m_CustomNpcQuestCharacterInfo.begin();it != this->m_CustomNpcQuestCharacterInfo.end();it++)
	{
		...
		{
			gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0, gMessageNew.GetMessage(592),quest, it->Level);

			if (Npc >= 0)
			{
				GCChatTargetNewSend(lpObj,Npc, gMessageNew.GetMessage(592),quest, it->Level);
			}
			return 0;
		}
		...
		{
			gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0, gMessageNew.GetMessage(593),quest, it->Reset);

			if (Npc >= 0)
			{
				GCChatTargetNewSend(lpObj,Npc, gMessageNew.GetMessage(593),quest, it->Reset);
			}
			return 0;
		}
		...
		{
			gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0, gMessageNew.GetMessage(594),quest, it->MReset);

			if (Npc >= 0)
			{
				GCChatTargetNewSend(lpObj,Npc, gMessageNew.GetMessage(594),quest, it->MReset);
			}
			return 0;
		}
		...
		{
			gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0, gMessageNew.GetMessage(595),quest, it->Zen);

			if (Npc >= 0)
			{
				GCChatTargetNewSend(lpObj,Npc, gMessageNew.GetMessage(595),quest, it->Zen);
			}
			return 0;
		}

		if (it->Coin1 > 0 && lpObj->ThisCoin[0] < it->Coin1)
		{
			gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0, gMessageNew.GetMessage(649),quest, it->Coin1);

			if (Npc >= 0)
			{
				GCChatTargetNewSend(lpObj,Npc, gMessageNew.GetMessage(649),quest, it->Coin1);
			}
			return 0;
		}

		if (it->Coin2 > 0 && lpObj->ThisCoin[1] < it->Coin2)
		{
			gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0, gMessageNew.GetMessage(650),quest, it->Coin2);

			if (Npc >= 0)
			{
				GCChatTargetNewSend(lpObj,Npc, gMessageNew.GetMessage(650),quest, it->Coin2);
			}
			return 0;
		}

		if (it->Coin3 > 0 && lpObj->ThisCoin[2] < it->Coin3)
		{
			gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0, gMessageNew.GetMessage(651),quest, it->Coin3);

			if (Npc >= 0)
			{
				GCChatTargetNewSend(lpObj,Npc, gMessageNew.GetMessage(651),quest, it->Coin3);
			}
			return 0;
		}
		...
		{
			gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessageNew.GetMessage(597),quest);

			if (Npc >= 0)
			{
				GCChatTargetNewSend(lpObj,Npc, gMessageNew.GetMessage(597),quest);
			}
			return 0;
		}
		...
		{
			gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0, gMessageNew.GetMessage(656),quest);

			if (Npc >= 0)
			{
				GCChatTargetNewSend(lpObj,Npc, gMessageNew.GetMessage(656),quest);
			}
			return 0;
		}
		...
		{
			gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0, gMessageNew.GetMessage(657),quest, it->Kills);

			if (Npc >= 0)
			{
				GCChatTargetNewSend(lpObj,Npc, gMessageNew.GetMessage(657),quest, it->Kills);
			}
			return 0;
		}
	}
	...
	for(std::vector<CUSTOM_QUEST_ITEM>::iterator it=this->m_CustomNpcQuestItemInfo.begin();it != this->m_CustomNpcQuestItemInfo.end();it++)
	{
		...
		if (it->Category >= 0 && it->ItemIndex >= 0)
		{
			...
			if (Qtd < it->Quantity)
			{
				...
				gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0, gMessageNew.GetMessage(598),quest,text);

				if (Npc >= 0)
				{
					GCChatTargetNewSend(lpObj,Npc, gMessageNew.GetMessage(598),quest,text);
				}
				return 0;
			}
		}
...
bool CCustomNpcQuest::CheckItemRewardInventorySpace(LPOBJ lpObj,int quest,int Npc)
{
	...
	for(std::vector<CUSTOM_QUEST_REWARD_ITEM>::iterator it=this->m_CustomNpcQuestRewardItemInfo.begin();it != this->m_CustomNpcQuestRewardItemInfo.end();it++)
	{
		...
		{
			gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0, gMessageNew.GetMessage(652),quest);

			if (Npc >= 0)
			{
				GCChatTargetNewSend(lpObj,Npc, gMessageNew.GetMessage(652),quest);
			}
			return 0;
		}
	}
...
void CCustomNpcQuest::DGCustomNpcQuestRecv(SDHP_CUSTOMNPCQUEST_RECV* lpMsg)
{
	...
	if (this->CheckQuest(lpObj,quest,lpMsg->indexnpc,lpMsg->questcount) == 0)
	{
		...
		{
			gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0, gMessageNew.GetMessage(660));

			if (lpMsg->indexnpc >= 0)
			{
				GCChatTargetNewSend(lpObj,lpMsg->indexnpc, gMessageNew.GetMessage(660));
			}
		}
		else 
		{
