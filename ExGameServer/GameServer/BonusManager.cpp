...
void CBonusManager::ProcState_EMPTY(BONUS_INFO* lpInfo)
{
	...
	{
		if(lpInfo->StartMessage != -1){gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessageNew.GetMessage(lpInfo->StartMessage));}
...
void CBonusManager::ProcState_START(BONUS_INFO* lpInfo)
{
	if(lpInfo->RemainTime <= 0)
	{
		if(lpInfo->FinalMessage != -1){gNotice.GCNoticeSendToAll(0,0,0,0,0,0, gMessageNew.GetMessage(lpInfo->FinalMessage));}
