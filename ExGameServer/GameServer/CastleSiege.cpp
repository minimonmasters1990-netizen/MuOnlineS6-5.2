...
int CCastleSiege::DataRequest()
{
	...
	else if(GetTickCount() - this->m_dwDbDataLoadTickCount > 3000)
	{
		bDbRequestData = TRUE;
		m_dwDbDataLoadTickCount = GetTickCount();
		LogAdd(LOG_ORGAN,"[CastleSiege] CCastleSiege::DataRequest() - REQUEST DATA AGAIN");
	}
...
void CCastleSiege::SetState_ENDSIEGE(BOOL bSetRemainMsec)
{
	...
	this->SendAllUserAnyMsg(gMessageNew.GetMessage(416),2);
}
...
void CCastleSiege::ProcState_REGSIEGE()
{
	...
	if( iTICK_MSEC >= 1000 )
	{
		...
		{
			this->SendAllUserAnyMsg(gMessageNew.GetMessage(417), 2);
...
void CCastleSiege::ProcState_REGMARK()
{
	...
	if( iTICK_MSEC >= 1000 )
	{
		...
		if( (GetTickCount() - this->m_dwEVENT_MSG_TICK_COUNT) > 1800000 )
		{
			this->SendAllUserAnyMsg(gMessageNew.GetMessage(418), 2);
			this->m_dwEVENT_MSG_TICK_COUNT = GetTickCount();
		}
...
void CCastleSiege::ProcState_IDLE_3()
{
	...
	if( iTICK_MSEC >= 1000 )
	{
		...
		{
			this->SendAllUserAnyMsg(gMessageNew.GetMessage(419), 2);
			this->m_dwEVENT_MSG_TICK_COUNT = GetTickCount();
		}
...
void CCastleSiege::ProcState_NOTIFY()
{
	...
	if( iTICK_MSEC >= 1000 )
	{
		...
		if( (GetTickCount() - this->m_dwEVENT_MSG_TICK_COUNT) > 1800000 )
		{
			this->SendAllUserAnyMsg(gMessageNew.GetMessage(420), 2);
			this->m_dwEVENT_MSG_TICK_COUNT = GetTickCount();
		}
...
void CCastleSiege::ProcState_READYSIEGE()
{
	...
	if( iTICK_MSEC >= 1000 )
	{
		...
		if( (GetTickCount() - this->m_dwEVENT_MSG_TICK_COUNT) > 1800000 )
		{
			this->SendAllUserAnyMsg(gMessageNew.GetMessage(421), 2);
			this->m_dwEVENT_MSG_TICK_COUNT = GetTickCount();
		}
		if( this->m_iCS_REMAIN_MSEC <= 300000 )
		{
			...
			{
				char szBuff[256]={0};
				wsprintf(szBuff, gMessageNew.GetMessage(422), (this->m_iCS_REMAIN_MSEC/60000)+1);
...
void CCastleSiege::ProcState_STARTSIEGE()
{
	...
	if( iTICK_MSEC >= 1000 )
	{
		...
		if( (GetTickCount() - this->m_dwEVENT_MSG_TICK_COUNT) > 1800000 )
		{
			this->SendAllUserAnyMsg(gMessageNew.GetMessage(423), 2);
			this->m_dwEVENT_MSG_TICK_COUNT = GetTickCount();
		}
...
int CCastleSiege::CheckCastleSiegeResult()
{
	...
	if(this->m_btIsCastleOccupied == TRUE && strcmp(this->m_szCastleOwnerGuild,"") != FALSE)
	{
		if(strcmp(this->m_szMiddleWinnerGuild,"") == FALSE || strcmp(this->m_szMiddleWinnerGuild,this->m_szCastleOwnerGuild) == FALSE)
		{
			...
			wsprintf(szMsg, gMessageNew.GetMessage(424),this->m_szCastleOwnerGuild);
			bRETVAL = FALSE;
		}
		else
		{
			...
			wsprintf(szMsg, gMessageNew.GetMessage(425),this->m_szCastleOwnerGuild);
			bRETVAL = TRUE;
		}
	}
	else
	{
		...
		{
			this->m_btIsCastleOccupied = 0;
			wsprintf(szMsg, gMessageNew.GetMessage(426));
			bRETVAL = FALSE;
		}
		else
		{
