...
void CCastleDeep::ProcState_EMPTY()
{
	if (this->m_RemainTime > 0 && this->m_RemainTime <= 300 && this->m_TimeNotify == 0)
	{
		...
		GDGlobalNoticeSend(gMapServerManager.GetMapServerGroup(), 0, 0, 0, 0, 0, 0, gMessageNew.GetMessage(352));
	}

	if (this->m_RemainTime <= 0)
	{
		GDGlobalNoticeSend(gMapServerManager.GetMapServerGroup(), 0, 0, 0, 0, 0, 0, gMessageNew.GetMessage(353));
...
void CCastleDeep::ProcState_START()
{
	...
	if (this->m_RemainTime <= 0)
	{
		gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessageNew.GetMessage(354));
...
void CCastleDeep::MonsterDieProc(LPOBJ lpObj, LPOBJ lpTarget)
{
	...
	{
		gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessageNew.GetMessage(355), lpTarget->Name);
	}

	if (lpObj->Attribute == 62 && lpObj->Class >= 300 && lpObj->Class <= 303)
	{
		gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessageNew.GetMessage(356), lpObj->Name, lpTarget->Name);
	}
}
