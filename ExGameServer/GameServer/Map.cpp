...
CMap::CMap()
{
	...
	m_NextWeatherTimer = (GetLargeRand() % 10000) + 10000;
	m_MapAttr = 0;
	m_width = 0;
	m_height = 0;
	m_path = new CMapPath();
	...
	m_MapRespawn[0] = { 130, 116, 151, 137 };
	m_MapRespawn[1] = { 106, 236, 112, 243 };
	m_MapRespawn[2] = { 197, 35, 218, 50 };
	m_MapRespawn[3] = { 174, 101, 187, 125 };
	m_MapRespawn[4] = { 201, 70, 213, 81 };
	m_MapRespawn[5] = { 89, 135, 90, 136 };
	m_MapRespawn[6] = { 89, 135, 90, 136 };
	m_MapRespawn[7] = { 14, 11, 27, 23 };
	m_MapRespawn[8] = { 187, 54, 203, 69 };
	m_MapRespawn[33] = { 82, 8, 87, 14 };
	m_MapRespawn[34] = { 133, 41, 140, 44 };
	m_MapRespawn[51] = { 40, 214, 43, 224 };
}
...
void CMap::Load(char* path, int map)
{
	...
	if (m_MapAttr != nullptr)
	{
		delete[] m_MapAttr;
		m_MapAttr = nullptr;
	}

	m_MapAttr = new BYTE[size];
	...
	if (ReadFile(file, m_MapAttr, size, &bytesRead, 0) == 0)
	{
		ErrorMessageBox(MEM_SCRIPT_ERROR_CODE2, path);
		CloseHandle(file);
		return;
	...
	m_path->SetMapDimensions(m_width, m_height, m_MapAttr);
...
BYTE CMap::GetAttr(int x, int y)
{
	...
	return m_MapAttr[(y * m_width) + x];
}

bool CMap::CheckAttr(int x, int y, BYTE attr)
{
	return (GetAttr(x, y) & attr) != 0;
}
...
{
	return !(CheckAttr(x, y, 2) || CheckAttr(x, y, 4) || CheckAttr(x, y, 8));
}
...
bool CMap::CheckItemGive(int aIndex, int index)
{
	...
	if (loot == 0)
	{
		gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(290));
		return 0;
	}
