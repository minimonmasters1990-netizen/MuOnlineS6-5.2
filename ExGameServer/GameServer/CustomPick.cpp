...
void CCustomPick::PickProc(LPOBJ lpObj)
{
	...
	if (gMapManager.GetMapCustomPick(lpObj->Map) == 0)
	{
		...
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0, gMessageNew.GetMessage(554));
...
bool CCustomPick::CommandPick(LPOBJ lpObj,char* arg)   
{
	...
	if(strcmp(mode,"exc") == 0)
	{
		...
		{
			lpObj->PickupExc = 0;
		    gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0, gMessageNew.GetMessage(553),mode);
		...
		{
			lpObj->PickupExc = 1;
			gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessageNew.GetMessage(552),mode);
...
void CCustomPick::OnPickClose(LPOBJ lpObj)
{
