...
void CParty::CGPartyRequestRecv(PMSG_PARTY_REQUEST_RECV* lpMsg,int aIndex)
{
	...
	{
		gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(46));
		return;
	}
	if (lpTarget->DieRegen != 0 || lpObj->DieRegen != 0)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(47));
		return;
	}
	...
	if (gServerInfo.m_PartyRestrict == 1 && this->IsParty(lpTarget->PartyNumber) == 0) 
	{
		if ( gObj[aIndex].PartyNumber >= 0 )
		{
			if ( gParty.GetLevel( gObj[aIndex].PartyNumber) == TRUE )
			{
				...
				{
					gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,2,0, gMessageNew.GetMessage(48),gServerInfo.m_DifferenceMaxLevelParty);
					return;
				}
...
void CParty::CGPartyListRecv(int aIndex)
{
	...
	if(this->IsParty(lpObj->PartyNumber) != 0)
	{
		...
		for(int n=0;n < MAX_PARTY_USER;n++)
		{
			...
			info.cClass = lpTarget->Class;

			info.cLevel = lpTarget->Level;
...
void CParty::GCPartyListSend(int index)
{
	...
	for(int n=0;n < MAX_PARTY_USER;n++)
	{
		...
		info.cClass = lpObj->Class;

		info.cLevel = lpObj->Level;
