...
void CItemOption::CalcItemCommonOption(LPOBJ lpObj,bool flag)
{
	for(int n=0;n < INVENTORY_WEAR_SIZE;n++)
	{
		if(lpObj->Inventory[n].IsItem() != 0 && lpObj->Inventory[n].m_IsValidItem != 0 && lpObj->Inventory[n].m_Durability != 0)
		{
			...
			this->InsertOptionBEX(lpObj, &lpObj->Inventory[n], flag);
...
void CItemOption::LoadBEX(char* FilePath) // OK
{
	...
	//--
	pugi::xml_node oItemOptionInfoBEX = file.child("ItemOptionInfoBEX");
	for (pugi::xml_node List = oItemOptionInfoBEX.child("List"); List; List = List.next_sibling())
	{
		ITEM_OPTION_BEX bInfo;

		bInfo.ItemSet = List.attribute("ItemSet").as_int();
		bInfo.ItemMinIndex = List.attribute("ItemMinIndex").as_int();
		bInfo.ItemMaxIndex = List.attribute("ItemMaxIndex").as_int();
		bInfo.ItemLevelMin = List.attribute("ItemLevelMin").as_int();
		bInfo.ItemLevelMax = List.attribute("ItemLevelMax").as_int();
		bInfo.ItemExc = List.attribute("ItemExc").as_int();

		bInfo.OptionIndex[0] = List.attribute("OptionIndex1").as_int(0);
		bInfo.OptionValue[0] = List.attribute("OptionValue1").as_int(0);
		//==
		bInfo.OptionIndex[1] = List.attribute("OptionIndex2").as_int(0);
		bInfo.OptionValue[1] = List.attribute("OptionValue2").as_int(0);
		//==
		bInfo.OptionIndex[2] = List.attribute("OptionIndex3").as_int(0);
		bInfo.OptionValue[2] = List.attribute("OptionValue3").as_int(0);
		//==
		bInfo.OptionIndex[3] = List.attribute("OptionIndex4").as_int(0);
		bInfo.OptionValue[3] = List.attribute("OptionValue4").as_int(0);


		this->m_ItemOptionInfoBEX.push_back(bInfo);
	}
}
bool CheckItemSlotEx(LPOBJ lpObj, int IndexItem, int LevelMin, int LevelMax, int Exc)
{
	int CountSlot = 0;
	for (int n = 2; n < 7; n++)
	{
		if (lpObj->Inventory[n].IsItem() != 0 && lpObj->Inventory[n].m_IsValidItem != 0 && lpObj->Inventory[n].m_Durability != 0)
		{
			if (lpObj->Inventory[n].m_Index == GET_ITEM(7, IndexItem) ||
				lpObj->Inventory[n].m_Index == GET_ITEM(8, IndexItem) ||
				lpObj->Inventory[n].m_Index == GET_ITEM(9, IndexItem) ||
				lpObj->Inventory[n].m_Index == GET_ITEM(10, IndexItem) ||
				lpObj->Inventory[n].m_Index == GET_ITEM(11, IndexItem)) // Check du 5 mon thi kich hoat cong dmg
			{
				if ((Exc == 1 && lpObj->Inventory[n].IsExcItem()) || Exc == -1)
				{
					if ((LevelMin <= lpObj->Inventory[n].m_Level) && (LevelMax >= lpObj->Inventory[n].m_Level))
					{
						CountSlot++;
					}
				}
	...
	if (((lpObj->Class == 3 || lpObj->Class == 6) && CountSlot >= 4) || CountSlot >= 5)
	{
		return 1;
	}
	return 0;
...
void CItemOption::InsertOptionBEX(LPOBJ lpObj, CItem* lpItem, bool flag) // OK
{
	...
	for (std::vector<ITEM_OPTION_BEX>::iterator it = this->m_ItemOptionInfoBEX.begin(); it != this->m_ItemOptionInfoBEX.end(); it++)
	{
		if (lpItem->m_Index >= it->ItemMinIndex && lpItem->m_Index <= it->ItemMaxIndex && it->ItemSet != 1)
		{
			if (lpItem->m_Level >= it->ItemLevelMin && lpItem->m_Level <= it->ItemLevelMax)
			{
				if (lpItem->m_NewOption >= it->ItemExc || it->ItemExc == -1)
				{
					for (int n = 0; n < 4; n++)
					{
						gItemOption.CBInsertOption(lpObj, it->OptionIndex[n], it->OptionValue[n]);
					}
				}
			}
		}
		else if (it->ItemSet == 1 && lpItem->m_Index >= it->ItemMinIndex && lpItem->m_Index <= it->ItemMaxIndex)
		{
			int GetIndexItemSet = (lpItem->m_Index % 512);
			if (CheckItemSlotEx(lpObj, GetIndexItemSet, it->ItemLevelMin, it->ItemLevelMax, it->ItemExc))
			{
				for (int n = 0; n < 4; n++)
				{
					gItemOption.CBInsertOption(lpObj, it->OptionIndex[n], (it->OptionValue[n]));
...
void CItemOption::CBInsertOption(LPOBJ lpObj, int SpecialIndex, int SpecialValue) // OK
{
	//for (int n = 0; n < MAX_SPECIAL; n++)
	{
		...
		case ITEM_OPTION_ADD_TRIPLE_DAMAGE_RATE:
			...
			break;
			//Kháng
		case ITEM_OPTION_ADD_RESISTE_DOUBLE_RATE:
			lpObj->ResistDoubleDamageRate += value;
			break;
		case ITEM_OPTION_ADD_RESISTE_IGNOREDEFENSE_RATE:
			lpObj->ResistIgnoreDefenseRate += value;
			break;
		case ITEM_OPTION_ADD_RESISTE_IGNORESD_RATE:
			lpObj->ResistIgnoreShieldGaugeRate += value;
			break;
		case ITEM_OPTION_ADD_RESISTE_CRITICA_DAMEGE_RATE:
			lpObj->ResistCriticalDamageRate += value;
			break;
		case ITEM_OPTION_ADD_RESISTE_EXCELLENT_DAMEGE_RATE:
			lpObj->ResistExcellentDamageRate += value;
			break;
		case ITEM_OPTION_ADD_RESISTE_STUN_RATE:
