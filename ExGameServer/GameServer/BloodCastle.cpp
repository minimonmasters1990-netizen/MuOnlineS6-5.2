...
void CBloodCastle::ProcState_EMPTY(BLOOD_CASTLE_LEVEL* lpLevel)
{
	if(lpLevel->RemainTime > 0 && lpLevel->RemainTime <= (this->m_WarningTime*60))
	{
		...
		if(lpLevel->MinutesLeft != minutes)
		{
			...
			if(lpLevel->Level == 0)
			{
				gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessageNew.GetMessage(14),(lpLevel->MinutesLeft+1));
			}
		}
	...
	if(lpLevel->RemainTime <= 0)
	{
		if(lpLevel->Level == 0)
		{
			gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessageNew.GetMessage(15));
		}
		this->NoticeSendToAll(lpLevel,1, gMessageNew.GetMessage(16),(lpLevel->Level+1),this->m_NotifyTime);
...
void CBloodCastle::ProcState_STAND(BLOOD_CASTLE_LEVEL* lpLevel)
{
	...
	if(lpLevel->RemainTime <= 0)
	{
		this->NoticeSendToAll(lpLevel,0, gMessageNew.GetMessage(17));
...
void CBloodCastle::ProcState_START(BLOOD_CASTLE_LEVEL* lpLevel)
{
	...
	{
		case 0:			this->SetStage0(lpLevel);			break;
		case 1:			this->SetStage1(lpLevel);			break;
		case 2:			this->SetStage2(lpLevel);			break;
		case 3:			this->SetStage3(lpLevel);			break;
		case 4:			this->SetStage4(lpLevel);			break;
	}
	...
	if(lpLevel->RemainTime <= 0)
	{
		this->NoticeSendToAll(lpLevel,0, gMessageNew.GetMessage(18));
...
void CBloodCastle::SetState(BLOOD_CASTLE_LEVEL* lpLevel,int state)
{
	...
	switch(lpLevel->State)
	{
		case BC_STATE_BLANK:			this->SetState_BLANK(lpLevel);			break;
		case BC_STATE_EMPTY:			this->SetState_EMPTY(lpLevel);			break;
		case BC_STATE_STAND:			this->SetState_STAND(lpLevel);			break;
		case BC_STATE_START:			this->SetState_START(lpLevel);			break;
		case BC_STATE_CLEAN:			this->SetState_CLEAN(lpLevel);			break;
	}
}
...
void CBloodCastle::SetStage0(BLOOD_CASTLE_LEVEL* lpLevel)
{
	...
	this->NoticeSendToAll(lpLevel,0, gMessageNew.GetMessage(19));
}
...
void CBloodCastle::SetStage2(BLOOD_CASTLE_LEVEL* lpLevel)
{
	...
	switch(lpLevel->Level)
	{
		case 0:			this->SetMonster(lpLevel,89);			break;
		case 1:			this->SetMonster(lpLevel,95);			break;
		case 2:			this->SetMonster(lpLevel,112);			break;
		case 3:			this->SetMonster(lpLevel,118);			break;
		case 4:			this->SetMonster(lpLevel,124);			break;
		case 5:			this->SetMonster(lpLevel,130);			break;
		case 6:			this->SetMonster(lpLevel,143);			break;
		case 7:			this->SetMonster(lpLevel,433);			break;
	}
}
...
void CBloodCastle::SetStage3(BLOOD_CASTLE_LEVEL* lpLevel)
{
	...
	this->NoticeSendToAll(lpLevel,0, gMessageNew.GetMessage(20));
...
void CBloodCastle::GiveUserRewardItem(BLOOD_CASTLE_LEVEL* lpLevel,int aIndex)
{
	...
	switch(lpLevel->Level)
	{
		case 0:			gItemBagManager.DropItemBySpecialValue(ITEM_BAG_BLOOD_CASTLE1,&gObj[lpUser->Index],gObj[lpUser->Index].Map,gObj[lpUser->Index].X,gObj[lpUser->Index].Y);			break;
		case 1:			gItemBagManager.DropItemBySpecialValue(ITEM_BAG_BLOOD_CASTLE2,&gObj[lpUser->Index],gObj[lpUser->Index].Map,gObj[lpUser->Index].X,gObj[lpUser->Index].Y);			break;
		case 2:			gItemBagManager.DropItemBySpecialValue(ITEM_BAG_BLOOD_CASTLE3,&gObj[lpUser->Index],gObj[lpUser->Index].Map,gObj[lpUser->Index].X,gObj[lpUser->Index].Y);			break;
		case 3:			gItemBagManager.DropItemBySpecialValue(ITEM_BAG_BLOOD_CASTLE4,&gObj[lpUser->Index],gObj[lpUser->Index].Map,gObj[lpUser->Index].X,gObj[lpUser->Index].Y);			break;
		case 4:			gItemBagManager.DropItemBySpecialValue(ITEM_BAG_BLOOD_CASTLE5,&gObj[lpUser->Index],gObj[lpUser->Index].Map,gObj[lpUser->Index].X,gObj[lpUser->Index].Y);			break;
		case 5:			gItemBagManager.DropItemBySpecialValue(ITEM_BAG_BLOOD_CASTLE6,&gObj[lpUser->Index],gObj[lpUser->Index].Map,gObj[lpUser->Index].X,gObj[lpUser->Index].Y);			break;
		case 6:			gItemBagManager.DropItemBySpecialValue(ITEM_BAG_BLOOD_CASTLE7,&gObj[lpUser->Index],gObj[lpUser->Index].Map,gObj[lpUser->Index].X,gObj[lpUser->Index].Y);			break;
		case 7:			gItemBagManager.DropItemBySpecialValue(ITEM_BAG_BLOOD_CASTLE8,&gObj[lpUser->Index],gObj[lpUser->Index].Map,gObj[lpUser->Index].X,gObj[lpUser->Index].Y);			break;
	}
...
void CBloodCastle::NpcAngelMessenger(LPOBJ lpNpc,LPOBJ lpObj)
{
	...
	if(BC_LEVEL_RANGE(level) == 0)
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessageNew.GetMessage(21));
		return;
	}
...
void CBloodCastle::DropEventItem(int map,int number,int aIndex)
{
	...
	this->NoticeSendToAll(&this->m_BloodCastleLevel[level],0,gMessageNew.GetMessage(22),gObj[aIndex].Name);
}
...
void CBloodCastle::GetEventItem(int map,int aIndex,CMapItem* lpItem)
{
	...
	this->NoticeSendToAll(&this->m_BloodCastleLevel[level],0, gMessageNew.GetMessage(23),gObj[aIndex].Name);
}
...
void CBloodCastle::MonsterDieProc(LPOBJ lpObj,LPOBJ lpTarget)
{
	...
	if(lpObj->Class == 131 && lpObj->Index == this->m_BloodCastleLevel[level].GateIndex && this->m_BloodCastleLevel[level].Stage == 2)
	{
		...
		this->NoticeSendToAll(&this->m_BloodCastleLevel[level],0,gMessageNew.GetMessage(24),lpTarget->Name);
		return;
	}
	...
	if(lpObj->Class >= 132 && lpObj->Class <= 134 && lpObj->Index == this->m_BloodCastleLevel[level].SaintStatueIndex && this->m_BloodCastleLevel[level].Stage == 4)
	{
		...
		this->NoticeSendToAll(&this->m_BloodCastleLevel[level],0,gMessageNew.GetMessage(25),lpTarget->Name);
		return;
	}
	...
	if (gServerInfo.BloodCSReward[0] == 1)
	{
		if (lpObj->Map == MAP_BLOOD_CASTLE8) 
		{
			lpTarget->ScoreKillBC++;
			gNotice.GCNoticeSend(lpTarget->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(171), lpTarget->ScoreKillBC, gServerInfo.BloodCSReward[1]);
			if (lpTarget->ScoreKillBC == gServerInfo.BloodCSReward[1])
			{
				...
				this->NoticeSendToAll(&this->m_BloodCastleLevel[level], 0, gMessageNew.GetMessage(172), lpTarget->Name, lpTarget->ScoreKillBC);
				return;
			}
			if (lpTarget->ScoreKillBC > gServerInfo.BloodCSReward[1])
			{
				lpTarget->ScoreKillBC = 0;
				Sleep(500);
			}
		}
	}
...
void CBloodCastle::CGBloodCastleEnterRecv(PMSG_BLOOD_CASTLE_ENTER_RECV* lpMsg,int aIndex)
{
	...
	switch(lpMsg->level)
	{
		case 0:			gObjMoveGate(aIndex,66);			break;
		case 1:			gObjMoveGate(aIndex,67);			break;
		case 2:			gObjMoveGate(aIndex,68);			break;
		case 3:			gObjMoveGate(aIndex,69);			break;
		case 4:			gObjMoveGate(aIndex,70);			break;
		case 5:			gObjMoveGate(aIndex,71);			break;
		case 6:			gObjMoveGate(aIndex,80);			break;
		case 7:			gObjMoveGate(aIndex,271);			break;
	}
}
