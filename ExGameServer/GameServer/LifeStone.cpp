...
bool CLifeStone::CreateLifeStone(int aIndex)
{
	...
	{
		gNotice.GCNoticeSend(iIndex,1,0,0,0,0,0,gMessageNew.GetMessage(448));
		return FALSE;
	}
	...
	{
		gNotice.GCNoticeSend(iIndex,1,0,0,0,0,0, gMessageNew.GetMessage(449));
		return FALSE;
	}

	if ( lpObj->Guild->LifeStone )
	{
		gNotice.GCNoticeSend(iIndex,1,0,0,0,0,0, gMessageNew.GetMessage(450));
		return FALSE;
	}

	if( cX > 150 && cX < 210 && cY > 175 && cY < 230 )
	{
		gNotice.GCNoticeSend(iIndex,1,0,0,0,0,0, gMessageNew.GetMessage(451));
		return FALSE;
	}
	...
	if ( iMonsterIndex >= 0 )
	{
		...
		gNotice.GCNoticeSend(iIndex,1,0,0,0,0,0, gMessageNew.GetMessage(452));
	...
	{
		gNotice.GCNoticeSend(iIndex,1,0,0,0,0,0, gMessageNew.GetMessage(453));
		return FALSE;
	}
	return TRUE;
