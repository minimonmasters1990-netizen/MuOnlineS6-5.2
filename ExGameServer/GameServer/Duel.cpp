...
bool CDuel::AddDuelArena(LPOBJ lpObj, LPOBJ lpTarget)
{
	for (int n = 0; n < MAX_DUEL_ARENA; n++)
	{
		...
		if (lpInfo->State == DA_STATE_EMPTY)
		{
			...
			if (gServerInfo.m_DuelArenaAnnounceSwitch != 0) { gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessageNew.GetMessage(222), lpObj->Name, lpTarget->Name); }
...
void CDuel::CheckDuelScore(LPOBJ lpObj, LPOBJ lpTarget)
{
	...
	if (lpObj->DuelScore >= gServerInfo.m_DuelMaxScore)
	{
		...
		if (lpInfo != 0)
		{
			...
			if (gServerInfo.m_DuelArenaAnnounceSwitch != 0) { gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessageNew.GetMessage(224), lpObj->Name, lpTarget->Name); }
...
void CDuel::CGDuelStartRecv(PMSG_DUEL_START_RECV* lpMsg, int aIndex)
{
	...
	{
		gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(225));
		return;
	}
	...
	{
		gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(226));
		return;
	}

	if ((lpTarget->Option & 2) == 0)
	{
		gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(227));
		return;
	}

	if (OBJECT_RANGE(lpObj->DuelUserReserved) != 0 || OBJECT_RANGE(lpObj->DuelUserRequested) != 0 || OBJECT_RANGE(lpObj->DuelUser) != 0)
	{
		gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(228));
		return;
	}

	if (OBJECT_RANGE(lpTarget->DuelUserReserved) != 0 || OBJECT_RANGE(lpTarget->DuelUserRequested) != 0 || OBJECT_RANGE(lpTarget->DuelUser) != 0)
	{
		gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(229));
		return;
	}

	if (lpObj->Guild != 0 && lpObj->Guild->WarState == 1)
	{
		gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(230));
		return;
	}

	if (lpTarget->Guild != 0 && lpTarget->Guild->WarState == 1)
	{
		gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(231));
		return;
	}
	...
	{
		gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(445), 50);
		return;
	}

	if (gMapServerManager.CheckMapServer(MAP_DUEL_ARENA) == 0)
	{
		gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(232));
		return;
	}
	...
	{
		gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(233));
		return;
	}
	...
	{
		gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(234));
		return;
	}
	...
	for (int n = 0; n < MAX_SELF_DEFENSE; n++)
	{
		...
		{
			gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(235));
			return;
		}

		if (lpTarget->SelfDefense[n] >= 0)
		{
			gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(236));
			return;
		}
	}
...
void CDuel::CGDuelOkRecv(PMSG_DUEL_OK_RECV* lpMsg, int aIndex)
{
	...
	{
		gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(237));
		this->ResetDuel(lpObj);
		this->ResetDuel(lpTarget);
	...
	{
		gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(223));
		this->ResetDuel(lpObj);
		this->ResetDuel(lpTarget);
		this->GCDuelStartSend(bIndex, 0, aIndex);
		return;
...
void CDuel::StartDuelBit(LPOBJ lpObj, LPOBJ lpTarget)
{
	if (this->AddDuelArena(lpObj, lpTarget) == 0)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(223));
		this->ResetDuel(lpObj);
		this->ResetDuel(lpTarget);
