...
void CInvasionManager::ProcState_EMPTY(INVASION_INFO* lpInfo)
{
	if (lpInfo->RemainTime > 0 && lpInfo->RemainTime <= (lpInfo->AlarmTime * 60))
	{
		if ((lpInfo->AlarmMinSave = (((lpInfo->RemainTime % 60) == 0) ? ((lpInfo->RemainTime / 60) - 1) : (lpInfo->RemainTime / 60))) != lpInfo->AlarmMinLeft)
		{
			...
			gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessageNew.GetMessage(0), lpInfo->AlertMessage, (lpInfo->AlarmMinLeft + 1));

		}
	}
	if (lpInfo->RemainTime <= 0)
	{
		gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, "%s %s %s", gMessageNew.GetMessage(200), lpInfo->AlertMessage, gMessageNew.GetMessage(201));
...
void CInvasionManager::ProcState_START(INVASION_INFO* lpInfo)
{
	if (lpInfo->RemainTime <= 0)
	{
		gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, "%s %s %s", gMessageNew.GetMessage(200), lpInfo->AlertMessage, gMessageNew.GetMessage(202));
...
void CInvasionManager::MonsterDieProc(LPOBJ lpObj, LPOBJ lpTarget)
{
	...
	for (int n = 0; n < MAX_INVASION; n++)
	{
		...
		if (lpObj->Class == lpInfo->BossIndex && lpInfo->BossMessage != -1)
		{
			gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessageNew.GetMessage(lpInfo->BossMessage), lpTarget->Name);
			continue;
		}
	}
}
...
void CInvasionManager::StartInvasion(int index)
{
	...
	LogAdd(LOG_EVENT, "Sự Kiện (%d) Diễn Ra Vào Lúc %2dGiờ %2dPhút 00Giây", index, hour, minute);
