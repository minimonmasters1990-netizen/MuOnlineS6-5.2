...
bool CItemManager::CheckItemMoveToInventory(LPOBJ lpObj, CItem* lpItem, int slot)
{
	...
	if (lpItem->m_Slot == ITEM_SLOT_CUSTOM)
	{
		return (slot >= INVENTORY_CUSTOM_WEAR_START && slot <= INVENTORY_CUSTOM_WEAR_END) ? 1 : 0;
	}
...
{
	if (lpItem->m_Slot == 0xFF || (INVENTORY_WEAR_RANGE(lpItem->m_Slot) == 0 && lpItem->m_Slot != ITEM_SLOT_CUSTOM))
	{
		return 0;
	}
