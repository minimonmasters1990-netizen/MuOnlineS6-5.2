...
void CTvTEvent::ProcState_EMPTY()
{
	if (this->m_RemainTime > 0 && this->m_RemainTime <= (this->m_WarningTime * 60))
	{
		...
		if ((this->AlarmMinSave = (((this->m_RemainTime % 60) == 0) ? ((this->m_RemainTime / 60) - 1) : (this->m_RemainTime / 60))) != this->AlarmMinLeft)
		{
			...
			gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessageNew.GetMessage(625), (this->AlarmMinLeft + 1));
		}
	}

	if (this->m_RemainTime <= 0)
	{
		gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessageNew.GetMessage(627));
		this->NoticeSendToAll(1, gMessageNew.GetMessage(628), this->m_StandTime);
...
void CTvTEvent::ProcState_STAND()
{
	...
	{
		this->NoticeSendToAll(0, gMessageNew.GetMessage(630));
		this->SetState(TVT_EVENT_STATE_EMPTY);
		return;
	}
	...
	if (this->m_RemainTime <= 0)
	{
		...
		{
			this->DivisionTeam();
			this->NoticeSendToAll(0, gMessageNew.GetMessage(629));
...
bool CTvTEvent::Dialog(LPOBJ lpObj, LPOBJ lpNpc)
{
	...
	{
		GCChatTargetSend(lpObj, lpNpc->Index, gMessageNew.GetMessage(626));
...
bool CTvTEvent::CheckEnterEnabled(LPOBJ lpObj)
{
	...
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(631));
		return 0;
	}
	...
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(632));
		return 0;
	}
