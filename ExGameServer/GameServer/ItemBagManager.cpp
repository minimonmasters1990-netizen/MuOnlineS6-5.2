...
bool CItemBagManager::DropItemByIndexItemBag(int IndexItemBag, LPOBJ lpObj)
{
	for (std::map<int, ITEM_BAG_MANAGER_INFO>::iterator it = this->m_ItemBagManagerInfo.begin(); it != this->m_ItemBagManagerInfo.end(); ++it)
	{
		if (it->second.Index != -1 && it->second.Index == IndexItemBag)
		{
			if (it->second.DropType == 1)
			{
				bool result = it->second.ItemBag.DropInventoryItem(lpObj, lpObj->Map, lpObj->X, lpObj->Y);
				...
				{
					LogAdd(LOG_RED, "[DropItemByIndexItemBag] Failed to drop inventory item for [%s][%s] - ItemBagIndex: %d", lpObj->Account, lpObj->Name, IndexItemBag);
					return 2;
				}
			}
			else
