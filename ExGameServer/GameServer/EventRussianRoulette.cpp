...
void CEventRussianRoulette::MainProc()
{
	...
	{ 
		if(this->m_RemainTime <= (this->m_TotalTime-gServerInfo.m_EventRussianRouletteTimeToEnter) && this->m_EventRussianRouletteEnter == 1)
		{
			if (this->m_Members < 2)
			{
				gNotice.GCNoticeSendToAll(0,0,0,0,0,0, gMessageNew.GetMessage(507));
			...
			{
				gNotice.GCNoticeSendToAll(0,0,0,0,0,0, gMessageNew.GetMessage(504));
				gNotice.GCNoticeSendToAll(0,0,0,0,0,0, gMessageNew.GetMessage(505));
		...
		{
			    this->m_EventRussianRouletteActive = 0;
				gNotice.GCNoticeSendToAll(0,0,0,0,0,0, gMessageNew.GetMessage(506));
	...
	{ 
		...
		{
			gNotice.GCNoticeSendToAll(0,0,0,0,0,0, gMessageNew.GetMessage(512));
		}

		if(this->m_RouletteRemainTime == 6)
		{
			gNotice.GCNoticeSendToAll(0,0,0,0,0,0, gMessageNew.GetMessage(513));
		}

		if(this->m_RouletteRemainTime == 4)
		{
			gNotice.GCNoticeSendToAll(0,0,0,0,0,0, gMessageNew.GetMessage(514));
		}
		if(this->m_RouletteRemainTime == 2)
		{
			gNotice.GCNoticeSendToAll(0,0,0,0,0,0, gMessageNew.GetMessage(515));
		}
		...
		if(this->m_RouletteRemainTime <= 0)
		{
				...
				if((GetLargeRand()%100) <= 25)
				{
					...
					gNotice.GCNoticeSendToAll(0,0,0,0,0,0, gMessageNew.GetMessage(516),lpObj->Name);
					LogAdd(LOG_EVENT,"[EVENT RUSSIAN ROULETTE] %s Removed",lpObj->Name);
				}
				else 
				{
					gNotice.GCNoticeSendToAll(0,0,0,0,0,0, gMessageNew.GetMessage(517),lpObj->Name);
					LogAdd(LOG_EVENT,"[EVENT RUSSIAN ROULETTE] %s Saved",lpObj->Name);
						...
						if (this->m_Members == 1)
						{
							...
							gNotice.GCNoticeSendToAll(0,0,0,0,0,0, gMessageNew.GetMessage(511),lpObj->Name);
							...
							{	
								GDSetCoinSend(lpObj->Index, gServerInfo.m_EventRussianRouletteAutoReward1, gServerInfo.m_EventRussianRouletteAutoReward2, gServerInfo.m_EventRussianRouletteAutoReward3, 0, 0,"RussianR.");
							}
...
void CEventRussianRoulette::CommandEventRussianRoulette(LPOBJ lpObj,char* arg) 
{
	...
	gNotice.GCNoticeSendToAll(0,0,0,0,0,0, gMessageNew.GetMessage(508),gServerInfo.m_EventRussianRouletteMaxPlayer);
	gNotice.GCNoticeSendToAll(0,0,0,0,0,0, gMessageNew.GetMessage(509));
...
void CEventRussianRoulette::CommandEventRussianRouletteJoin(LPOBJ lpObj,char* arg) 
{
	...
	gNotice.GCNoticeSendToAll(0,0,0,0,0,0, gMessageNew.GetMessage(510),this->m_Members, lpObj->Name);
