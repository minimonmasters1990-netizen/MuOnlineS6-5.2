...
bool CMercenary::CreateMercenary(int aIndex,int MonsterClass,BYTE x,BYTE y)
{
	...
	{
		gNotice.GCNoticeSend(iIndex,1,0,0,0,0,0,gMessageNew.GetMessage(464));
		return FALSE;
	}
	...
	if( iMercenaryTypeIndex == 286 || iMercenaryTypeIndex == 287)
	{
		...
		{
			gNotice.GCNoticeSend(iIndex,1,0,0,0,0,0, gMessageNew.GetMessage(465));
			return FALSE;
		}
		if(lpObj->GuildStatus != 0x80 && lpObj->GuildStatus != 0x40)
		{
			gNotice.GCNoticeSend(iIndex,1,0,0,0,0,0, gMessageNew.GetMessage(466));
				return FALSE;
		}
	}
	...
	{
		gNotice.GCNoticeSend(iIndex,1,0,0,0,0,0, gMessageNew.GetMessage(467));
		return FALSE;
	}
	...
	if(iMonsterIndex >= 0)
	{
		...
		gNotice.GCNoticeSend(iIndex,1,0,0,0,0,0, gMessageNew.GetMessage(468));
	...
	{
		gNotice.GCNoticeSend(iIndex,1,0,0,0,0,0, gMessageNew.GetMessage(469));
		return FALSE;
	}
