...
void CViewport::GCViewportMonsterSend(int aIndex)
{
	...
	for(int n=0;n < MAX_VIEWPORT;n++)
	{
		...
		info.CurHp = (BYTE)((lpTarget->Life * 100) / (lpTarget->MaxLife + lpTarget->AddLife));
...
void CViewport::GCViewportSimpleMonsterSend(LPOBJ lpObj)
{
	...
	info.CurHp		= (BYTE)((lpObj->Life * 100) / (lpObj->MaxLife + lpObj->AddLife));
	info.Level		= lpObj->Level;
	info.Life		= lpObj->Life;
...
bool CViewport::CheckCustomEventPkViewport(LPOBJ lpObj, LPOBJ lpTarget)
{
	...
	if (gCastleStartGuild.CheckPlayerTarget(lpObj, lpTarget, 0) == 1)
	{
		return 1;
	}
