...
void CEventKillAll::MainProc()
{
	...
	{ 
		...
		if(this->m_RemainTime <= (this->m_TotalTime-gServerInfo.m_EventKillAllTimeToEnter) && this->m_EventKillAllEnter == 1)
		{
			if (this->m_Members < 2)
			{
				gNotice.GCNoticeSendToAll(0,0,0,0,0,0, gMessageNew.GetMessage(608));
			...
			{
				gNotice.GCNoticeSendToAll(0,0,0,0,0,0, gMessageNew.GetMessage(605));
				gNotice.GCNoticeSendToAll(0,0,0,0,0,0, gMessageNew.GetMessage(606));
		...
		{
			    this->m_EventKillAllActive = 0;
				gNotice.GCNoticeSendToAll(0,0,0,0,0,0, gMessageNew.GetMessage(607));
			...
			for(int n=0;n < MAX_KILLALL_USER;n++)
			{
				if(OBJECT_RANGE(this->User[n].Index) != 0)
				{
					if(this->User[n].Rank == 1)
					{
						...
						gNotice.GCNoticeSendToAll(0,0,0,0,0,0, gMessageNew.GetMessage(612),gObj[this->User[n].Index].Name,this->User[n].Score);
					...
					if(this->User[n].Rank == 2)
					{
						...
						gNotice.GCNoticeSendToAll(0,0,0,0,0,0, gMessageNew.GetMessage(613),gObj[this->User[n].Index].Name,this->User[n].Score);
					...
					if(this->User[n].Rank == 3)
					{
						...
						gNotice.GCNoticeSendToAll(0,0,0,0,0,0, gMessageNew.GetMessage(614),gObj[this->User[n].Index].Name,this->User[n].Score);

						LogAdd(LOG_EVENT,"[EVENT PVP] (Winner3: %s)",gObj[this->User[n].Index].Name);
					}

					gNotice.GCNoticeSend(this->User[n].Index,1,0,0,0,0,0, gMessageNew.GetMessage(615),this->User[n].Rank,this->User[n].Score);
...
bool CEventKillAll::CommandEventKillAll(LPOBJ lpObj,char* arg) 
{
	...
	gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessageNew.GetMessage(609),gServerInfo.m_EventKillAllMaxPlayers);
	gNotice.GCNoticeSendToAll(0,0,0,0,0,0, gMessageNew.GetMessage(610));
...
void CEventKillAll::CommandEventKillAllJoin(LPOBJ lpObj,char* arg) 
{
	...
	gNotice.GCNoticeSendToAll(0,0,0,0,0,0, gMessageNew.GetMessage(611),this->m_Members, lpObj->Name);
...
void CEventKillAll::UserDieProc(LPOBJ lpObj,LPOBJ lpTarget)
{
	...
	for(int n=0;n < MAX_KILLALL_USER;n++)
	{
		if(this->User[n].Index == lpObj->Index)
		{
			...
			gNotice.GCNoticeSend(this->User[n].Index,1,0,0,0,0,0, gMessageNew.GetMessage(616),this->User[n].Score);
		}
