...
void CCrywolfAltar::ResetAltarUserIndex(int AltarObjIndex, int Class)
{
	...
	{
		gNotice.GCNoticeSend(lpAltarInfo->m_UserIndex, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(399), AltarNumber);
...
bool CCrywolfAltar::SetAltarUserIndex(int AltarObjIndex, int Class, int UserIndex)
{
	...
	if (gObj[lpAltarInfo->m_UserIndex].Connected == OBJECT_ONLINE)
	{
		gNotice.GCNoticeSend(UserIndex, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(392), AltarNumber);
		return 0;
	}

	if ((CurrentTime - lpAltarInfo->m_LastValidContractTime) < 10000)
	{
		gNotice.GCNoticeSend(UserIndex, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(393), AltarNumber);
		return 0;
	}

	if (lpAltarInfo->m_ContractCount > MAX_CRYWOLF_ALTAR_CONTRACT)
	{
		gNotice.GCNoticeSend(UserIndex, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(394), AltarNumber);
		return 0;
	}

	if (abs(gObj[AltarObjIndex].X - gObj[UserIndex].X) > 0 || abs(gObj[AltarObjIndex].Y - gObj[UserIndex].Y) > 0)
	{
		gNotice.GCNoticeSend(UserIndex, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(395), AltarNumber);
		return 0;
	}
	...
	gCrywolfUtil.SendCrywolfUserAnyMsg(0, gMessageNew.GetMessage(396), gObj[lpAltarInfo->m_UserIndex].Name, AltarNumber);
	...
	gNotice.GCNoticeSend(UserIndex, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(397));
	gNotice.GCNoticeSend(UserIndex, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(398));
	return 1;
}
...
void CCrywolfAltar::SetAltarValidContract(int AltarObjIndex, int Class)
{
	...
	{
		gNotice.GCNoticeSend(lpAltarInfo->m_UserIndex, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(390), AltarNumber);

		gCrywolfUtil.SendCrywolfUserAnyMsg(0, gMessageNew.GetMessage(391), gObj[lpAltarInfo->m_UserIndex].Name, AltarNumber);
