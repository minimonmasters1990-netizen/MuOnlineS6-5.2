...
void CGvGEvent::ProcState_EMPTY()
{
	if (this->m_RemainTime > 0 && this->m_RemainTime <= (this->m_WarningTime * 60))
	{
		...
		if ((this->AlarmMinSave = (((this->m_RemainTime % 60) == 0) ? ((this->m_RemainTime / 60) - 1) : (this->m_RemainTime / 60))) != this->AlarmMinLeft)
		{
			...
			gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessageNew.GetMessage(663), (this->AlarmMinLeft + 1));
		}
	}

	if (this->m_RemainTime <= 0)
	{
		gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessageNew.GetMessage(665));
		this->NoticeSendToAll(1, gMessageNew.GetMessage(666), this->m_StandTime);
...
void CGvGEvent::ProcState_STAND()
{
	...
	{
		this->NoticeSendToAll(0, gMessageNew.GetMessage(575));
		this->SetState(GVG_EVENT_STATE_EMPTY);
		return;
	}
	...
	if (this->m_RemainTime <= 0)
	{
		if (this->TotalPlayer >= gServerInfo.m_GvGEventMinUsers)
		{
			...
			this->NoticeSendToAll(0, gMessageNew.GetMessage(667));
			this->SetState(GVG_EVENT_STATE_START);
		}
		else
		{
			this->NoticeSendToAll(0, gMessageNew.GetMessage(668));
			this->SetState(GVG_EVENT_STATE_EMPTY);
		}
	}
}
...
void CGvGEvent::SetState_BLANK()
{

}

void CGvGEvent::SetState_EMPTY()
{
	this->EnterEnabled = 0;
	this->AlarmMinSave = -1;
	this->AlarmMinLeft = -1;
	...
	for (int n = 0; n < MAX_GVGEVENT_USER; n++)
	{
		if (OBJECT_RANGE(this->User[n].Index) != 0)
		{
			...
			if (this->User[n].Guild == this->Winner)
			{
				...
				{
					GDSetCoinSend(lpObj->Index, this->Coin1, this->Coin2, this->Coin3, 0, 0, "GvGEvent");
				}
...
bool CGvGEvent::Dialog(LPOBJ lpObj, LPOBJ lpNpc)
{
	...
	{
		GCChatTargetSend(lpObj, lpNpc->Index, gMessageNew.GetMessage(664));
...
bool CGvGEvent::CheckEnterEnabled(LPOBJ lpObj)
{
	...
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(669));
		return 0;
	}
	...
	if (lpObj->Guild == 0)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(572));
		return 0;
	}
	...
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(670));
		return 0;
	}

	if (lpObj->GuildStatus == G_MASTER && this->CheckReqItems(lpObj) == 0)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(571));
		return 0;
	}
	...
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(252), this->MinLevel);
		return 0;
	}

	if (this->MaxLevel != -1 && this->MaxLevel < lpObj->Level)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(256), this->MaxLevel);
		return 0;
	}

	if (this->MinReset != -1 && this->MinReset > lpObj->Reset)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(254), this->MinReset);
		return 0;
	}

	if (this->MaxReset != -1 && this->MaxReset < lpObj->Reset)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(255), this->MaxReset);
		return 0;
	}

	if (this->MinMasterReset != -1 && this->MinMasterReset > lpObj->MasterReset)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(618), this->MinMasterReset);
		return 0;
	}

	if (this->MaxMasterReset != -1 && this->MaxMasterReset < lpObj->MasterReset)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(619), this->MaxMasterReset);
		return 0;
	}

	if (lpObj->GuildStatus != G_MASTER && this->CheckGuildMaster(lpObj->GuildNumber) == 0)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(573), this->MaxMasterReset);
		return 0;
	}

	if (lpObj->GuildStatus == G_MASTER && this->AddGuild(lpObj->GuildNumber) == 0)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(669), this->MaxMasterReset);
		return 0;
	}
...
bool CGvGEvent::CheckReqItems(LPOBJ lpObj)
{
	...
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(633));
		return false;
	}
...
int CGvGEvent::GetEnterEnabled()
{
	return this->EnterEnabled;
}

int CGvGEvent::GetEnteredUserCount()
{
	return this->GetUserCount();
}

bool CGvGEvent::CheckEnteredUser(int aIndex)
{
	return ((this->GetUser(aIndex) == 0) ? 0 : 1);
}

bool CGvGEvent::CheckPlayerTarget(LPOBJ lpObj)
{
...
bool CGvGEvent::CheckStandTarget(LPOBJ lpObj)
{
...
bool CGvGEvent::CheckPlayerJoined(LPOBJ lpObj, LPOBJ lpTarget)
{
...
bool CGvGEvent::AddUser(int aIndex)
{
...
bool CGvGEvent::AddGuild(int index)
{
	if (index <= 0)
	{
		return 0;
...
bool CGvGEvent::DelUser(int aIndex)
{
	if (OBJECT_RANGE(aIndex) == 0)
	{
		return 0;
...
GVG_EVENT_USER* CGvGEvent::GetUser(int aIndex)
{
	if (OBJECT_RANGE(aIndex) == 0)
	{
		return 0;
...
GVG_EVENT_GUILD* CGvGEvent::GetGuild(int aIndex)
{
	if (OBJECT_RANGE(aIndex) == 0)
	{
		return 0;
...
bool CGvGEvent::CheckGuildMaster(int index)
{
	if (index <= 0)
	{
		return 0;
...
bool CGvGEvent::GetUserRespawnLocation(LPOBJ lpObj, int* gate, int* map, int* x, int* y, int* dir, int* level)
{
	if (gServerInfo.m_GvGEventSwitch == 0)
	{
		return 0;
...
void CGvGEvent::UserDieProc(LPOBJ lpObj, LPOBJ lpTarget)
{
	if (gServerInfo.m_GvGEventSwitch == 0)
	{
		return;
	...
	this->NoticeSendToAll(0, gMessageNew.GetMessage(574), lpGuild->Point, lpTarget->GuildName);
}

void CGvGEvent::NoticeSendToAll(int type, char* message, ...)
{
	char buff[256];
	...
	for (int n = 0; n < MAX_GVGEVENT_GUILD; n++)
	{
		if (this->Guild[n].Guild >= 0)
		{
			...
			if (this->Guild[n].Rank == 1)
			{
				...
				if (lpGuildInfo != NULL)
				{
					this->NoticeSendToAll(0, gMessageNew.GetMessage(576), lpGuildInfo->Name);
				}
