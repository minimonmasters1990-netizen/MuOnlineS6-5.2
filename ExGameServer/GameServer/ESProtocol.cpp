	...
	if (lpMsg->btFlag == 1)
	{
		...
		for (int i = 0; i < MAX_GUILD_USER; i++)
		{
			if (lpNode->Use[i] > 0 && lpNode->Index[i] >= 0)
			{
				...
				{
					gNotice.GCNoticeSend(lpNode->Index[i], 0, 0, 0, 0, 0, 0, gMessageNew.GetMessage(432), lpMsg->szTargetName);
				}
				else if (lpMsg->btGuildStatus == GUILD_BATTLE_MASTER)
				{
					gNotice.GCNoticeSend(lpNode->Index[i], 0, 0, 0, 0, 0, 0, gMessageNew.GetMessage(433), lpMsg->szTargetName);
				}
				else
				{
					gNotice.GCNoticeSend(lpNode->Index[i], 0, 0, 0, 0, 0, 0, gMessageNew.GetMessage(434), lpMsg->szTargetName);
				}
			}
		}
	}
	else if (lpMsg->btFlag == 0)
	{
		...
		for (int n = 0; n < MAX_GUILD_USER; n++)
		{
			if (lpNode->Use[n] > 0 && lpNode->Index[n] >= 0)
			{
				if (lpMsg->btGuildStatus == GUILD_ASSISTANT)
				{
					gNotice.GCNoticeSend(lpNode->Index[n], 0, 0, 0, 0, 0, 0, gMessageNew.GetMessage(432), lpMsg->szTargetName);
				}
				else if (lpMsg->btGuildStatus == GUILD_BATTLE_MASTER)
				{
					gNotice.GCNoticeSend(lpNode->Index[n], 0, 0, 0, 0, 0, 0, gMessageNew.GetMessage(433), lpMsg->szTargetName);
				}
				else
				{
					gNotice.GCNoticeSend(lpNode->Index[n], 0, 0, 0, 0, 0, 0, gMessageNew.GetMessage(434), lpMsg->szTargetName);
				}
			}
		}
	}
...
void DGGuildAnsAssignType(EXSDHP_GUILD_ASSIGN_TYPE_RESULT* lpMsg)
{
	if (lpMsg->btFlag == G_TYPE_GUARD)
	{
		...
		for (int i = 0; i < MAX_GUILD_USER; i++)
		{
			if (lpNode->Use[i] > 0 && lpNode->Index[i] >= 0)
			{
				gNotice.GCNoticeSend(lpNode->Index[i], 0, 0, 0, 0, 0, 0, gMessageNew.GetMessage(435), lpMsg->szGuildName, lpMsg->btGuildType);
			}
		}
	}
	else if (lpMsg->btFlag == G_TYPE_COMMON)
	{
		...
		for (int n = 0; n < MAX_GUILD_USER; n++)
		{
			if (lpNode->Use[n] > 0 && lpNode->Index[n] >= 0)
			{
				gNotice.GCNoticeSend(lpNode->Index[n], 0, 0, 0, 0, 0, 0, gMessageNew.GetMessage(435), lpMsg->szGuildName, lpMsg->btGuildType);
			}
		}
	}
}
	...
	for (int n = 0; n < MAX_GUILD_USER; n++)
	{
		if (lpReqGuild->Use[n] > 0)
		{
			if (lpReqGuild->Index[n] >= 0)
			{
				if (lpMsg->btRelationShipType == G_RELATIONSHIP_UNION)
				{
					gNotice.GCNoticeSend(lpReqGuild->Index[n], 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(436), lpTargetGuild->Name);
				}
				else if (lpMsg->btRelationShipType == G_RELATIONSHIP_RIVAL)
				{
					gNotice.GCNoticeSend(lpReqGuild->Index[n], 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(437), lpTargetGuild->Name);
				}
			}
		}
	}
	...
	for (int n = 0; n < MAX_GUILD_USER; n++)
	{
		if (lpTargetGuild->Use[n] > 0)
		{
			if (lpTargetGuild->Index[n] >= 0)
			{
				if (lpMsg->btRelationShipType == G_RELATIONSHIP_UNION)
				{
					gNotice.GCNoticeSend(lpTargetGuild->Index[n], 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(438), lpReqGuild->Name, lpTargetGuild->Name);
				}
				else if (lpMsg->btRelationShipType == G_RELATIONSHIP_RIVAL)
				{
					gNotice.GCNoticeSend(lpTargetGuild->Index[n], 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(437), lpReqGuild->Name);
				}
			}
		}
	}
...
void DGUnionBreakOff(EXSDHP_RELATIONSHIP_BREAKOFF_RESULT* lpMsg)
{
	...
	for (int n = 0; n < MAX_GUILD_USER; n++)
	{
		if (lpReqGuild->Use[n] > 0)
		{
			if (lpReqGuild->Index[n] >= 0)
			{
				gNotice.GCNoticeSend(lpReqGuild->Index[n], 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(439));
			}
		}
	}
	...
	for (int n = 0; n < MAX_GUILD_USER; n++)
	{
		if (lpTargetGuild->Use[n] > 0)
		{
			if (lpTargetGuild->Index[n] >= 0)
			{
				gNotice.GCNoticeSend(lpTargetGuild->Index[n], 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(440), lpReqGuild->Name, lpTargetGuild->Name);
			}
		}
	}
}
	...
	for (int n = 0; n < MAX_GUILD_USER; n++)
	{
		if (lpReqGuild->Use[n] > 0)
		{
			if (lpReqGuild->Index[n] >= 0)
			{
				if (lpMsg->btRelationShipType == G_RELATIONSHIP_UNION)
				{
					gNotice.GCNoticeSend(lpReqGuild->Index[n], 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(441), lpTargetGuild->Name);
				}
				else if (lpMsg->btRelationShipType == G_RELATIONSHIP_RIVAL)
				{
					gNotice.GCNoticeSend(lpReqGuild->Index[n], 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(442), lpTargetGuild->Name);
				}
			}
		}
	}
		...
		if (lpGuildInfo)
		{
			...
			for (int n = 0; n < MAX_GUILD_USER; n++)
			{
				if (lpGuildInfo->Use[n] > 0)
				{
					...
					{
						gNotice.GCNoticeSend(lpGuildInfo->Index[n], G_NOTIFICATION_SET, 0, 0, 0, 0, 0, gMessageNew.GetMessage(443), lpGuildInfo->Name);
					}
				}
			}
		}
...
void DGRelationShipAnsKickOutUnionMember(EXSDHP_KICKOUT_UNIONMEMBER_RESULT* lpMsg)
{
	...
	if (lpUnionMasterGuild)
	{
		for (int n = 0; n < MAX_GUILD_USER; n++)
		{
			if (lpUnionMasterGuild->Use[n] > 0)
			{
				...
				{
					gNotice.GCNoticeSend(lpUnionMasterGuild->Index[n], 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(440), lpMsg->szUnionMemberGuildName, lpMsg->szUnionMasterGuildName);
				}
			}
		}
	}
	...
	for (int n = 0; n < MAX_GUILD_USER; n++)
	{
		if (lpUnionMemberGuild->Use[n] > 0)
		{
			...
			{
				gNotice.GCNoticeSend(lpUnionMemberGuild->Index[n], 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(439));
			}
		}
	}
}
