...
void InitLuaFunction(lua_State* L) // OK
{
	...
	lua_register(L, "SendTimeLimit", LuaSendTimeLimit);

	lua_register(L, "ServerCommandSend", LuaServerCommandSend);
}
...
int LuaChatTargetSend(lua_State* L) // OK
{
	...
	if (aIndex != -1)
	{
		...
		GCChatTargetSend(&gObj[aIndex], NPC, (char*)aString);
	}
	else
	{
		GCChatTargetSend(&gObj[aIndex], NPC, (char*)aString);
...
int LuaCommandCheckGameMasterLevel(lua_State* L) // OK
{
	...
	if (gGameMaster.CheckGameMasterLevel(&gObj[aIndex], aValue))
	{
		lua_pushinteger(L, 1);
	}
	else
...
int LuaItemDrop(lua_State* L) // OK
{
	...
	gItemBagManager.DropItemBySpecialValue(aValue, &gObj[aIndex], bValue, cValue, dValue);
...
int LuaServerCommandSend(lua_State* L) // OK
{
	if (lua_gettop(L) != 4)
	{
		return luaL_error(L, LUA_SCRIPT_CODE_ERROR1, 4);
...
int LuaMessageSend(lua_State* L) // OK
{
	...
	//GCNewMessageSend2(aIndex,aValue,bValue,"%s",aString);
	GCNewMessageSend(&gObj[aIndex], "teste");
	gNotice.NewMessageDevTeam(aIndex, "msg teste");

	// type = 1  msg azul lateral / 0 = meio tela / 2 = guild member
	gNotice.GCNoticeSend(aIndex, -1, 10, 0, 0, 0, 0, "Oieeeeeeeeeeeee");
	PostMessageUserON("TEST", gMessage.GetMessage(640));
	PostMessagePK("FUCK", "YEAH", "XX");

	PostMessage4("PST 4", "MSG4", "4");
	PostMessage3("PST 3", "MSG3", "3");
	PostMessage2("PST 2", "MSG2", "2");
	PostMessage1("PST 1", "MSG1", "1");
...
int LuaMessageSendToAll(lua_State* L) // OK
{
	...
	for (int n = OBJECT_START_USER; n < MAX_OBJECT; n++)
	{
		...
		{
			GCNewMessageSend2(n, aValue, bValue, "%s", aString);
		}
	}
...
int LuaObjectGetCoin(lua_State* L) // OK
{
	...
	lua_pushstring(L, "Coin4");
	lua_pushnumber(L, gObj[aIndex].Ruud);
	lua_settable(L, -3);
...
int LuaObjectAddCoin(lua_State* L) // OK
{
	...
	GDSetCoinSend(aIndex, aValue, bValue, cValue, "LUA/ADD/COIN");
...
int LuaObjectSubCoin(lua_State* L) // OK
{
	...
	GDSetCoinSend(aIndex, -aValue, -bValue, -cValue, "LUA/SUB/COIN");
...
int LuaSendTimeLimit(lua_State* L) // OK
{
	if (lua_gettop(L) != 2)
	{
		return luaL_error(L, LUA_SCRIPT_CODE_ERROR1, 2);
	...
	GCTimeLimitSend(&gObj[aIndex], aIndex, bIndex);
