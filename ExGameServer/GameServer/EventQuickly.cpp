...
void CEventQuickly::MainProc()
{
	...
	if (this->m_RemainTime > 0)
	{
		...
		if (this->MinutesLeft != minutes)
		{
			...
			gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessageNew.GetMessage(461), (MinutesLeft + 1));
		}

		if (this->m_RemainTime <= 5)
		{
			gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessageNew.GetMessage(462), m_RemainTime);
		}
		...
		if (this->m_RemainTime <= 0)
		{
			...
			this->Clear();
			gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessageNew.GetMessage(455));
...
void CEventQuickly::CommandEventQuickly(LPOBJ lpObj, char* arg) 
{
	...
	if (this->m_RemainTime > 0)
	{
		...
		gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessageNew.GetMessage(457));
		return;
	}
	...
	if (map < 0 || x <= 0 || y <= 0)
	{
		...
		//gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessageNew.GetMessage(456));
		//return;
	...
	gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessageNew.GetMessage(460), MapName);
	gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessageNew.GetMessage(456), x, y);
...
void CEventQuickly::ClickNPC(LPOBJ lpObj) 
{
	...
	gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessageNew.GetMessage(463), lpObj->Name);
