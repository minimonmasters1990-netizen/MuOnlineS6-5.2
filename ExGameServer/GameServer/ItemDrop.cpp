...
int CItemDrop::DropItem(LPOBJ lpObj,LPOBJ lpTarget)
{
	...
	else
	{
		...
		if (!gItemOptionRate.GetItemOption4(lpItemDropInfo->Option4, &ItemNewOption) || ItemNewOption == 0)
		{
			// 如果 GetItemOption4 返回 0 或 ItemNewOption 为 0，使用 XML 中配置的 ExcOption 值
			ItemNewOption = lpItemDropInfo->Option4;
		}
