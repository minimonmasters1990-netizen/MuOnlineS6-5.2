...
void CCrywolfStatue::CrywolfStatueAct(int aIndex)
{
	...
	if (ContractedAlterCount == 0 || this->m_Shield.m_ShieldHP == 0)
	{
		...
		{
			gCrywolfUtil.SendCrywolfUserAnyMsg(0, gMessageNew.GetMessage(400));
		...
		else if (this->m_Shield.m_PriestNumber != ContractedAlterCount)
		{
			...
			this->m_Shield.m_PriestNumber = ContractedAlterCount;
			gCrywolfUtil.SendCrywolfUserAnyMsg(0, gMessageNew.GetMessage(401), ContractedAlterCount, PriestHPSum);
			this->SetStatueViewState(lpObj, ContractedAlterCount);
		}
	...
	{
		gCrywolfUtil.SendCrywolfUserAnyMsg(0, gMessageNew.GetMessage(402));
		this->SetStatueViewState(lpObj, ContractedAlterCount);
	}
	else if (this->m_Shield.m_PriestNumber != ContractedAlterCount)
	{
