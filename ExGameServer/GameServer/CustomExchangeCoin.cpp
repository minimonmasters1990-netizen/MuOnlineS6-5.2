...
CCustomExchangeCoin::CCustomExchangeCoin()
{
	this->m_CustomExchangeCoinInfo.clear();
}

void CCustomExchangeCoin::Load(char* path)
{
...
bool CCustomExchangeCoin::GetInfo(int index,CUSTOMEXCHANGECOIN_INFO* lpInfo)
{
...
bool CCustomExchangeCoin::GetInfoByName(LPOBJ lpObj, char* message, int Npc)
{
	char command[32] = {0};
	...
	for(std::map<int,CUSTOMEXCHANGECOIN_INFO>::iterator it=this->m_CustomExchangeCoinInfo.begin();it != this->m_CustomExchangeCoinInfo.end();it++)
	{
		...
		{
			if (it->second.RequireCoin1 > 0 && it->second.RequireCoin1 > lpObj->ThisCoin[0])
			{
				gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0, gMessageNew.GetMessage(577), it->second.RequireCoin1);
				if (Npc >= 0)
				{
					GCChatTargetNewSend(lpObj,Npc, gMessageNew.GetMessage(577), it->second.RequireCoin1);
				}
				return 1;
			}
			if (it->second.RequireCoin2 > 0 && it->second.RequireCoin2 > lpObj->ThisCoin[1])
			{
				gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0, gMessageNew.GetMessage(578), it->second.RequireCoin2);
				if (Npc >= 0)
				{
					GCChatTargetNewSend(lpObj,Npc, gMessageNew.GetMessage(578), it->second.RequireCoin2);
				}
				return 1;
			}
			if (it->second.RequireCoin3 > 0 && it->second.RequireCoin3 > lpObj->ThisCoin[2])
			{
				gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0, gMessageNew.GetMessage(579), it->second.RequireCoin3);
				if (Npc >= 0)
				{
					GCChatTargetNewSend(lpObj,Npc, gMessageNew.GetMessage(579), it->second.RequireCoin3);
				}
				return 1;
			}

			gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessageNew.GetMessage(653));
			if (Npc >= 0)
			{
				GCChatTargetNewSend(lpObj,Npc, gMessageNew.GetMessage(653));
			}
			...
			GDSetCoinSend(lpObj->Index, Coin1, Coin2, Coin3, 0, 0,"ExchangeCoin");
			return 1;
		}
	}
