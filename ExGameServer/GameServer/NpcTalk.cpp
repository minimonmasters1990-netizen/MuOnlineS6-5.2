...
bool CNpcTalk::NpcTalk(LPOBJ lpNpc,LPOBJ lpObj)
{
	...
	if (gCastleStartGuild.Dialog(lpObj->Index, lpNpc->Index) != 0)
	{
		return 1;
	}
...
void CNpcTalk::NpcCastleSiegeCrownSwitch(LPOBJ lpNpc,LPOBJ lpObj)
{
	...
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessageNew.GetMessage(427));
		return;
	}
...
void CNpcTalk::NpcCastleSiegeSenior(LPOBJ lpNpc,LPOBJ lpObj)
{
	...
	GCChatTargetSend(lpObj,lpNpc->Index,gMessageNew.GetMessage(258));
	...
	if(gCastleSiege.GetCastleState() == CASTLESIEGE_STATE_STARTSIEGE)
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessageNew.GetMessage(259));
		return;
	}

	if(gCastleSiege.CheckGuildOwnCastle(lpObj->GuildName) == 0)
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessageNew.GetMessage(260));
		return;
	}

	if(lpObj->GuildStatus != 0x40 && lpObj->GuildStatus != 0x80)
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0, gMessageNew.GetMessage(261));
		return;
	}
...
{
	GCChatTargetSend(lpObj,lpNpc->Index, gMessageNew.GetMessage(314));
...
void CNpcTalk::NpcCastleSiegeGuardsman(LPOBJ lpNpc,LPOBJ lpObj)
{
	...
	if(gCastleSiege.GetCastleState() == CASTLESIEGE_STATE_STARTSIEGE)
	{
		GCChatTargetSend(lpObj,lpNpc->Index,gMessageNew.GetMessage(262));
		return;
	}
...
void CNpcTalk::NpcGuildMaster(LPOBJ lpNpc,LPOBJ lpObj)
{
	if(lpObj->GuildNumber > 0)
	{
		if (gServerInfo.m_GuildWarehouseEnable == 1)
		{
			...
			{
				gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0, gMessageNew.GetMessage(640));
			}
			else
			{
		...
		{
			gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessageNew.GetMessage(263));
		}
		return;
	}
	...
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessageNew.GetMessage(264));
		return;
	}
...
{
	gEffectManager.AddEffect(lpObj, 1, EFFECT_MANA_SHIELD, gServerInfo.m_TimeBuff * 60, gServerInfo.m_Buff1, 0, 0, 0);
	gEffectManager.AddEffect(lpObj, 1, EFFECT_GREATER_LIFE, gServerInfo.m_TimeBuff * 60, gServerInfo.m_Buff2, 0, 0, 0);
	gEffectManager.AddEffect(lpObj, 1, EFFECT_GREATER_DEFENSE, gServerInfo.m_TimeBuff * 60, gServerInfo.m_Buff3, 0, 0, 0);
	gEffectManager.AddEffect(lpObj, 1, EFFECT_GREATER_DAMAGE, gServerInfo.m_TimeBuff * 60, gServerInfo.m_Buff4, 0, 0, 0);
	gEffectManager.AddEffect(lpObj, 1, EFFECT_GREATER_CRITICAL_DAMAGE, gServerInfo.m_TimeBuff * 60, gServerInfo.m_Buff5, 0, 0, 0);
	gEffectManager.AddEffect(lpObj, 1, EFFECT_DAMAGE_REFLECT, gServerInfo.m_TimeBuff * 60, gServerInfo.m_Buff6, 0, 0, 0);
	gEffectManager.AddEffect(lpObj, 1, EFFECT_FITNESS, gServerInfo.m_TimeBuff * 60, gServerInfo.m_Buff7, 0, 0, 0);
	//gEffectManager.AddEffect(lpObj, 1, EFFECT_GREATER_IGNORE_DEFENSE_RATE, gServerInfo.m_TimeBuff * 60, gServerInfo.m_Buff8, 0, 0, 0);
	//gEffectManager.AddEffect(lpObj, 1, EFFECT_GREATER_DEFENSE_SUCCESS_RATE, gServerInfo.m_TimeBuff * 60, gServerInfo.m_Buff9, 0, 0, 0);
...
void CNpcTalk::CGNpcTalkRecv(PMSG_NPC_TALK_RECV* lpMsg,int aIndex)
{
	...
	{
		GCChatTargetSend(lpObj,lpNpc->Index, gMessageNew.GetMessage(661));
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0, gMessageNew.GetMessage(661));
		return;
	}

	if(gShopManager.GetItemCountByIndex(lpNpc->ShopNumber) == 0)
	{
		GCChatTargetSend(lpObj,lpNpc->Index,gMessageNew.GetMessage(265));
		return;
	}
...
void CNpcTalk::CGNpcTalkCloseRecv(int aIndex)
{
	...
	// Nếu không dùng giao diện, hoặc không có giao diện, hoặc là Chaos Box thì bỏ qua
	if (lpObj->Interface.use == 0 ||
	...
	case INTERFACE_SHOP:
	{
		// Nếu đang mở shop thì đóng lại
		if (lpObj->Interface.state == 1)
		{
			lpObj->Interface.state = 0;
		}
	...
	// Nếu cần thêm interface mới => bổ sung ở đây
	}

	// 🧹 Dọn dẹp toàn bộ thông tin sau khi đóng giao diện
