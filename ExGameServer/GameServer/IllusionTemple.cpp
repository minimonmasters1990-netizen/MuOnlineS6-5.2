...
void CIllusionTemple::ProcState_EMPTY(ILLUSION_TEMPLE_LEVEL* lpLevel)
{
	if (lpLevel->RemainTime > 0 && lpLevel->RemainTime <= (this->m_WarningTime * 60))
	{
		...
		if (lpLevel->MinutesLeft != minutes)
		{
			...
			if (lpLevel->Level == 0)
			{
				gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessageNew.GetMessage(238), (lpLevel->MinutesLeft + 1));
			}
		}
	...
	if (lpLevel->RemainTime <= 0)
	{
		if (lpLevel->Level == 0) 
		{ 
			gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessageNew.GetMessage(239)); 
		}

		this->NoticeSendToAll(lpLevel, 1, gMessageNew.GetMessage(240), (lpLevel->Level + 1), this->m_NotifyTime);
...
void CIllusionTemple::ProcState_STAND(ILLUSION_TEMPLE_LEVEL* lpLevel)
{
	...
	if (lpLevel->RemainTime <= 0)
	{
		this->NoticeSendToAll(lpLevel, 0, gMessageNew.GetMessage(241));
...
void CIllusionTemple::ProcState_START(ILLUSION_TEMPLE_LEVEL* lpLevel)
{
	...
	if (lpLevel->RemainTime <= 0)
	{
		this->NoticeSendToAll(lpLevel, 0, gMessageNew.GetMessage(242));
...
void CIllusionTemple::SetState_CLEAN(ILLUSION_TEMPLE_LEVEL* lpLevel)
{
	...
	{
		this->NoticeSendToAll(lpLevel, 0, gMessageNew.GetMessage(569));
	}
	else
	{
		this->NoticeSendToAll(lpLevel, 0, gMessageNew.GetMessage(570));
	}
...
void CIllusionTemple::NpcMirageTheMummy(LPOBJ lpNpc, LPOBJ lpObj)
{
	...
	if (IT_LEVEL_RANGE(level) == 0)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(243));
		return;
	}
	...
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(244));
		return;
	}
...
#if (GAMESERVER_UPDATE <= 603)
	this->NoticeSendToAll(&this->m_IllusionTempleLevel[level], 0, gMessageNew.GetMessage(245), gObj[aIndex].Name);
#endif
}
...
void CIllusionTemple::GetEventItem(int map, int aIndex, CMapItem* lpItem)
{
	...
	this->NoticeSendToAll(&this->m_IllusionTempleLevel[level], 0, gMessageNew.GetMessage(246), gObj[aIndex].Name);
}
...
void CIllusionTemple::CGIllusionTempleEnterRecv(PMSG_ILLUSION_TEMPLE_ENTER_RECV* lpMsg, int aIndex)
{
	...
	if (this->GetEnterEnabled(lpMsg->level) == 0)
	{
		pMsg.result = 3;
		DataSend(aIndex, (BYTE*)&pMsg, pMsg.header.size);
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessageNew.GetMessage(267));
